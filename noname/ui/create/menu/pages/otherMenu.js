...
import { formatBuildLabel } from "../../../../util/meta.js";
...
const otherMenu = function(connectMenu) {
  ...
  const mountHelp = function(node2) {
    const page = node2.link;
    if (!page) {
      return;
    }
    const content = lib.help[node2.helpName];
    node2._helpPersist = false;
    node2._helpTeardown = null;
    if (content && typeof content === "object") {
      if (typeof content.mount === "function") {
        const teardown = content.mount(page);
        if (teardown && typeof teardown.then === "function") {
          console.warn(`lib.help["${node2.helpName}"] 的 mount 返回了 Promise，帮助页只支持同步 teardown，该返回值将被忽略`);
          node2._helpPersist = true;
          return;
        }
        if (typeof teardown === "function") {
          node2._helpTeardown = teardown;
          return;
        }
        node2._helpPersist = true;
        return;
      ...
      if (typeof content.data === "function" || typeof content.setup === "function") {
        ...
        if (content.persist === false) {
          node2._helpTeardown = () => app.unmount();
          return;
        }
        node2._helpPersist = true;
        return;
  ...
  const teardownHelp = function(node2) {
    node2._helpTeardown?.();
    node2._helpTeardown = null;
    node2._helpPersist = false;
    node2.link?.replaceChildren();
  };
  var clickMode = function() {
    ...
    if (active2) {
      ...
      if (active2.type === "help" && !active2._helpPersist) {
        teardownHelp(active2);
      }
      active2.link?.remove();
    }
    active2 = this;
