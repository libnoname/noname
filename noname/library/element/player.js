...
class Player extends HTMLDivElement {
  ...
  addTip(index, message, isTemp = false, css = {}, nobroadcast) {
    ...
    const func = (player2, index2, message2, css2) => {
    ...
    if (args.length == 1 && params != null && get.is.object(params) && get.itemtype(params) == null) {
      ...
      if (params?.areaNames != null) {
        ...
        for (const areaName of params.areaNames) {
          const item = commonAreas.find(([name2, area]) => name2 === areaName || area?.fromName === areaName);
          if (item != null) {
    ...
    if (get.itemtype(name2) == "cards") {
      ...
      if (get.itemtype(name2) == "card") {
        ...
        if (lib.skill[name2] && lib.skill[name2].markimage) {
          ...
          let str = lib.translate[name2 + "_bg"];
          if (!str || str[0] == "+" || str[0] == "-") {
            str = get.translation(name2)[0];
          }
          ui.create.div(".background.skillmark", node).innerHTML = str;
          if (lib.skill[name2] && get.is.zhuanhuanji(name2, this) && str == "☯") {
            const zhuanhuanLimit = get.zhuanhuanItemNum(name2, this);
            const storage = this.storage[name2];
            let index;
            if (zhuanhuanLimit == 2) {
              if (get.info(name2).zhuanhuanji == "number" || typeof storage == "number") {
                index = this.countMark(name2) % zhuanhuanLimit;
              } else {
                index = storage ? 1 : 0;
              }
            } else {
...
hasSkillTag：`, arg);
   ...
   */
  hasJudge(name2, hasJudgeSlots = true) {
    ...
    return judges.some((card) => {
      ...
      if (hasJudgeSlots && get.judgeSlots(card, this).containsSome(...get.judgeSlots(name2, this))) {
        return true;
      }
    });
  }
