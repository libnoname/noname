...
const defaultHooks = {
  ...
  addNature: [
    function addColor2(nature, _translation, config) {
  ...
  checkTarget: [
    ...
    function addTargetPrompt(target, event) {
      ...
      const str = event.targetprompt2.flatMap((func) => func(target) || "").filter((prompt) => prompt.length).toUniqued().join("<br>");
      let node;
  ...
  uncheckEnd: [],
  checkOverflow: [
    function updateDialog(itemOption, itemContainer, addedItems, game2) {
      const gap = 3;
