...
import { DefaultFileSystemAdapter } from "./fs/adapter.js";
...
class Library {
      ...
      config: {
        ...
        ui_zoom: {
          ...
          onblur(e) {
            ...
            rollbackTimeout = setTimeout(() => finish(false), 1e4);
          }
        },
        image_background: {
          name: "游戏背景",
  ...
  filter = {
     ...
     * @returns { boolean }
     */
    characterDisabled(name2, _characters, ...options) {
      const character = lib.character[name2];
      if (!character || character.isUnseen) {
        return true;
      }
      if (!options.includes("ignoreForibidden")) {
        if (lib.config.forbidai.includes(name2) || character.isAiForbidden) {
          return true;
        }
      }
      if (lib.characterFilter[name2] && !lib.characterFilter[name2](get.mode())) {
        return true;
      }
      if (_status.connectMode) {
        if (lib.configOL.banned.includes(name2) || lib.connectBanned.includes(name2)) {
          return true;
        }
        ...
        if (doubleCharacter && lib.config.forbiddouble.includes(name2)) {
          return true;
        }
      } else {
        if (lib.config.banned.includes(name2)) {
          return true;
        }
     ...
     * @returns { boolean }
     */
    characterDisabled2(name2, ...options) {
      const character = lib.character[name2];
      if (!character) {
        return true;
      }
    ...
    selectTarget(card, player) {
      ...
      if (select === void 0 || select === null) {
        if (info.filterTarget === void 0 || info.filterTarget === null) {
          return [0, 0];
        }
        range = [1, 1];
      } else if (typeof select === "number") {
