...
import { FileSystem } from "../library/fs/index.js";
import { installLegacyFileSystemAPI } from "../library/fs/legacy.js";
import { FileSystemErrorCode, FileSystemError } from "../library/fs/errors.js";
...
class BrowserAdapter {
  ...
  async write(path, data) {
    ...
    await requestBackend("/writeFile", path, void 0, {
      method: "POST",
...
function describeBackendResponse(response) {
  ...
  return /^[a-z0-9!#$&^_.+-]+\/[a-z0-9!#$&^_.+-]+$/.test(mediaType) ? `HTTP ${status}, Content-Type: ${mediaType}` : `HTTP ${status}`;
}
