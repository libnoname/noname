...
const skills = {
	...
	mbjili: {
		...
		async content(event, trigger, player) {
			...
			if (!trigger.mbjili) trigger.mbjili = {};
		...
		subSkill: {
			effect: {
				...
				async content(event, trigger, player) {
					...
					if (count < num) {
						if (num < 4) await player.draw(4 - num);
					} else if (count == num) {
						if (num > 0 && player.countCards("he")) await player.chooseToGive(evt.player, num, true, "he");
					} else {
						...
						if (player.canUse(card, evt.player, false)) {
							const bool = await player
								...
								.forResultBool();
							if (bool) await player.useCard(card, evt.player, false);
						}
					}
				},
			},
	...
	mbshishu: {
		...
		filter(event, player) {
			if (event.name != "gain" && event.type != "gain") return false;
		...
		async content(event, trigger, player) {
			...
			if (number == min || number == max) {
				...
				if (!player.hasSkill("mbjinzu_used")) {
					...
					if (stat.mbjinzu) delete stat.mbjinzu;
				}
			} else {
	...
	mbanxian: {
		...
		filter(event, player) {
			...
			if (!evt?.hs?.some(i => get.position(i, true) == "d" && i.name == "sha")) return false;
		...
		async content(event, trigger, player) {
			...
			if (player.hasUseTarget(cards[0], false, false)) await player.chooseUseTarget(cards[0], true, false, "nodistance");
		},
		group: "mbanxian_draw",
		subSkill: {
			draw: {
	...
	mbfeijing: {
		...
		filter(event, player) {
			...
			if (event.targets?.length != 1 || !event.target?.isIn()) return false;
				...
				const result = await player
					...
					.set("ai", button => {
						...
						for (let target of targets) {
							if (lib.filter.targetEnabled2(trigger.card, player, target)) eff += get.effect(target, trigger.card, player, player);
						}
						return eff;
					})
					.forResult();
			...
			if (targets.length) {
				...
				if (result2.bool) {
					...
					if (targetx.length) {
						...
						if (!trigger.getParent().feijingExtra) trigger.getParent().feijingExtra = [];
		...
		subSkill: {
			viewas: {
				...
				viewAsFilter(player) {
					if (!player.countCards("hes", card => get.type2(card) == "trick" && get.tag(card, "damage"))) return false;
				},
		...
		async content(event, trigger, player) {
			if (trigger.name == "damage") {
				...
				target.getHistory("lose", evtx => {
					...
					if (evtv?.name != "mbfeijing") return false;
					if (evtv?.getTrigger()?.getParent() != evt) return false;
	...
	mbqingdao: {
		...
		async content(event, trigger, player) {
			...
			if (link == "sha" || link == "shan") {
				...
				else player.chat(`孩子们，一张${get.translation(link)}都没有力`);
			}
			if (link == "discard" && targets.length) {
				player.line(targets);
				if (!targets[0].countDiscardableCards(player, "hej")) return;
	...
	mbchenshe: {
		...
		async content(event, trigger, player) {
			...
			for (const target of targets) {
				...
				if (target == player) result = await target.chooseToDiscard(`陈赦：请弃置一张牌`, "he", true).forResult();
				else result = await player.discardPlayerCard(`陈赦：请弃置${get.translation(target)}一张牌`, target, "he", true).forResult();
	...
	mbchizhang: {
		...
		async cost(event, trigger, player) {
			event.result = await player
				...
				.set("ai", card => {
					...
					if (suits?.includes(get.suit(card, get.player()))) return 0;
		...
		async content(event, trigger, player) {
			const cards = event.cards,
				suits = cards.map(card => get.suit(card, player)).unique(),
				targets = game.filterPlayer(target => target != player);
			await player.discard(cards);
			for (const target of targets) target.addTempSkill(event.name + "_global");
			if (!trigger.card.storage) trigger.card.storage = {};
			trigger.card.storage.mbchizhang = [targets, suits];
		},
		subSkill: {
			global: {
				charlotte: true,
				mod: {
					cardEnabled(card, player) {
						...
						if (!evt?.respondTo || !evt.respondTo[1]?.storage?.mbchizhang) return;
						const suit = get.suit(card, player),
							suits = evt.respondTo[1].storage.mbchizhang[1],
							targets = evt.respondTo[1].storage.mbchizhang[0];
						if (suit === "unsure" || !targets.includes(player)) return;
		...
		subSkill: {
			use: {
				...
				async content(event, trigger, player) {
					for (const card of player.getExpansions("mbduanyang")) {
						...
						player.$gain2(card);
						await player.chooseUseTarget(card, 1, true, false);
					}
			...
			damage: {
				...
				filter(event, player) {
					...
					return event.card?.name == "sha" && target.isIn() && target.countCards("hej", card => target.canRecast(card));
				},
				async cost(event, trigger, player) {
					const target = trigger.player;
				...
				async content(event, trigger, player) {
					...
					const basic = cards.filter(card => get.type(card, false) == "basic" && !get.owner(card));
					let result;
					if (!basic?.length) result = { bool: true, control: "选项二" };
					else {
						result = await player
							.chooseControl()
	...
	mbganggeng: {
		...
		delay: false,
		check(card) {
			if (get.player().countCards("h") < 3) 8 - get.value(card);
	...
	mbsijian: {
		...
		async cost(event, trigger, player) {
			...
			const result = await player
				...
				.set("filterButton", button => {
					...
					if (button.link == "discard") return bool1;
					if (button.link == "draw") return bool2;
					if (button.link == "both") return (bool1 || bool2) && !_status.dying.length;
				})
				.set("ai", button => {
					if (button.link == "draw") {
						const target = _status.currentPhase;
						if (target?.isIn() && get.attitude(get.player(), target) > 0) return 2;
						return 0;
					}
					if (button.link == "both") {
						if (get.event().count > 1) return 0;
						return 3;
					}
					return 1;
				})
		...
		subSkill: {
			discard: {
				...
				content() {
					player.removeSkill(event.name);
					if (player.countDiscardableCards(player, "he")) player.chooseToDiscard("he", true);
				},
				intro: {
					content: "下次使用牌后弃置一张牌",
				},
					...
					async content(event, trigger, player) {
						...
						const result = await player
							.chooseButtonTarget({
								...
								filterTarget(card, player, target) {
									if (get.event().used) return !target.hasHistory("gain", evt => evt.cards.length);
									return true;
								},
								ai1(button) {
									return get.event()[button.link].length;
								...
								ai2(target) {
									if (!get.event().used && get.player() == target) return 114514;
						...
						if (result?.links && result?.targets) {
							...
							player.addMark("mbrunwei_used", gain.length, false);
							player.addTip("mbrunwei_used", `润微  ${gain.length}`);
							let gaintag = [];
							if (player == target) {
								gaintag = ["mbrunwei"];
								player
		...
		async cost(event, trigger, player) {
			const result = await player
				...
				.set("ai", button => {
					...
					if (eff > 0) return 0;
					if (trigger.player.hasSkillTag("maixie") && trigger.num === 1 && button.link == "tao") return 1 + Math.random();
					return Math.random();
				})
				.forResult();
			if (result.bool) {
	...
	mbxuehen: {
		...
		subSkill: {
			...
			sha: {
				mod: {
					cardname(card) {
						if (get.itemtype(card) == "card" && card.hasGaintag("mbxuehen_sha")) return "sha";
					},
					cardnature(card) {
						if (get.itemtype(card) == "card" && card.hasGaintag("mbxuehen_sha")) return false;
					},
					cardUsable(card, player, num) {
						if (card?.cards?.some(cardx => cardx.hasGaintag("mbxuehen_sha"))) return Infinity;
					},
				},
				charlotte: true,
				onremove(player, skill) {
		...
		async cost(event, trigger, player) {
			...
			const result = await player
				...
				.set("ai", () => {
					...
					if (num < get.info("pothongyi").maxMark()) return 0; // || !player.hasValueTarget(card)
					return 1;
				})
				.forResult();
		...
		async content(event, trigger, player) {
			...
			player
				...
				.then(() => {
					...
					if (target.hasHistory("damage", evtx => trigger === evtx?.getParent(2))) return;
	...
	mbzhuguo: {
		...
		async content(event, trigger, player) {
			...
			if (num > 0) await target.drawTo(target.maxHp);
			else if (num < 0 && target.countDiscardableCards(target, "h") > 0) await target.chooseToDiscard("h", -num, true);
			const isDraw = target.hasHistory("gain", evt => evt.getParent().name == "draw" && evt.getParent(2) == event);
			if (!isDraw && target.isDamaged()) await target.recover();
		...
		async content(event, trigger, player) {
			...
			const result = await source
				...
				.set("ai", card => {
					...
					if (["tao", "jiu"].includes(get.name(card, source))) return 0;
	...
	mbjianji: {
		...
		delay: false,
		check(card) {
			if (get.player().getHp() == 1) return 8 - get.value(card);
			if (get.name(card, get.player()) == "sha") return 7 - get.value(card);
			return 6 - get.value(card);
		},
		selectTarget: 2,
		multitarget: true,
	...
	mbtanfeng: {
		...
		async content(event, trigger, player) {
			...
			if (choices.includes("discard") && game.hasPlayer(c => c.countDiscardableCards(player, "he"))) {
				...
				if (result.targets[0].countCards("h") <= player.countCards("h")) player.skip("phaseDraw");
			}
			if (choices.includes("damage")) {
				...
				if (result.targets[0].getHp() <= player.getHp()) player.skip("phaseUse");
			}
		},
	},
	//孙韶
	mbganjue: {
		...
		mod: {
			cardUsable(card, player) {
				if (card?.storage?.mbganjue) return Infinity;
			},
			targetInRange(card, player, target) {
				if (card?.storage?.mbganjue) return true;
			},
		},
	},
	mbzhuji: {
	...
	mbxiezheng: {
		...
		async cost(event, trigger, player) {
			...
			event.result = await player
				...
				.set("ai", target => {
					const player = get.player();
					if (!player.hasValueTarget({ name: "binglinchengxiax", isCard: true, mbxiezheng: true })) return 0;
					let val = 0;
		...
		mod: {
			playerEnabled(card, player, target) {
				const mode = get.mode();
				if (mode !== "identity" || !card.mbxiezheng || player.storage.mbzhaoxiong || target.group === player.group) return;
				if (game.hasPlayer(current => current.group === player.group && player.canUse(card, current))) return false;
			},
		},
	},
	mbqiantun: {
		...
		async content(event, trigger, player) {
			...
			if (result3.winner == player) {
				...
				if (cards.length) {
					if (mode !== "doudizhu") await target.give(cards, player);
					else {
						const result2 =
							cards.length > 2
								? await player
						...
						if (result2?.bool && result2.links?.length) await target.give(result2.links, player);
					}
				}
			} else {
								...
								? await player
										.chooseButton(
											[
												...
												(() => {
													if (player.hasSkillTag("viewHandcard", null, target, true)) return cards;
			...
			player
				...
				.then(() => {
					...
					if (cards.length) {
						...
						const mode = get.mode();
						if (mode !== "doudizhu") trigger.player.give(cards, player);
						else player.gainPlayerCard(trigger.player, "h", true);
					}
				});
			if (player.canUse(card, target)) await player.useCard(card, target);
		},
	...
	friendbiwei: {
		...
		filterCard(card, player) {
			...
			if (!lib.filter.cardDiscardable(card, player) || typeof number !== "number") return false;
		...
		async content(event, trigger, player) {
			...
			else delete player.getStat("skill")[event.name];
		},
		ai: {
			order: 10,
			result: {
	...
	friendgongli_cuijun: {
		...
		filter(event, player) {
			if (lib.suit.every(suit => suit === "heart" || player.getStorage("friendgongli_cuijun_shunyi").includes(suit))) return false;
			if (!game.hasPlayer(target => lib.characterSort?.mobile?.mobile_laoyouji?.some(name => get.is.playerNames(target, name)))) return false;
			return event.name !== "phase" || game.phaseNumber === 0;
		},
		forced: true,
		async content(event, trigger, player) {
			...
			result: {
				player(player) {
					...
					for (let i of game.filterPlayer()) {
						if (!player.canUse(card, i)) continue;
	...
	friendgongli_shitao: {
		...
		filter(event, player) {
			if (lib.inpile.map(i => get.type2(i)).every(type => player.getStorage("friendgongli_shitao_qinying").includes(type))) return false;
			if (!game.hasPlayer(target => lib.characterSort?.mobile?.mobile_laoyouji?.some(name => get.is.playerNames(target, name)))) return false;
			return event.name !== "phase" || game.phaseNumber === 0;
		},
		forced: true,
	...
	mbzengou: {
		...
		async content(event, trigger, player) {
			...
			if (!goon && !allNames.length) return;
			let result;
			if (!goon) result = { index: 1 };
			else if (!allNames.length) result = { index: 0 };
			else
				result = await player
					.chooseControl()
					.set("choiceList", [
					...
					const [choice] =
						list.length > 1
							? await player
									...
									.forResult("links")
							: list;
					if (choice) await player.chooseUseTarget(new lib.element.VCard({ name: choice[2], nature: choice[3] }), true, false, "nodistance");
				}
			} else {
				...
				for (let i = 0; i < cards.length; i++) {
					while (gains[i].length < cards[i].length) {
						const card = get.cardPile2(card => {
							if (gains.flat().includes(card)) return false;
							return card.name === "sha";
						});
						if (card) gains[i].push(card);
						else break;
					}
				}
						...
						? await player
								...
								.set("ai", () => {
									...
									if (get.attitude(player, target) < 0) {
										...
										if (names.includes("sha") && controls.includes("sha")) return "sha";
										if (names.includes("tao") && controls.includes("tao")) return "tao";
									}
		...
		subSkill: {
			...
			debuff: {
				...
				trigger: { player: "useCardAfter" },
				filter(event, player) {
					if (player.getHistory("useCard").indexOf(event) !== 0) return false;
				...
				mod: {
					aiOrder(player, card, num) {
						if (player.getHistory("useCard").length > 0 || !player.storage["mbzengou_debuff"]?.[card.name]) return;
						const effect = get.effect(player, { name: "losehp" }, player, player);
						if (effect < 0) return num / 1145141919810;
	...
	mbfeili: {
		...
		filter(event, player) {
			if (!player.hasSkill("mbzengou", null, false, false)) return false;
		...
		async cost(event, trigger, player) {
			...
			const result = await player
				...
				.set("ai", () => {
					...
					const trigger = get.event().getTrigger();
					if (get.damageEffect(player, trigger.source, player, trigger.nature) * trigger.num >= 0) return "cancel2";
					return controls[0];
				})
	...
	potjiyu: {
		...
		subSkill: {
			...
			refresh: {
				...
				filter(event, player) {
					if (player.hasCard(card => card.hasGaintag("potjiyu_effect"), "h") || typeof player.getStat("skill")?.["potjiyu"] !== "number") return false;
					const evt = event.getl(player);
					if (!evt?.hs?.length) return false;
					if (event.name === "lose") {
	...
	potdaozhuan: {
		...
		chooseButton: {
			...
			filter(button, player) {
				...
				if (player.getStorage("potdaozhuan_used").includes(button.link[2])) return false;
		...
		hiddenCard(player, name) {
			if (player.isTempBanned("potdaozhuan") || player.getStat("skill")["potdaozhuan"]) return false;
	...
	potfuji: {
		...
		async content(event, trigger, player) {
			...
			await game
				...
				.setContent("gaincardMultiple");
			for (const list of gain_list) list[0].addSkill("potfuji_effect");
	...
	friendmanjuan: {
		...
		filter(event, player) {
			...
			if (cards.length < 2 || !cards.some(i => get.owner(i) === player && ["h", "e"].includes(get.position(i)))) return false;
			...
			const {
				...
				.set("processAI", list => {
					...
					let cards = listx[1].filter(card => {
						if (!get.discardPile(c => get.type2(c) !== get.type2(card))) return false;
	...
	friendyangming: {
		...
		subSkill: {
			...
			check: {
				...
				filter(event, player) {
					if (!player.isPhaseUsing() || player.countCards("h")) return false;
	...
	xuanjian_skill: {
		...
		selectCard() {
			const player = get.player();
			if (ui.selected.cards.length) {
				if (!player.hasSkill("friendxushugongli") || !get.info("friendgongli").isFriendOf(player, "friend_zhugeliang")) return -1;
			}
			return 1;
		},
		ai: {
	...
	friendyance: {
		...
		async cost(event, trigger, player) {
			const { result } = await player
				...
				.set("filterButton", button => {
					if (button.link !== "minigame") return true;
					const player = get.player();
			...
			record: {
				...
				intro: {
					...
					content([trrList, gussList, type, num, show], player) {
						if (!show && !player.isUnderControl(true)) return "天机可知却不可说...";
						return [
	...
	shoufa: {
		...
		filter(event, player, name) {
			...
			return game.hasPlayer(target => {
				...
				if (name == "damageEnd" && get.distance(target, player) < num) return false;
	...
	refuli: {
		...
		async content(event, trigger, player) {
			...
			if (player.isMaxHp(true)) await player.turnOver();
		},
		ai: {
			save: true,
