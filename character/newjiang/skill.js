...
const skills = {
			...
			use: {
				mod: {
					cardUsable: function (card, player, num) {
						...
						if (card.cards.some((i) => i.hasGaintag("newj_yuxi")))
							return Infinity;
					},
					aiOrder: function (player, card, num) {
						if (
							get.itemtype(card) == "card" &&
							card.hasGaintag("newj_yuxi")
						)
							return num - 1;
					},
				},
	...
	newj_porong: {
		...
		subSkill: {
			lazy: {
				...
				content: function () {
					if (
						get.tag(
							lib.skill.newj_porong.getLastUsed(
								player,
								null,
								null
							).card,
						...
						player.addTip(
							event.name,
							get.translation("newj_porong") + "可用"
						);
					else {
						player.addTip(
							event.name,
							get.translation("newj_porong") + "不可用"
						);
					}
				},
			},
		},
		getLastUsed: function (player, event, num) {
			let history = player.getAllHistory("useCard");
			let index;
			if (event) index = history.indexOf(event) - 1;
			else if (num) index = history.length - num;
		...
		frequent: true,
		filter: function (event, player) {
			let evt = lib.skill.newj_porong.getLastUsed(player, event);
			...
			return (
				...
				player.getStorage("newj_porong") !=
					lib.skill.newj_porong.getLastUsed(player, null, 2).card &&
				event.targets.length
			);
		},
		async content(event, trigger, player) {
			trigger.directHit.addArray(game.filterPlayer());
			for (let target of trigger.targets.toUniqued()) {
				for (let i of [target.getNext(), target.getPrevious()])
					if (player !== i)
						await player
							.gainPlayerCard(i, true, "h")
							.set("ai", (button) =>
	...
	chuanxie: {
		...
		forced: true,
		async content(event, trigger, player) {
			const [suit, current, gain] = Array.from({ length: 3 }).map(
				(_, num) => player.getStorage("chuanxie")[num]
			);
		...
		intro: {
			...
			content: (suits) =>
				"当前顺序：" +
				suits.map((suit) => get.translation(suit)).join("|"),
		},
	},
	yjqiaosi: {
		audio: 2,
		...
		async cost(event, trigger, player) {
			const result = await player
				.chooseButton([
					get.prompt2("yjqiaosi"),
					[
						[3, 1].map((i) => [
							"",
							"",
							"lukai_" + player.getStorage("chuanxie")[i],
						]),
						"vcard",
					],
				])
		...
		async content(event, trigger, player) {
			...
			game.log(
				...
				"#g【传械】",
				"的首个花色改为了",
				"#y" + get.translation(event.cost_data)
			);
			while (
				player.getStorage("chuanxie").indexOf(event.cost_data) !== 0
			) {
	...
	yuliao: {
		...
		filter(event, player) {
			if (
				player.hasSkill("yuliao_used") ||
				player.getExpansions("yuliao").length >= 8
			)
				return false;
		...
		async cost(event, trigger, player) {
			const result = await player
				.chooseButton([
					"###" +
						get.prompt("yuliao") +
						'###<div class="text center">将其中一张牌置于武将牌上',
					trigger.getd(),
				])
		...
		async content(event, trigger, player) {
			player.addTempSkill("yuliao_used", [
				"phaseBefore",
				"phaseChange",
				"phaseAfter",
			]);
	...
	qiqiao: {
		...
		filter(event, player) {
			if (
				player
					.getExpansions("yuliao")
					.filter((card) =>
						["basic", "trick", "equip"].includes(get.type2(card))
					).length < 2
			)
				return false;
			return game.hasPlayer((target) =>
				lib.skill.qiqiao.filterTarget(null, player, target)
			);
		},
		...
		chooseButton: {
			...
			filter(button, player) {
				...
				if (
					bool !==
					player.getExpansions("yuliao").includes(button.link)
				)
					return false;
				return (
					!bool ||
					["basic", "trick", "equip"].includes(get.type2(button.link))
				);
			},
			check(button) {
				...
				return (
					...
					get.attitude(player, target) *
						get.value(button.link.cards || [], target)
				);
			},
			select: 3,
			backup(links, player) {
				return {
					...
					async content(event, trigger, player) {
						...
						const listx = target.storage["qiqiao_effect"].find(
							(list) => list[0] === card
						);
						if (listx) {
							for (let i = 0; i < typeList.length; i++) {
								if (types.includes(typeList[i]))
									listx[1][i] += lose.filter(
										(j) => get.type2(j) === typeList[i]
									).length;
							}
						} else {
							let list = [0, 0, 0];
			...
			prompt(links) {
				...
				return (
					...
					[
						...
						[
							"equip",
							get.translation(card) + "离开装备区后摸两张牌",
						],
					]
						.filter((list) => {
							return lose
			...
			effect: {
				...
				filter(event, player) {
					...
					if (event.name === "useCard")
						return (
							...
							storage.some((list) => list[1][1] > 0)
						);
					const evt = event.getl(player);
					return evt?.es?.some((card) =>
						storage.some(
							(list) =>
								evt.vcard_map.get(card)?.cardid ===
								list[0].cardid
						)
					);
				},
				...
				async content(event, trigger, player) {
					if (trigger.name === "useCard") {
						...
						const lists = storage.filter((list) =>
							evt.es.some(
								(card) =>
									evt.vcard_map.get(card)?.cardid ===
									list[0].cardid
				...
				mod: {
					cardUsable(card, player, num) {
						if (card.name === "sha")
							return (num += player.storage[
								"qiqiao_effect"
							].reduce((sum, list) => sum + list[1][0], 0));
					},
				},
				ai: {
					directHit_ai: true,
		...
		chooseButton: {
			...
			filter(button) {
				return (
					...
					get.type2(button.link) ===
						get.type2(ui.selected.buttons[0].link)
				);
			},
			check(button) {
	...
	xiongjin: {
		...
		intro: {
			content(storage) {
				if (!storage)
					return "出牌阶段开始时，你可以摸X张牌，然后本回合的弃牌阶段开始时，你弃置所有非基本牌（X为你已损失的体力值，至少为1，至多为3）";
		...
		filter(event, player) {
			return (
				(event.player !== player) === Boolean(player.storage.xiongjin)
			);
		},
		logTarget: "player",
		prompt2(event, player) {
			...
			return (
				(goon ? "" : "令其") +
				"摸" +
				get.cnNumber(Math.min(3, Math.max(1, player.getDamagedHp()))) +
				"张牌，本回合的弃牌阶段开始时，" +
		...
		content() {
			...
			target.markAuto("xiongjin_effect", [
				target === player ? "nobasic" : "basic",
			]);
	...
	xiawei: {
		...
		filter(event, player) {
			...
			return event.cards.some(
				(card) =>
					!player.getStorage("xiawei").includes(get.suit(card, false))
			);
		},
		forced: true,
		async content(event, trigger, player) {
			...
			player.storage.xiawei.sort(
				(a, b) => lib.suit.indexOf(b) - lib.suit.indexOf(a)
			);
			player.addTip(
				"xiawei",
				get.translation("xiawei") +
					player
						.getStorage("xiawei")
	...
	baoxi: {
		...
		filter(event, player) {
			...
			if (
				...
				return false;
			return (
				event.cards.filter((card) => get.type(card) === "basic")
					.length > 1 &&
				player.hasUseTarget(new lib.element.VCard({ name: "juedou" }))
			);
		},
		async cost(event, trigger, player) {
			event.result = await player
				.chooseTarget(
					get.prompt("baoxi"),
					"减1点体力上限，视为对一名角色使用【决斗】",
					(card, player, target) => {
						return player.canUse(
							new lib.element.VCard({ name: "juedou" }),
							target
						);
					}
				)
		...
		subSkill: {
			...
			sha: {
				audio: "baoxi",
				trigger: {
					global: ["loseEnd", "cardsDiscardEnd", "loseAsyncEnd"],
				},
				filter(event, player) {
		...
		async content(event, trigger, player) {
			...
			if (result.bool) {
				...
				if (get.name(carda) == get.name(cardb)) {
					if (get.owner(cardb))
						await get.owner(cardb).give(cardb, player);
					else await player.gain(cardb, "gain2");
				}
				if (
					get.number(carda) == get.number(cardb) &&
					!player.hasSkill("mpmiaoxi_fresh")
				) {
	...
	xianmou: {
		...
		zhuanhuanji(player, skill) {
			player.storage[skill] = !player.storage[skill];
			player.changeSkin(
				{ characterName: "yj_sb_guojia" },
				"yj_sb_guojia" + (player.storage[skill] ? "_shadow" : "")
			);
		},
		intro: {
			...
			if (player.storage.xianmou) {
				...
				const result = await player
					.discardPlayerCard(
						target,
						"h",
						`是否弃置${get.translation(target)}至多${num}张牌?`,
						[1, num],
						"visible"
					)
					.set("ai", function (button) {
		...
		subSkill: {
			change: {
				...
				prompt2(event, player) {
					return (
						"切换【先谋】为状态" +
						(player.storage.dcsbyingmou ? "阳" : "阴")
					);
				},
		...
		async content(event, trigger, player) {
			...
			if (list.length > 0) {
				const skill = list.randomGet(),
					voiceMap = get.Audio.skill({
						skill,
						player: map[type][skill],
					}).audioList;
				if (type == "info") {
				...
				await player.addAdditionalSkills(
					`zhinang_${get.type2(trigger.card)}`,
					skill
				);
			}
		},
	...
	gouzhu: {
		...
		filter(_1, player, _3, skill) {
			...
			return (
				list.length &&
				list.some((item) => item in lib.skill.gouzhu.effectMap)
			);
		},
		...
		async content(event, trigger, player) {
			...
			const result = await player
				...
				.set("ai", (card) => {
					...
					return (
						5 +
						2 * get.sgn(playerEffect - targetEffect) -
						get.value(card)
					);
				})
				.forResult();
			await targets[0].swapHandcards(
				targets[1],
				result[0].cards,
				result[1].cards
			);
			if (result[0].cards.length == result[1].cards.length) {
				...
				const aim =
					targets[
						result[0].cards.length > result[1].cards.length ? 0 : 1
					];
				const sha = new lib.element.VCard({ name: "sha" });
		...
		async content(event, trigger, player) {
			...
			const result = await player
				...
				.set("ai", () => {
					...
					suits.sort(
						(a, b) =>
							player.countCards("h", { suit: a }) -
							player.countCards("h", { suit: b })
					);
					return suits[0];
				})
				.forResult();
	...
	yjqimei: {
		...
		filter(event, player) {
			...
			if (count && count > 0 && !player.hasSkill("yjqimei_rewrite"))
				return false;
			return true;
		},
		...
		async content(event, trigger, player) {
			...
			await target.draw(2);
			const targets = [player, target].filter((current) =>
				current.countDiscardableCards(current, "he")
			);
			if (targets.length) {
				const result = await player
					.chooseCardOL(
				...
				const suits = cards.reduce(
					(list, card) => list.add(get.suit(card)),
					[]
				);
				switch (suits.length) {
					case 1:
				...
				.set("ai", (target) => {
					...
					return (
						...
						Array.from({ length: 5 })
							.map((_, i) => i + 1)
							.reduce(
								(sum, i) => sum + target.countEmptySlot(i),
								0
							)
					);
				})
			...
			buff: {
				...
				filter(event, player) {
					...
					return (
						...
						evt.es &&
						evt.es.some((i) =>
							player.getStorage("yjzhuiji_buff").includes(i)
						)
					);
				},
				forced: true,
	...
	fazhu: {
		...
		filter(event, player) {
			return player.hasCard(
				(card) => !get.tag(card, "damage") && player.canRecast(card),
				"hej"
			);
		},
		async cost(event, trigger, player) {
				...
				const result = await player
					.chooseCardTarget({
						...
						filterCard(card, player) {
							...
							return !get
								.event("list")
								.some((list) => list[1] == card);
						},
						filterTarget(card, player, target) {
							return !get
								.event("list")
								.some((list) => list[0] == target);
						},
						ai1(card) {
				...
				for (const target of targets) {
					await target
						.chooseToUse(function (card, player, event) {
							...
							return lib.filter.cardEnabled.apply(
								this,
								arguments
							);
						})
		...
		filter: function (event, player) {
			return (
				player.countCards("he", (card) =>
					lib.skill.tongwei.filterCard(card, player)
				) > 1 && game.hasPlayer((i) => i != player)
			);
		},
		filterTarget: lib.filter.notMe,
				...
				.then(() => {
					...
					if (!names.length) event.finish();
						...
						playerx
							...
							.set("ai", (button) => {
								return (
									button.link[0][2] == _status.event.choice
								);
							})
							.set(
								"choice",
	...
	cuguo: {
		...
		content: function () {
			...
			player
				.chooseToDiscard(
					"蹙国：请弃置一张牌",
					`视为你对${get.translation(
						trigger.target
					)}使用一张${get.translation(
						card.nature || ""
	...
	qingbei: {
		...
		content: function () {
			...
			var next = player.chooseButton(
				[
					'###擎北：是否选择任意种花色？###<div class="text center">你不能于本轮使用这些花色，且使用牌后摸等同于选择花色数的牌</div>',
			...
			if (result.bool) {
				...
				player.addTip(
					"qingbei_effect",
					get.translation("qingbei_effect") +
						player
							.getStorage("qingbei_effect")
				...
				intro: {
					content: (storage) =>
						`本轮内不能使用${get.translation(
							storage
						)}花色的牌，且使用一张有花色的牌后摸${get.cnNumber(
							storage.length
						)}张牌`,
				},
				mod: {
	...
	zhenfeng: {
		...
		check: function (event, player) {
			...
			if (
				...
				event.player.hasCard((card) =>
					event.player.hasValueTarget(card)
				)
			)
				return false;
			if (
				...
				(!event.player.isAllCardsKnown(player) ||
					2 * event.player.getHp() +
						0.7 * event.player.countCards("hs") <=
						6 - player.countMark("zhenfeng"))
			)
				return false;
			return true;
		},
		...
		content: function () {
			...
			var choices = Array.from({
				length: trigger.player.countCards("h") + 1,
			}).map((_, i) => get.cnNumber(i, true));
			var type = get.type2(trigger.card, trigger.player);
			player
				.chooseControl(choices)
				.set(
					"prompt",
					"镇锋：猜测其手牌中的" + get.translation(type) + "牌数"
				)
				.set("ai", () => {
				...
				.set(
					"choice",
					(function () {
						var num = trigger.player.countCards(
							"h",
						...
						if (
							trigger.player.isAllCardsKnown(_status.event.player)
						) {
							return knownNum;
						}
						...
						if (type == "basic")
							numx =
								num + Math.floor(Math.random() * restNum + 1);
						else if (type == "trick") {
							if (num > 2) numx = 2;
							else numx = 1;
							if (Math.random() < 0.5) {
			...
			("step 2");
			var type = get.type2(trigger.card, trigger.player);
			var count = trigger.player.countCards(
				"h",
				(card) => get.type2(card) == type
			);
			...
			if (count == guessedNum) {
				...
				if (
					Math.abs(count - guessedNum) > 1 &&
					trigger.player.canUse("sha", player, false)
				) {
	...
	dcduliang: {
		...
		content: function () {
			...
			if (result.bool) {
				...
				game.log(
					target,
					"获得了" + get.cnNumber(cardsx.length) + "张牌"
				);
	...
	shiming: {
		...
		content: function () {
			...
			player
				...
				.set(
					"damage",
					get.damageEffect(trigger.player, trigger.player, player) >
						0 && trigger.player.hp <= 3
						? 1
						: -1
				);
			while (cards.length)
			...
			trigger.player
				.chooseBool(
					"是否跳过摸牌阶段并对自己造成1点伤害，然后从牌堆底摸三张牌？"
				)
				.set("ai", () => _status.event.bool)
				.set(
					"bool",
					get.damageEffect(trigger.player, trigger.player) >= -6 ||
						trigger.player.hp > 3
				);
			("step 3");
			if (result.bool) {
			...
			globalFrom: function (from, to, distance) {
				return (
					...
					Math.min(
						1,
						from
							.getExpansions("duwang")
							.filter((i) => i.name != "sha").length
					)
				);
			},
	...
	cibei: {
		...
		filter: function (event, player) {
			if (
				!player.getExpansions("duwang").filter((i) => i.name != "sha")
					.length
			)
				return false;
		...
		content: function () {
			...
			if (result.bool) {
				...
				player
					.addToExpansion(trigger.cards.filterInD("d"), "gain2")
					.gaintag.add("duwang");
				if (
					game.hasPlayer(
						(current) =>
							current.countDiscardableCards(player, "hej") > 0
					)
				)
					player
						.chooseTarget(
		...
		subSkill: {
			fullyReady: {
				...
				filter: function (event, player) {
					...
					return (
						storage.length > 0 &&
						storage.every((i) => i.name == "sha")
					);
				},
				content: function () {
					player
						.gain(player.getExpansions("duwang"), "gain2")
						.gaintag.add("cibei_mark");
	...
	liandui: {
		...
		async cost(event, trigger, player) {
			...
			const { result } = await trigger.player
				.chooseBool(
					"是否对" + get.translation(previous) + "发动【联对】？",
					"令" + get.translation(previous) + "摸两张牌"
				)
				.set("ai", () => _status.event.bool)
				.set(
					"bool",
				...
				check: function (card) {
					...
					if (
						game.hasPlayer(function (current) {
							return (
								lib.skill.biejun_give.filterTarget(
									null,
									player,
									current
	...
	sangu: {
		...
		getEffect: function (player, target, event, list1, list2) {
			...
			let getv = function (name, player, arg) {
				...
				v = player.getUseValue(
					{ name: name, storage: { sangu: true } },
					arg
				);
		...
		content: function () {
			...
			if (event.list1.length) {
				dialog.push(
					'<div class="text center">本回合已使用过的牌</div>'
				);
				dialog.push([
					event.list1.map((i) => [get.type(i), "", i]),
					"vcard",
				]);
			}
			if (event.list2.length) {
				dialog.push(
					'<div class="text center">本回合未使用过的牌</div>'
				);
				dialog.push([
					event.list2.map((i) => [get.type(i), "", i]),
					"vcard",
				]);
			}
			player
			...
			if (result.bool) {
				...
				target.storage.sangu_effect =
					target.storage.sangu_effect.concat(names);
				game.log(
					player,
					"为",
					target,
					"选择了",
					"#y" + get.translation(names)
				);
				target.addTempSkill("sangu_effect", {
				...
				if (bool) {
					target.addTempSkill("sangu_prevent", {
						player: "phaseUseAfter",
					});
			...
			viewas: {
				...
				filter: function (event, player) {
					return (
						...
						player.getStorage("sangu_effect").length > 0
					);
				},
				content: function () {
					if (!trigger.card.storage) trigger.card.storage = {};
					trigger.card.storage.sangu = true;
					player.unmarkAuto("sangu_effect", [
						player.getStorage("sangu_effect")[0],
					]);
				},
			},
			prevent: {
	...
	kousheng: {
		...
		content: function () {
			...
			player
				...
				.set(
					"standard",
					player.getUseValue({ name: "sha" }, null, true)
				)
					...
					if (target.countCards("h") > 0)
						target
							...
							.set("ai", function () {
								...
								if (
									get.value(hs1, player) >=
									get.value(hs2, target)
								)
									return false;
								return true;
							});
	...
	lkbushi: {
		...
		content() {
			...
			var next =
				player.chooseToMove("卜筮：是否调整〖卜筮〗的花色顺序？");
			next.set("list", [
				[
			...
			if (result.bool) {
				...
				for (var i = 0; i < 4; i++) {
					if (list[i] != list2[i]) {
						...
						player.addTip(
							"lkbushi",
							get.translation("lkbushi") +
								lib.skill.lkbushi
									.getBushi(player)
									.reduce(
										(str, i) => str + get.translation(i),
										""
									)
						);
						var str = "#g";
		...
		content: function () {
			...
			next.set("ai", function (target) {
				...
				if (
					i == es.length &&
					(_status.event.nojudge ||
						!ui.selected.targets[0].countCards(
							"j",
							function (card) {
			...
			return (
				player
					.getHistory("useCard", function (ev) {
						return (
							ev.getParent("phaseUse") == evt &&
							get.tag(ev.card, "damage") &&
							["basic", "trick"].includes(get.type(ev.card))
						);
					})
					.indexOf(event) == 0 &&
				game.hasPlayer(function (current) {
		...
		content: function () {
			...
			if (result.bool) {
				...
				game.log(
					target,
					"获得了一个",
					"#g【惧(" + get.translation(trigger.card.name) + ")】",
					"标记"
				);
			...
			onunmark: function (storage, player) {
				if (player.storage.weifeng2) {
					game.log(
						...
						"移去了一个",
						"#g【惧(" +
							get.translation(player.storage.weifeng2) +
							")】",
						"标记"
					);
	...
	weifeng3: {
		...
		filter: function (event, player) {
			...
			return (
				event.player != player &&
				typeof event.player.storage.weifeng2 == "string"
			);
		},
		content: function () {
			if (trigger.name == "damage") {
				...
				if (
					trigger.card &&
					trigger.card.name == trigger.player.storage.weifeng2
				)
					trigger.num++;
	...
	gnjinfan: {
		...
		filter: function (event, player) {
			...
			return (
				player.countCards("h", function (card) {
					return (
						_status.connectMode || !list.includes(get.suit(card))
					);
				}) > 0
			);
		},
			...
			player
				...
				.set("ai", function (card) {
					...
					if (
						...
						return 0;
					if (
						["sha", "shan", "wuxie", "caochuan"].includes(card.name)
					)
		...
		content: function () {
			...
			for (var i of trigger.ss) {
				...
				var card = get.cardPile2(function (card) {
					return (
						!cards.includes(card) && get.suit(card, false) == suit
					);
				});
		...
		content: function () {
			...
			if (game.countPlayer() == 2)
				...
				player
					...
					.set("ai", (target) => {
						return target ==
							get.event("aiTargets")[ui.selected.targets.length]
							? 10
							: 0;
					})
					.set(
						"aiTargets",
						lib.skill.mbhuiyao.getUnrealDamageTargets(
							player,
							[
		...
		getUnrealDamageTargets: (player, lists, forced) => {
				...
				.map((current) => {
					...
					current.getSkills(null, false, false).forEach((skill) => {
						...
						if (
							...
							(info.ai.maixie ||
								info.ai.maixie_hp ||
								info.ai.maixie_defend)
						)
							val = Math[val > 0 ? "max" : "min"](
								val > 0 ? 0.1 : -0.1,
								val + 2 * att
							);
					});
			...
			const list2 = sourceList
				...
				.map((current) => {
					...
					const att = -get.sgnAttitude(player, current);
					const eff =
						get.damageEffect(targetx, current, current) * att;
					targetx.hp = _hp;
		...
		ai: {
			...
			result: {
				player: function (player) {
					...
					var limit = 25;
					var quesong =
						player.hasSkill("mbquesong") &&
						!player.getStat().damaged;
					if (quesong) {
						limit -= 7.5;
					}
					if (
					...
					if (
						...
						game.hasPlayer((target) => {
							...
							var val =
								get.damageEffect(target, player, target) * att;
							target
	...
	mbquesong: {
		...
		content: function () {
			"step 0";
			player
				.chooseTarget(get.prompt2("mbquesong"))
				.set("ai", (target) => {
					var player = _status.event.player;
	...
	mpjianlin: {
		...
		isUseOrRespond(event, player) {
			...
			return (
				["useCard", "respond"].includes(evt2.name) &&
				evt2.player == player
			);
		},
		filter(event, player) {
		...
		async cost(event, trigger, player) {
			...
			const {
				...
				.chooseButton(["俭吝：你可以获得其中一张牌", cards])
	...
	mpsixiao: {
		...
		subSkill: {
			use: {
				...
				intro: {
					content:
						"当你需要使用或打出牌时，你可以观看$的手牌并可以使用或打出其中一张牌，然后$摸一张牌",
				},
				hiddenCard(player, name) {
					if (
						!lib.inpile.includes(name) ||
						player.hasSkill("mpsixiao_used", null, null, false)
					)
						return false;
				...
				chooseButton: {
					...
					ai: {
						...
						skillTagFilter(player, tag) {
							...
							return lib.skill.mpsixiao_use.hiddenCard(
								player,
								name
							);
						},
	...
	mpjiusong: {
		...
		filter: function (event, player) {
			if (event.name == "chooseToUse")
				return player.hasCard(
					(card) => get.type2(card) == "trick",
					"hs"
				);
			return (
				event.card.name == "jiu" && player.countMark("mpjiusong") < 3
			);
		},
		forced: true,
		locked: false,
	...
	mpmaotao: {
		...
		prompt2: function (event, player) {
			...
			if (get.type(event.card) != "delay")
				...
				list = game.filterPlayer((current) =>
					current.canAddJudge(event.card)
				);
			const gainText = `${
				list.length > 1 && !player.storage.mpmaotao_gained
					? `若新目标与原目标相同，你`
					: ""
			}${
				!player.storage.mpmaotao_gained
					? "获得牌堆中的一张锦囊牌。"
					: ""
			}`;
			return `移去1枚“醉”${
				list.length > 1
					? `，令${get.translation(
							event.card
					  )}目标改为${get.translation(list)}中的一名随机角色`
					: ""
			}。${gainText}`;
		},
		check: function (event, player) {
			...
			let list2 = list.filter(
				(current) =>
					get.effect(current, event.card, player, player) > eff
			);
			let list3 = list.filter(
				(current) => get.effect(current, event.card, player, player) > 0
			);
			return (
				list2.length >= list.length / 2 ||
				(player.countMark("mpjiusong") >= 2 &&
					list3.length >= list.length / 2)
			);
		},
		content: function () {
			...
			if (get.type(trigger.card) != "delay")
				...
				list = game.filterPlayer((current) =>
					current.canAddJudge(trigger.card)
				);
