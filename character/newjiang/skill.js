...
const skills = {
	...
	dcxiujue: {
		...
		filter(event, player) {
			return game.hasPlayer((current) =>
				get.info("dcxiujue").filterTarget(null, player, current)
			);
		},
		...
		async content(event, trigger, player) {
			...
			if (winner) {
				const cards = [result.player, result.target]
					.filterInD("d")
					.filter((card) => winner.hasUseTarget(card));
				while (cards.length) {
			...
			result: {
				target(player, target) {
					...
					if (
						...
						get.number(hs[0]) - ts.length >=
							9 + Math.min(2, player.hp / 2)
					)
	...
	fjzhitu: {
		...
		async cost(event, trigger, player) {
			...
			const prompt2 =
				"为" + get.translation(card) + "增加任意个你与其距离相等的目标";
	...
	dcfujue: {
		...
		async content(event, trigger, player) {
			...
			const lose = player.hasHistory(
				"lose",
				(evt) => evt.getParent(3) == event
			);
			const bool1 =
				lose &&
			...
			return (
				event.cards.some(
					(card) =>
						!player
							.getStorage("zhenbian")
							.includes(get.suit(card, false))
				) ||
				(player.getStorage("zhenbian").length >= 4 && player.maxHp < 9)
			);
		},
		forced: true,
		async content(event, trigger, player) {
				...
				player.addTip(
					...
					get.translation("zhenbian") +
						player
							.getStorage("zhenbian")
							.reduce(
								(str, suit) => str + get.translation(suit),
								""
							)
				);
			}
		...
		async content(event, trigger, player) {
			...
			if (result.bool) {
				...
				if (
					...
					!player.hasSkill("mpmiaoxi_rewrite", null, null, false)
				) {
	...
	jingyi: {
		...
		async content(event, trigger, player) {
			...
			if (num > 0)
				await player.draw(num).set("jingyi_subtype", [subtypes]);
	...
	yjqimei: {
		...
		async content(event, trigger, player) {
			...
			const targets = [player, target].filter(
				(current) => current.countCards("h") > 1
			);
			if (targets.length) {
				const result = await player
					.chooseCardOL(targets, "h", true, 2, "齐眉：请展示两张手牌")
					.forResult();
				const videoId = lib.status.videoId++;
				game.broadcastAll(
					(targets, result, id, player) => {
						const dialog = ui.create.dialog(
							get.translation(player) + "发动了【齐眉】"
						);
						dialog.videoId = id;
						...
						for (let i = 0; i < result.length; i++) {
							const current = targets[i],
								cards = result[i].cards.filter((card) => {
									return (
										get.owner(card) === current &&
										current.canRecast(card)
									);
								});
