...
const skills = {
	...
	dcyuxian: {
		...
		intro: {
			content: (_, player) =>
				"已记录花色:" + get.translation(player.getStorage("dcyuxian")),
		...
		filter: (event, player) =>
			player.isPhaseUsing() &&
			event.player === player &&
			player.getStorage("dcyuxian").length < 4 &&
			get.suit(event.card, player) !== "none",
		forced: true,
		locked: false,
		content() {
			player.storage[event.name] = player
				.getStorage(event.name)
				.concat([get.suit(trigger.card, player)]);
			...
			draw: {
				...
				filter: (event, player) => {
					...
					const num = event.player
						...
						.indexOf(event);
					return (
						player.getStorage("dcyuxian")[num] ===
						get.suit(event.card, event.player)
					);
				},
				check: (event, player) =>
					get.attitude(player, event.player) > 0,
				prompt: (event, player) =>
					"是否与" + get.translation(event.player) + "各摸一张牌",
	...
	dcminshan: {
		...
		async cost(event, trigger, player) {
			event.result = player
				.chooseTarget(
					"你可令一名角色获得两张" +
						get.translation(get.suit(trigger.card, player)) +
						"牌"
				)
				.set("ai", (target) => {
					...
					if (
						att > 0 &&
						target.countCards("h", { name: "tao" }) +
							target.getHp() <
							3 &&
						["heart", "diamond"].includes(get.event("suit"))
					)
						return att * 2;
					return att;
				})
	...
	dcbaguan: {
		...
		mod: {
			aiOrder(player, card, num) {
				if (
					lib.skill.dcbaguan.getUsed(player, true) &&
					typeof card === "object"
				) {
					if (
		...
		async content(event, trigger, player) {
			...
			next.set("filterTarget", (...args) =>
				lib.filter.targetEnabled.apply(this, args)
			);
	...
	dcxiaowu: {
		...
		content() {
			...
			let card = get.cardPile2((card) => {
				return (
					card.name == "sha" ||
					lib.skill.shencai.getStr(card).includes("【杀】")
				);
			});
	...
	dckegou: {
		...
		async content(event, trigger, player) {
			...
			await player.gain(
				discardPile
					.filter((c) => get.number(c) === maxNumber)
					.randomGets(1),
				"gain2"
			);
		},
	...
	dcjiduan: {
		...
		filter: (event, player) =>
			event.isFirstTarget &&
			event.targets
				.filter((c) => !player.getStorage("dcjiduan_used").includes(c))
				.some((z) => z.countCards("h")),
		async cost(event, trigger, player) {
			let targets = trigger.targets.filter(
				(c) => !player.getStorage(event.name + "_used").includes(c)
			);
			event.result = await player
				...
				.set(
					"filterTarget",
					(card, player, target) =>
						targets.includes(target) && target.countCards("h")
				)
				.set("ai", (target) => {
					...
					if (get.attitude(player, target) < 0)
						return (
							count *
							get.effect(target, { name: "guohe_copy2" }, player)
						);
				})
				.forResult();
		},
		async content(event, trigger, player) {
			...
			const show = await target
				...
				.set(
					"prompt2",
					`若此牌点数小于${numberx}则你被${get.translation(
						player
					)}执行摸牌或弃牌效果`
				)
				.set("ai", (card) => {
			...
			player
				.when({ global: ["phaseBeginStart", "phaseEnd"] })
				.then(() => (player.storage.dcjiduan_used = []));
			...
			let suitx = [
				...new Set(
					target.getCards("h").map((item) => get.suit(item, target))
				),
			];
			if (result.index == 0) {
				...
				for (const suit of suits) {
					const card = get.cardPile(
						(card) =>
							get.suit(card) === suit && !gains.includes(card)
					);
	...
	dcdixian: {
		...
		async content(event, trigger, player) {
			...
			const cardPile = Array.from(ui.cardPile.childNodes).sort(
				lib.sort.number2
			);
			const next = player.gain(
				cardPile.splice(0, event.cards.length),
				"draw"
			);
			...
			buff: {
				...
				mod: {
					globalFrom: function (from, to) {
						if (from.getStorage("twodcspzhuiji").includes(to))
							return -Infinity;
					},
				},
			},
	...
	twodcspshichou: {
		...
		filterTarget(card, player, target) {
			const targetsx = player.getStorage("twodcspshichou_used", [
				[],
				[],
			])[1];
		...
		async content(event, trigger, player) {
			for (const i of get.players()) {
				const {
					...
					.set(
						"prompt",
						`将任意张牌置于${get.translation(player)}的武将牌上`
					);
				if (bool) {
	...
	dcwoheng: {
		...
		async cost(event, trigger, player) {
			...
			event.result = await player
				.chooseTarget(
					`斡衡：令一名角色摸或弃置${get.cnNumber(num + 1)}张牌`,
					lib.filter.notMe
				)
				.forResult();
		},
		...
		async content(event, trigger, player) {
			...
			const num = player.hasSkill("dcwoheng")
				? player.countMark("dcwoheng_used")
				: 1;
			if (player.hasSkill("dcwoheng")) {
			...
			let directcontrol =
				...
				(await player
					...
					.set(
						"choice",
						get.attitude(player, target) > 0 ? str1 : str2
					)
					.forResultControl());
			if (directcontrol) {
			...
			if (
				player.countMark("dcwoheng_used") >= 5 ||
	...
	dcyuhui: {
		...
		async cost(event, trigger, player) {
			event.result = await player
				.chooseTarget(`御麾：选择任意名其他吴势力角色`, [1, Infinity])
		...
		subSkill: {
			buff: {
				...
				async cost(event, trigger, player, target) {
					const { dcyuhui_buff: storage } = player.storage;
					const { result } = await player.chooseToGive(
						storage[0],
						(card) => {
							return (
								get.color(card) == "red" &&
		...
		async content(event, trigger, player) {
			...
			if (bool) {
				...
				if (player.getExpansions("dczhengyue")[0]) {
					player.addTip(
						"dczhengyue",
						get.translation(player.getExpansions("dczhengyue")[0])
					);
				} else {
		...
		subSkill: {
			useCard: {
				...
				filter(event, player) {
					...
					if (
						get.suit(firstCard) == get.suit(event.card) ||
						get.number(firstCard) == get.number(event.card) ||
						get.name(firstCard) == get.name(event.card)
					)
						return true;
				...
				async content(event, trigger, player) {
					...
					if (
						...
						next.set("processAI", (list) => {
							const cards = list[1][1].randomGets(
								5 - list[0][1].length,
								list[1][1].length
							);
							return [
								list[0][1].addArray(cards),
								list[1][1].removeArray(cards),
							];
						});
						next.set("filterOk", (moved) => {
							...
							return (
								...
								Math.min(
									5,
									list[0][1].length + list[1][1].length
								)
							);
						});
						const {
						...
						player.addMark(
							"dczhengyue_count",
							puts.length - moved[1].length
						);
						if (player.storage.dczhengyue_count >= 2) {
							player.storage.dczhengyue_count =
								player.storage.dczhengyue_count % 2;
					...
					if (player.getExpansions("dczhengyue")[0]) {
						player.addTip(
							"dczhengyue",
							get.translation(
								player.getExpansions("dczhengyue")[0]
							)
						);
					} else {
						player.removeTip("dczhengyue");
				...
				.step(async (event, trigger, player) => {
					const targets = game.filterPlayer((current) => {
						...
						return trigger.targets.some((target) =>
							[target.getPrevious(), target.getNext()].includes(
								current
							)
						);
					});
			...
			result: {
				target(player, target) {
					...
					if (
						...
						return 1;
					return (
						Math.max(0, eff) * Math.sqrt(target.countCards("h") + 1)
					);
				},
			},
		},
			...
			effect: {
				...
				mod: {
					cardUsable(card) {
						if (
							get.number(card) === "unsure" ||
							card.cards?.every((card) =>
								card.hasGaintag("dcyuxi")
							)
						)
							return Infinity;
					},
	...
	dcporong: {
		...
		check(event, player) {
			...
			const targets = event.targets
				.map((target) =>
					[target, target.getNext(), target.getPrevious()].filter(
						(current) =>
							current != player &&
		...
		subSkill: {
			mark: {
				...
				async content(event, trigger, player) {
					if (
						!trigger.dcporong &&
						get.tag(trigger.card, "damage") > 0.5
					) {
		...
		filter(event, player, name) {
			...
			return suits.some((suit) => {
				if (
					!event
						.getd?.()
						.some((card) => get.suit(card, false) === suit)
				)
					return false;
				return (
					game
	...
	dcsblieji: {
		...
		filter(event, player) {
			return (
				...
				player.hasCard((card) => get.tag(card, "damage") > 0.5, "h")
			);
		},
		frequent: true,
		content() {
		...
		subSkill: {
			effect: {
					...
					return player.hasHistory(
						...
						(evtx) =>
							...
							Object.keys(evtx.gaintag_map).some((i) => {
								return evtx.gaintag_map[i].some((tag) =>
									tag.startsWith("dcsblieji_effect")
								);
							})
					);
				},
				...
				content() {
					...
					trigger.num += Object.keys(evtx.gaintag_map).reduce(
						(sum, i) => {
							const tag = evtx.gaintag_map[i].find((tag) =>
								tag.startsWith(skill)
							);
				...
				game.broadcastAll(
					(player, card, str, id, cardid) => {
						...
						if (game.chess)
							...
							event.node = player.$throwordered(
								card.copy(),
								true
							);
		...
		async content(event, trigger, player) {
					...
					humans.map((current) => {
						return new Promise(async (resolve, reject) => {
							if (current.isOnline()) {
								...
								const next = lib.skill.dcsbyaozuo.chooseCard(
									player,
									current
								);
			...
			if (locals.length > 0) {
				for (const current of locals) {
					const result = await lib.skill.dcsbyaozuo
						.chooseCard(player, current)
						.forResult();
					if (result && result.bool)
			...
			if (first && first.isIn()) {
				...
				if (
					!game.hasPlayer(
						(current) => ![first, player].includes(current)
					)
				)
					return;
				const result = await first
					...
					.set("ai", (target) => {
						...
						if (
							...
							target.hp <= Math.floor(target.maxHp)
						)
							return hs * 2;
						return hs;
					})
	...
	dcsbzhuanwen: {
		...
		async content(event, trigger, player) {
			...
			let cards = game.cardsGotoOrdering(
				get.cards(Math.min(5, target.countCards("h")))
			).cards;
			await player.showCards(
				cards,
				get.translation(player) + "发动了〖撰文〗"
			);
			let damages = cards.filter(
					(card) =>
						get.tag(card, "damage") &&
			...
			const list = [
				`依次对${get.translation(target)}使用${
					damages.length ? get.translation(damages) : "空气"
				}`,
				`令${get.translation(target)}获得${
					nodamages.length ? get.translation(nodamages) : "空气"
				}`,
			];
			const result = await player
			...
			if (result.control == "使用伤害牌") {
				while (damages.length) {
					...
					damages = damages.filter(
						(cardx) =>
							card != cardx && player.canUse(cardx, target, false)
					);
				}
			} else {
	...
	dcchaozhen: {
			...
			const control = await player
				.chooseControl(list, () => {
					...
					let cards = game
						...
						.sort(
							(a, b) =>
								get.number(a, false) - get.number(b, false)
						);
					...
					let valueCards = cards.filter((card) => {
						...
						if (
							get.position(card) == "j" &&
							(card.viewAs || card.name) == "jsrg_xumou"
						)
							att *= -1;
						if (
							get.position(card) == "e" &&
							get.equipValue(card, owner) > 0
						)
							att *= -1;
						return att > 0;
					});
		...
		async content(event, trigger, player) {
			...
			if (control == "场上") {
				...
				card = cards
					.filter((card) => get.number(card, false) == num)
					.randomGet();
			} else {
				while (num < 14) {
					let cardx = get.cardPile2(
						(card) => get.number(card, false) == num
					);
					if (cardx) {
						card = cardx;
						break;
		...
		async cost(event, trigger, player) {
			event.result = await player
				...
				.set(
					"drawed",
					player
						.getStorage("dclianjie_used")
						.includes(get.number(trigger.card, player) || 0)
				)
				.set("ai", (target) => {
					const player = get.player();
					...
					const eff2 = get.effect(
						target,
						{ name: "draw" },
						player,
						player
					);
					if (player == target && !get.event("drawed"))
						return (
							eff2 * (1 + player.maxHp - player.countCards("h"))
						);
					return eff1;
				})
				.forResult();
		...
		async content(event, trigger, player) {
			...
			const cards = target.getCards("h"),
				minNumber = cards
					...
					.sort((a, b) => a - b)[0];
			const toLose = cards.filter(
				(card) => get.number(card) === minNumber
			);
			if (target != player || toLose.length <= 1) {
				...
				const result = await player
					.chooseCard(
						"h",
						(card) => get.event("toLose")?.includes(card),
						true
					)
					.set("toLose", toLose)
		...
		mod: {
			aiOrder(player, card, num) {
				...
				if (player.countCards("h") < player.maxHp) {
					...
					!player.getStorage("dclianjie_used").includes(number)
				) {
					return num - 0.5;
				}
			},
	...
	dcjiangxian: {
		...
		async content(event, trigger, player) {
			...
			player
				...
				.filter((evt, player) => {
					return (
						evt == evtx &&
						["dcchaozhen", "dclianjie"].some((skill) =>
		...
		subSkill: {
			effect: {
				...
				intro: {
					content:
						"本回合因使用〖连捷〗摸的牌造成的伤害+1，回合结束后失去〖连捷〗或〖朝镇〗",
		...
		filter(event, player) {
			if (
				player
					.getHistory("gain", (evt) => {
						return (
							evt.getParent().name == "draw" && evt.cards.length
						);
					})
			...
			player
				...
				.then(() => {
					if (
						...
						target.addTempSkill("dcsbkongwu_effect", {
							player: "phaseEnd",
						});
					}
				})
			...
			effect: {
				...
				content() {
					if (trigger.name == "phaseDraw") {
						...
						player.disableSkill(
							event.name,
							lib.skill.dcsbkongwu.getSkills(player)
						);
				},
				onremove(player, skill) {
	...
	dcshangyi: {
		...
		filter: function (event, player) {
			return (
				...
				game.hasPlayer((current) =>
					lib.skill.dcshangyi.filterTarget(null, player, current)
				)
			);
		},
		filterTarget(card, player, target) {
		...
		filter(event, player) {
			...
			if (event.name == "dying") {
				return (
					...
					event.reason.source?.group == player.group
				);
			}
			return (
				event.player?.group == player.group && player.countCards("h")
			);
		},
		async content(event, trigger, player) {
	...
	dcchuanxin: {
		...
		filter(event, player) {
			...
			return (
				...
				event.getParent().name == event.card.name
			);
		},
		logTarget: "player",
		check(event, player) {
			if (get.attitude(player, event.player) < 0) {
				if (
					event.player.hp == 1 &&
					event.player.countCards("e") < 2 &&
					event.player.name2 != "gz_pangtong"
				)
					return false;
				return true;
			}
	...
	dcfengshi: {
		...
		filter: function (event, player) {
			if (event.card.name != "sha" || event.target.inRange(player))
				return false;
			return event.target.getCards("e", (card) =>
		...
		async cost(event, trigger, player) {
			event.result = await player
				.choosePlayerCard(
					"e",
					trigger.target,
					get.prompt2("dcfengshi", trigger.target)
				)
				.set("filterButton", (button) => {
					return ["equip2", "equip3"].includes(
			...
			if (index == 0) {
				...
				target
					...
					.then(() => {
						...
						if (
							!player.hasCard((card) =>
								card.hasGaintag("dcsbzuojun_tag")
							)
						) {
				...
				while (
					cards.some(
						(i) => get.owner(i) == target && target.hasUseTarget(i)
					)
				) {
				...
				filter(event, player) {
					...
					if (event.name == "lose") {
						return evt.cards2.some((card) =>
							(evt.gaintag_map[card.cardid] || []).includes(
								"dcsbmuwang_tag"
							)
						);
					}
			...
			const result = await target
				...
				.set("processAI", (list) => {
					...
					const cards1 = list[0][1]
						.slice()
						.sort(
							(a, b) => get.value(a, "raw") - get.value(b, "raw")
						);
					const cards2 = list[1][1]
						.slice()
						.sort(
							(a, b) => get.value(b, "raw") - get.value(a, "raw")
						);
					return [
	...
	dcyanzuo: {
		...
		check(card) {
			const player = _status.event.player;
			let val = ["trick", "basic"].includes(get.type(card, player))
		...
		async content(event, trigger, player) {
			...
			await next;
			const cards = player
				.getExpansions("dcyanzuo")
				.filter((i) => ["trick", "basic"].includes(get.type(i)));
	...
	dczuyin: {
		...
		async content(event, trigger, player) {
			...
			if (cards.some((card) => card.name == trigger.card.name)) {
				...
				const discards = cards.filter(
					(card) => card.name == trigger.card.name
				);
				if (discards.length) await player.loseToDiscardpile(discards);
			} else {
				if (
					player.countMark("dcyanzuo_add") < 2 &&
					player.hasSkill("dcyanzuo", null, null, false)
				) {
	...
	dcpijian: {
		...
		filter(event, player) {
			return (
				player.getExpansions("dcyanzuo").length >= game.countPlayer()
			);
		},
		locked: true,
		async cost(event, trigger, player) {
			...
			const { result } = await player
				.chooseButton(
					[
						...
						'<div class="text center">' +
							lib.translate[event.name + "_info"] +
							"</div>",
						[
							[
								"失去体力",
				...
				.set("ai", (button) => {
					...
					const listx = list.filter(
						(subtype) =>
							subtype !== "失去体力" &&
							!player.getEquips(subtype).length
					);
					return choice === (listx.length ? listx : list).randomGet()
						? 10
						: 0;
				});
	...
	dcjingyin: {
		...
		filter(event, player) {
			...
			if (!game.hasPlayer((target) => target != event.player))
				return false;
			return (
				event.card.name == "sha" && event.cards && event.cards.someInD()
			);
		},
		async cost(event, trigger, player) {
			event.result = await player
				.chooseTarget(
					get.prompt("dcjingyin"),
					"令一名角色获得" +
				...
				.set("ai", (target) => {
					...
					return (
						get.attitude(player, target) *
						cards.reduce(
							(sum, card) => sum + get.value(card, target),
							0
						)
					);
				})
		...
		async content(event, trigger, player) {
			...
			target
				.gain(trigger.cards.filterInD(), "gain2")
				.set("gaintag", "dcjingyin_tag");
		},
		subSkill: {
			tag: {
				charlotte: true,
	...
	dcchixing: {
		...
		async content(event, trigger, player) {
			const result = await player
				.draw(lib.skill.dcchixing.getNum(trigger).length)
				.forResult();
			if (
				Array.isArray(result) &&
				result.some((card) => get.name(card, false) == "sha")
			) {
				await player
					.chooseToUse(function (card) {
						...
						return (
							cards.containsSome(...evt.cards) &&
							get.name(card, false) == "sha"
						);
					}, "迟行：是否使用一张【杀】？")
					.set("cards", result);
			}
		...
		getNum(event) {
			return game
				...
				.reduce(
					(list, evt) =>
						list.addArray(evt.cards.filter((i) => i.name == "sha")),
					[]
				);
		},
	},
				...
				player(player, target) {
					return game
						...
						.reduce(
							(e, p) =>
								e +
								get.effect(p, { name: "draw" }, player, player),
							0
						);
				},
			},
	...
	dcwuxie: {
		...
		async cost(event, trigger, player) {
			event.result = await player
				...
				.set("ai", (target) => {
					const player = get.player();
					return (
						-get.attitude(player, target) * target.countCards("h")
					);
				})
				.forResult();
		},
		async content(event, trigger, player) {
			...
			if (cards1.length != cards2.length) {
				...
				const result = await player
					.chooseBool(
						"是否令" + get.translation(recover) + "回复1点体力？"
					)
					.set(
						"choice",
						get.recoverEffect(recover, player, player) > 0
					)
					.forResult();
	...
	dccilv: {
		...
		filter(event, player) {
			return (
				...
				player.getStorage("dccilv").length < 3
			);
		},
		async content(event, trigger, player) {
			...
			if (player.countCards("h") > player.maxHp) {
				let result,
					list = ["无效", "防伤", "获得"].filter(
						(i) => !player.getStorage("dccilv").includes(i)
					);
			...
			effect: {
				...
				filter(event, player) {
					...
					return (
						evt &&
						evt.name == "useCard" &&
						player.getStorage("dccilv_effect").includes(evt.card)
					);
				},
				forced: true,
				content() {
		...
		async content(event, trigger, player) {
			...
			if (gainSkills.length) {
				...
				Object.keys(target.storage)
					.filter((i) =>
						gainSkills.some((skill) => i.startsWith(skill))
					)
				...
				for (const skill of gainSkills) {
					...
					if (info.usable !== undefined) {
						if (
							target.hasSkill("counttrigger") &&
							target.storage.counttrigger[skill] &&
							target.storage.counttrigger[skill] >= 1
						) {
	...
	dcsbfuxi: {
		...
		async cost(event, trigger, player) {
			...
			if (!player.countCards("he")) {
				result = await player
					.chooseBool(
						get.prompt("dcsbfuxi", target),
						"弃置" + str + "的一张牌，然后视为对其使用一张【杀】"
					)
					.set(
						"choice",
						get.effect(
				...
				result = await player
					...
					.set(
						"choice",
						get.attitude(player, target) > 0 ||
							player.hasCard((card) => card.name == "du", "h")
					)
					.forResult();
				result.index = 0;
			} else {
	...
	dcsbhaoyi: {
		...
		prompt(event, player) {
			return (
				get.prompt("dcsbhaoyi") +
				"（可获得" +
				get.translation(lib.skill.dcsbhaoyi.getCards()) +
				"）"
			);
		},
		async content(event, trigger, player) {
			...
			while (
				player.hasCard(
					(card) =>
						cardx.includes(card) &&
						!card.hasGaintag("olsujian_given"),
					"h"
				)
			) {
				const {
		...
		getCards() {
			...
			const historyx = game.getGlobalHistory(
				"cardMove",
				(evt) => evt.name == "cardsDiscard"
			);
			if (historyx.length) {
			...
			const result = await player
				...
				.set("ai", () => {
					...
					return controls.sort((a, b) => {
						return (
							player.countCards("h", {
								color: a == "none2" ? "none" : a,
							}) -
							player.countCards("h", {
								color: b == "none2" ? "none" : b,
							})
						);
					})[0];
				})
			...
			effect: {
				...
				async content(event, trigger, player) {
					const func = () => {
						...
						event.controls = [
							ui.create.control(
								controls.concat(["清除选择", "stayleft"])
							),
						];
					};
					if (event.isMine()) func();
					else if (event.isOnline()) event.player.send(func);
					let types = trigger.cards.reduce(
						...
						.set("custom", {
							...
							replace: {
								button(button) {
									...
									const evt = event.parent;
									if (evt.controls)
										evt.controls[0].classList.remove(
											"disabled"
										);
									game.check();
								},
					...
					if (result.bool) {
						...
						if (result.links.includes(0))
							await player.draw(
								result.links.filter((count) => count == 0)
									.length
							);
						...
						if (result.links.includes(2)) {
							...
							player.addMark(
								"dcsbwuwei_count",
								result.links.filter((count) => count == 2)
									.length,
								false
							);
						}
				...
				async content(event, trigger, player) {
					let cards = game
						...
						.reduce(
							(cards, evt) =>
								cards.addArray(
									evt.getParent(3).cards.filterInD("d")
								),
							[]
						);
					while (cards.length) {
						const result = await player
							...
							.set("filterButton", (button) => {
								return get
									.event("player")
									.hasUseTarget(button.link, false);
							})
							.set("ai", (button) => {
		...
		async content(event, trigger, player) {
			...
			while (num > 0 && player.countCards("h")) {
				...
				if (compare.bool) {
					...
					let targets = player.storage[event.name].filter(
						(current) =>
							current != player &&
			...
			backup(result, player) {
				return {
					...
					async content(event, trigger, player) {
						const [target] = event.targets;
						const chosenNumber =
							get.info("dcsbtaozhou_backup").chosenNumber;
						const cards = await target
							.chooseToGive(
								`${get.translation(player)}对你发动了【讨州】`,
								"你可以交给其至多三张手牌",
								[1, 3],
								player
							)
							.set("ai", (card) => {
								if (get.event("att") > 0) {
									if (
										get.event("chosenNumber") <
										ui.selected.cards.length +
											(get.event().getRand() < 0.5)
									) {
						...
						const roundNumberToRestore =
							game.roundNumber + chosenNumber;
	...
	dcsbhoude: {
		...
		filter(event, player) {
			...
			const filter = (card) => {
				...
				return (
					(get.name(card) === "sha" && color === "red") ||
					(get.type(card) === "trick" && color === "black")
				);
			};
			const evt = event.getParent();
			if (
		...
		async cost(event, trigger, player) {
			...
			if (get.name(trigger.card) === "sha") {
				result = await player
					.chooseToDiscard(
						get.prompt("dcsbhoude", target),
						`弃置一张牌，令${get.translation(
							trigger.card
						)}对你无效。`,
						"chooseonly",
						"he"
					...
					.set(
						"goon",
						get.effect(player, trigger.card, target, player) < 0
					)
					.forResult();
			} else {
				result = await player
					.choosePlayerCard(
						`###${get.prompt(
							"dcsbhoude",
							target
						)}###<div class="text center">弃置其的一张牌，令${get.translation(
							trigger.card
						)}对你无效。</div>`,
						target,
						"he"
					...
					.set("ai", (button) => {
						...
						if (
							get.attitude(get.player(), get.owner(button.link)) >
							0
						)
							return -val;
						return val;
		...
		async content(event, trigger, player) {
			...
			if (result.links && result.links.length) {
				await target
					.discard(result.links, "notBySelf")
	...
	dcsbsushen: {
		...
		content() {
			...
			player.storage.dcsbsushen_reload = [
				Boolean(player.storage.dcsbfumou),
				player.countCards("h"),
				player.getHp(),
			];
		...
		*content(event, map) {
			...
			if (player.countCards("h") != storage[1]) {
				if (player.countCards("h") < storage[1]) {
					...
					yield player.chooseToDiscard(
						"h",
						true,
						player.countCards("h") - storage[1]
					);
				}
			}
			if (player.getHp() != storage[2]) {
				yield player[
					player.getHp() > storage[2] ? "loseHp" : "recover"
				](Math.abs(player.getHp() - storage[2]));
			}
	...
	dcsbfumou: {
		...
		filterTarget(card, player, target) {
			...
			return (
				!player.storage.dcsbfumou &&
				game.countPlayer((target) => target != player) > 1
			);
		},
		selectTarget() {
			const player = get.event("player");
		...
		targetprompt() {
			...
			return player.storage.dcsbfumou
				? ""
				: ["看牌角色", "得牌角色"][ui.selected.targets.length - 1];
		},
		prompt() {
			const player = get.event("player");
		...
		async content(event, trigger, player) {
			...
			await player.showCards(
				cards,
				get.translation(player) + "发动了【覆谋】"
			);
			if (!storage) {
				const aim = event.targets[1];
				if (aim) {
					cards = cards.filter((card) =>
						lib.filter.canBeGained(card, aim, target)
					);
					if (cards.length) {
		...
		zhuanhuanji(player, skill) {
			...
			player.changeSkin(
				{ characterName: "dc_sb_jiaxu" },
				"dc_sb_jiaxu" + (player.storage[skill] ? "_shadow" : "")
			);
		},
		marktext: "☯",
		mark: true,
		...
		ai: {
			...
			result: {
				...
				target(player, target) {
					...
					return Math.ceil(
						ui.selected.targets[0].countCards("h") / 2
					);
				},
			},
		},
				...
				if (!forced) {
					next.set("prompt", get.prompt("dcshouzhi"))
						...
						.set("ai", (card) => {
							const player = get.player();
							if (
								player.hasSkill("dcxingmen") &&
								get.recoverEffect(player, player) > 0
							)
								return 6 - get.value(card);
							return 0;
						});
		...
		onChooseToUse(event) {
			...
			event.set(
				...
				game.filterPlayer((current) => {
					return evts.filter((evt) => evt.targets.includes(current))
						.length;
				})
			);
		},
	...
	dcxingmen: {
		...
		filter(event, player) {
			return (
				event.getParent(2).name === "dcshouzhi" && player.isDamaged()
			);
		},
		frequent: true,
		prompt2: "你可以回复1点体力。",
		...
		subSkill: {
			norespond: {
				...
				filter(event, player) {
					return (
						...
						event.cards.some((card) => get.color(card) === "red")
					);
				},
				forced: true,
				locked: false,
			...
			directHit: {
				...
				filter(event, player) {
					return player.hasHistory("lose", (evt) => {
						if (evt.getParent() !== event) return false;
						return Object.values(evt.gaintag_map).some((tags) =>
							tags.includes("dcxingmen")
						);
					});
				},
				async content(event, trigger, player) {
			...
			inTwo: {
				...
				filter(event, player) {
					...
					return game.hasPlayer((current) => {
						...
						if (!evt) return false;
						return (
							evt.hs.some((i) => cards.includes(i)) ||
							evt.es.some((i) => cards.includes(i)) ||
							evt.js.some((i) => cards.includes(i))
						);
					});
				},
				forced: true,
				...
				async content(event, trigger, player) {
					...
					game.countPlayer((current) => {
						...
						if (!evt) return false;
						cards.addArray(
							evt.hs.filter((i) => gcards.includes(i))
						);
						cards.addArray(
							evt.es.filter((i) => gcards.includes(i))
						);
						cards.addArray(
							evt.js.filter((i) => gcards.includes(i))
						);
					});
				...
				return {
					...
					async content(event, trigger, player) {
						...
						if (position === "field") {
							...
							cards.addArray(
								Array.from(ui.discardPile.childNodes).filter(
									(card) => {
										return get.type(card) === "equip";
									}
								)
				...
				player(player) {
					...
					if (
						...
						(player.getHp() > 3 &&
							...
							!knownEnemies.some(
								(enemy) => get.attitude(player, enemy) <= -9
							))
					)
						return 0;
					const val1 = game
	...
	dchuiji: {
		...
		chooseButton: {
			...
			check(button) {
				...
				const drawWugu =
					target.countCards("h") + 2 > game.countPlayer();
			...
			result: {
				target(player, target) {
					...
					if (wugu)
						return (
							Math.min(0, att) *
							Math.min(3, target.countCards("h"))
						);
					return (
						Math.max(0, att) * Math.min(3, target.countCards("h"))
					);
				},
			},
		},
			...
			aiOrder(player, card, num) {
				if (
					...
					player.countCards("h", (cardx) => {
						return (
							cardx.hasGaintag("dczhimin_tag") && cardx !== card
						);
					}) < player.maxHp
				)
					return num / 10;
	...
	dcsbquanmou: {
		...
		zhuanhuanji(player, skill) {
			player.storage[skill] = !player.storage[skill];
			player.changeSkin(
				{ characterName: "dc_sb_simayi" },
				"dc_sb_simayi" + (player.storage[skill] ? "_shadow" : "")
			);
		},
		marktext: "☯",
		enable: "phaseUse",
		...
		async content(event, trigger, player) {
			...
			const cards = await target
				.chooseCard(
					"he",
					true,
					`选择交给${get.translation(player)}一张牌`
				)
				.forResultCards();
			if (cards && cards.length) {
				await target.give(cards, player);
				const key = `dcsbquanmou_${Boolean(
					!player.storage.dcsbquanmou
				)}`;
				player.addTempSkill(key, {
					global: ["phaseUseBefore", "phaseChange"],
				});
				player.markAuto(key, [target]);
				target.addAdditionalSkill(
					`${key}_${player.playerid}`,
					`${key}_mark`
				);
			}
		},
		...
		subSkill: {
			change: {
				...
				prompt2(event, player) {
					return (
						"切换【权谋】为状态" +
						(player.storage.dcsbquanmou ? "阳" : "阴")
					);
				},
				check: () => Math.random() > 0.5,
				content() {
			...
			true: {
				...
				async content(event, trigger, player) {
					...
					target.removeAdditionalSkill(
						`dcsbquanmou_true_${player.playerid}`
					);
					if (
						game.hasPlayer(
			...
			false: {
				...
				async content(event, trigger, player) {
					...
					target.removeAdditionalSkill(
						`dcsbquanmou_false_${player.playerid}`
					);
					trigger.cancel();
				},
				onremove(player, skill) {
				...
				ai: {
					...
					skillTagFilter(player, tag, arg) {
						return (
							arg &&
							arg.player &&
							arg.player.hasSkill("dcsbquanmou_false")
						);
					},
					effect: {
						target(card, player, target) {
	...
	dcsbpingliao: {
		...
		async content(event, trigger, player) {
			...
			const prompt = `###是否打出红色基本牌响应${get.translation(
				player
			)}？###${get.translation(
				player
				...
				if (target.isIn() && player.inRange(target)) {
					const result = await target
						...
						.set("ai", (card) => {
							...
							return event.getRand("dcsbpingliao") >
								1 / Math.max(1, player.hp)
								? 0
								: get.order(card);
						})
			...
			if (nonnonTargetResponded) {
				player.draw(2);
				player.addTempSkill("dcsbpingliao_buff", {
					global: "phaseChange",
				});
		...
		async content(event, trigger, player) {
			const {
				...
				.set("ai", (target) => {
					const player = get.event("player");
					return (
						-get.attitude(player, target) * target.countCards("he")
					);
				});
			if (bool) {
				const target = targets[0],
				...
				let choiceList = [
					"此阶段结束时，若" +
						str +
						"本阶段获得过牌，则你获得其手牌区和装备区各一张牌",
				];
				if (player.countCards("h", { type: "basic" }))
					choiceList.push(
						"交给" +
							str +
							"一张基本牌，然后其交给你一张非基本牌或两张基本牌"
					);
				const {
					result: { index },
				} = await player
	...
	dczhantao: {
		...
		check(event, player) {
			if (
				...
				return 0;
			return (
				get.effect(event.source, { name: "sha" }, player, player) >= 0
			);
		},
		logTarget: "player",
		async content(event, trigger, player) {
			player
				.judge((card) => {
					const evt = get
						.event()
						.getParent(get.event("eventName"))
						.getTrigger();
					if (
						!evt.source ||
						!evt.source.isIn() ||
						!evt.card ||
						typeof get.number(evt.card) !== "number"
					)
						return 0;
		...
		async content(event, trigger, player) {
			const maxCount =
				player.getAllHistory(
					"useSkill",
					(evt) => evt.skill === "dcanjing"
				).length + 1;
			const result = await player
				.chooseTarget(
					get.prompt2("dcanjing"),
	...
	dcjichun: {
		...
		filter(event, player) {
			return player.countCards("he", (card) =>
				lib.skill.dcjichun.filterCard(card, player)
			);
		},
		filterCard(card, player) {
	...
	dchanying: {
		...
		async content(event, trigger, player) {
			const card = get.cardPile(
				(card) =>
					get.type(card) == "equip" && !get.cardtag(card, "gifts")
			);
			if (!card) {
				player.chat("无牌可得？！");
	...
	dcyuxin: {
		...
		filter(event, player) {
			return (
				event.player.hp < (event.player == player ? 1 : player.getHp())
			);
		},
		prompt2(event, player) {
			return (
				"令其将体力值回复至" +
				(event.player == player ? 1 : player.getHp()) +
				"点"
			);
		},
		check(event, player) {
		...
		async content(event, trigger, player) {
			...
			trigger.player.recover(
				(trigger.player == player ? 1 : player.getHp()) -
					trigger.player.hp
			);
		},
	},
	...
	dczhangji: {
		...
		filter(event, player) {
			...
			if (
				...
				event.getParent().triggeredTargets4.length
			)
				return false;
		...
		async content(event, trigger, player) {
			...
			evtx.triggeredTargets4 = [
				player,
				...evtx.triggeredTargets4.remove(player),
			];
		...
		subSkill: {
			debuff: {
				...
				mod: {
					...
					aiUseful() {
						return lib.skill.dczengou.subSkill.debuff.mod.aiValue.apply(
							this,
							arguments
						);
					},
				...
				.set("ai", (card) => {
					let player = _status.event.player,
						...
						cards = ui.selected.cards.concat(
							player.getExpansions("dclingxi")
						),
						suit = get.suit(card, false);
					if (_status.event.suits.length < 4)
						_status.event.suits.add(
							get.suit(ui.selected.cards.at(-1), false)
						);
					...
					if (
						_status.event.hvt.length === 1 &&
						card === _status.event.hvt[0]
					)
						return 0;
					let temp;
					if (
			...
			effect: {
				...
				filter: function (event, player) {
					...
					return game.getGlobalHistory("cardMove", function (evt) {
						...
						for (var i in evt.gaintag_map) {
							if (
								evt.gaintag_map[i].includes("dclingxi") &&
								evt.player == player
							)
								return true;
						}
	...
	dczhifou: {
		...
		filter: function (event, player) {
			var num =
				player.getHistory("useSkill", (evt) => evt.skill == "dczhifou")
					.length + 1;
			...
			var result = yield player
				...
				.set(
					...
					(() => {
						...
						if (
							temp > 0 ||
							(!temp && num < Math.max(2, 5 - player.hp))
						)
							cs = true;
						else cs = false;
						return {
				...
				if (result2.bool) {
					...
					switch (result3.control) {
						case "置入“翼”":
							...
							var result4 = yield target.chooseCard(
								"he",
								choiceList[0],
								true
							);
	...
	dcsbyingmou: {
		...
		zhuanhuanji(player, skill) {
			player.storage[skill] = !player.storage[skill];
			player.changeSkin(
				{ characterName: "dc_sb_zhouyu" },
				"dc_sb_zhouyu" + (player.storage[skill] ? "_shadow" : "")
			);
		},
		intro: {
			content: function (storage) {
				if (!storage)
					return "每回合限一次，当你对其他角色使用牌后，你可以选择其中一名目标角色，你将手牌数摸至与其相同（至多摸五张），然后视为对其使用一张【火攻】。";
						...
						ai2: function (target) {
							...
							if (!ui.selected.targets.length) {
								var targets = game.filterPlayer((target) =>
									target.isMaxHandcard()
								);
								targets.sort(
									(a, b) =>
										getNum(b, target, player) -
										getNum(a, target, player)
								);
								return getNum(targets[0], target, player) + 1;
							}
							return (
								getNum(target, ui.selected.targets[0], player) +
								1
							);
						},
					})
	...
	dcsbmingshi: {
		...
		content: function () {
			...
			player
				.when("phaseDrawEnd")
				.filter(
					(evt, player) => evt == trigger && player.countCards("h")
				)
				.then(() => {
					var str =
						"明势：请展示三张牌并令一名其他角色选择获得其中的一张牌";
					if (player.countCards("h") <= 3)
						str =
							"明势：展示手牌并令一名其他角色选择获得其中的一张牌";
	...
	dcsbmengmou: {
		...
		zhuanhuanji(player, skill) {
			player.storage[skill] = !player.storage[skill];
			player.changeSkin(
				{ characterName: "dc_sb_lusu" },
				"dc_sb_lusu" + (player.storage[skill] ? "_shadow" : "")
			);
		},
		intro: {
			content: function (storage) {
		...
		filter: function (event, player) {
			...
			if (
				player
					.getStorage("dcsbmengmou_used")
					.includes(player.storage.dcsbmengmou ? "yin" : "yang")
			)
				return false;
			...
			if (targets.length == 1) {
				...
				result = yield player
					...
					.set(
						"choice",
						(storage
							? check_false(player, target)
							: check_true(player, target)) > 0
					);
			} else {
				result = yield player
					.chooseTarget(
		...
		subSkill: {
			...
			change: {
				...
				prompt2(event, player) {
					return (
						"切换【盟谋】为状态" +
						(player.storage.dcsbmengmou ? "阳" : "阴")
					);
				},
				check: () => Math.random() > 0.5,
				content() {
	...
	dcmiyi: {
		...
		content: function* (event, map) {
			...
			result = yield player
				.chooseTarget(
					`蜜饴：令任意名角色${result.control.slice(
						0,
						2
					)}1点${result.control.slice(2)}`,
					[1, Infinity]
				)
				.set("ai", (target) => {
	...
	dcyinjun: {
		...
		content: function* (event, map) {
			...
			if (
				player.getHistory("useSkill", (evt) => evt.skill == "dcyinjun")
					.length > player.getHp()
			) {
		...
		content: function* (event, map) {
			...
			if (result.bool) {
				...
				targets.forEach((current, ind) => {
					current
						.addToExpansion(cards[ind], "gain2")
						.gaintag.add("dcxiaoyin");
					game.log(
						current,
						"将",
						cards[ind],
			...
			player
				...
				.then(() => {
					if (trigger.target.getExpansions("dcxiaoyin").length) {
						...
						player
							.chooseBool(
								`是否更改${get.translation(
									trigger.card
								)}的目标？`,
								`将此牌的目标改为所有有“硝引”的角色（${get.translation(
									targets
								)}）。`
							)
							.set(
	...
	dcxiongmu: {
		...
		content: function () {
			...
			if (result.bool) {
				...
				game.log(
					player,
					`将${get.cnNumber(cards.length)}张牌置入了牌堆`
				);
				player
					.loseToDiscardpile(cards, ui.cardPile, "blank")
					.set("log", false).insert_index = function () {
	...
	dczhangcai: {
			...
			aiOrder: (player, card, num) => {
				if (
					...
					ui.cardPile.childNodes.length +
						ui.discardPile.childNodes.length <
						20
				)
					return 0;
			},
		...
		prompt2: function (event, player) {
			const num = player.hasSkill("dczhangcai_all")
		...
		locked: false,
		content: function () {
			var num = player.hasSkill("dczhangcai_all")
	...
	dcshangyu: {
		...
		onremove: () => {
			if (
				!game.hasPlayer(
					(i) => i.hasSkill("dcshangyu", null, null, false),
					true
				)
			)
		...
		subSkill: {
			ai: {
				mod: {
					...
					aiUseful: function () {
						return lib.skill.dcshangyu_ai.mod.aiValue.apply(
							this,
							arguments
						);
					},
		...
		content: function () {
			...
			player
				...
				.set(
					"goon",
					player.needsToDiscard() ||
						player.getExpansions("dcxingwu").length == 2
				);
			("step 1");
			if (result.bool) {
			...
			if (result.bool) {
				...
				player.chooseTarget(
					...
					return (
						-get.attitude(player, target) *
						att *
						Math.max(1, target.countCards("e"))
					);
				};
			}
			("step 4");
			if (result.bool) {
				...
				result.targets[0].damage(
					result.targets[0].sex == "female" ? 1 : list.length
				);
			}
		},
	},
			...
			if (!game.online && !event.dcposuo_cards) {
				...
				suits = suits.filter(
					(suit) =>
						...
						player.countCards(
							"hs",
							(card) => get.suit(card, player) == suit
						)
				);
				if (
					!suits.length ||
					...
					for (var name of lib.inpile) {
						if (
							...
							get.type(new lib.element.VCard({ name: name })) ===
								"delay"
						)
							continue;
			...
			backup: function (links, player) {
				return {
					...
					precontent: function () {
						...
						player.markAuto("dcposuo_suits", [
							get.suit(event.result.cards[0]),
						]);
					},
				};
			},
	...
	dcxiaoren: {
		...
		check: (event, player) => {
			let rev = game.countPlayer((i) => {
				return (
					i.isDamaged() && get.attitude(_status.event.player, i) > 0
				);
			});
		...
		content: function () {
			...
			("step 7");
			if (
				player.storage.dcxiaoren_dying ||
	...
	dcpandi: {
			...
			if (!game.online && event.type == "phase" && !event.dcpandi) {
				var players = game.filterPlayer(function (current) {
					return (
						...
						current.getHistory("sourceDamage").length == 0
					);
				});
		...
		getUseValue: function (card, player, viewer) {
			...
			for (var i = 0; i < targets.length; i++) {
				if (
					lib.skill.dcpandi.pandi_canUse(
						player,
						card,
						targets[i],
						null,
					...
					var eff = lib.skill.dcpandi.pandi_effect(
						targets[i],
						card,
						player,
						viewer
				...
				ai: {
					order: function (card, player, target, current) {
						...
						if (
							...
							get.itemtype(source.storage.dcpandi_effect) ==
								"player"
						)
							return;
						return [
							0,
							lib.skill.dcpandi.pandi_effect_use(
								target,
								card,
								source,
								player
							...
							0,
							lib.skill.dcpandi.pandi_effect(
								target,
								card,
								source,
								target
				...
				.chooseCard(
					...
					function (card, player) {
						...
						return !player.hasCard((card2) => {
							return (
								card != card2 && get.number(card2, player) < num
							);
						});
					}
				)
	...
	dcchangqu: {
		...
		content: function () {
			...
			if (target.hasMark("dcchangqu_warshipx")) {
				...
				target
					.chooseCard(
						get.translation(player) + "对你发动了【长驱】",
						prompt2,
						num
					)
					.set("ai", (card) => {
						...
						return (
							...
							0.8 *
								Math.min(
									5,
									_status.event.target.hp +
										_status.event.target.hujia
								) -
							get.value(card)
						);
					})
			...
			if (nextPlayer) {
				...
				nextPlayer.addMark(
					"dcchangqu_warship",
					target.countMark("dcchangqu_warship")
				);
				nextPlayer.addMark(
					"dcchangqu_warshipx",
	...
	dcshiji: {
		...
		content: function () {
			...
			if (list.length) {
				player
					.chooseButton([
						get.prompt("dcshiji", trigger.player),
						[list, "vcard"],
					])
					.set("ai", (button) => {
		...
		content: function () {
			...
			if (result.bool) {
				...
				player
					...
					.set(
						"prompt",
						"请选择要将" + get.translation(card) + "置于的位置"
					)
					.set("ai", () => {
						return _status.event.choice;
					})
	...
	dcjianguo: {
		...
		filter: function (event, player) {
			return ["discard", "draw"].some(
				(i) => !player.getStorage("dcjianguo_used").includes(i)
			);
		},
		chooseButton: {
			dialog: function (event, player) {
			...
			check: function (button) {
				...
				if (button.link == "draw") {
					var draw = Math.max.apply(
						...
						game
							.filterPlayer((current) => {
								return lib.skill.dcjianguo_draw.filterTarget(
									null,
									player,
									current
								);
	...
	dcxunbie: {
		...
		filter: function (event, player) {
			...
			game.countPlayer((current) => {
				if (
					current.name1 == "dc_ganfuren" ||
					current.name2 == "dc_ganfuren"
				) {
					characters.remove("dc_ganfuren");
				}
				if (
					current.name1 == "dc_mifuren" ||
					current.name2 == "dc_mifuren"
				) {
		...
		content: function () {
			...
			player.reinitCharacter(
				get.character(player.name2, 3).includes("dcxunbie")
					? player.name2
					: player.name1,
				character
			);
	...
	dcyongjue: {
		...
		content: function () {
			...
			var choices = ["选项一"];
			var choiceList = [
				"令" + get.translation(trigger.card) + "不计入次数",
				"获得此牌",
			];
			if (trigger.cards.length) {
				choices.push("选项二");
		...
		content: function () {
			...
			player
				...
				.set("ai", (target) => {
					var player = _status.event.player;
					return (
						get.recoverEffect(target, player, player) / 2 +
						get.attitude(player, target)
					);
				});
			("step 2");
				...
				player
					...
					.set(
						...
						(function () {
							if (
								target.hp <= 2 ||
								get.recoverEffect(target, player, player) > 20
							)
								return 0;
							return "选项二";
						})()
					...
					backup: function (links, player) {
						return {
							...
							onuse: function (links, player) {
								...
								player.markAuto("dczhaowen_viewed", [
									links.card.name,
								]);
							},
						};
			...
			draw: {
				...
				filter: function (event, player) {
					var cards = event.cards.filter(
						(card) => get.color(card, player) == "red"
					);
					return player.hasHistory("lose", (evt) => {
						if (event != evt.getParent()) return false;
						for (var i in evt.gaintag_map) {
				...
				content: function () {
					var num = 0;
					var cards = trigger.cards.filter(
						(card) => get.color(card, player) == "red"
					);
		...
		filter: function (event, player) {
			...
			if (player.hasSkill("jiu"))
				return player.countCards("h", (card) => {
					return (
						_status.connectMode ||
						lib.filter.cardDiscardable(card, player, "dcjiudun")
					);
				});
			return true;
		},
						...
						(function () {
							if (
								...
								-4 *
									Math.max(
										0,
										5 - Math.sqrt(player.countCards("h"))
									)
							)
								return true;
							return false;
				...
				.set("processAI", (list) => {
					...
					switch (name) {
						...
						case "phaseZhunbei":
							...
							for (
								let i =
									Math.min(cards.length, judges.length) - 1;
								i >= 0;
								i--
							) {
								let j = judges[i],
								...
								if (
									wuxie > 0 &&
									get.effect(target, j, target, target) < 0
								) {
									wuxie--;
									continue;
								}
						...
						default:
							cards.sort((a, b) => {
								return (
									get.value(b, target) - get.value(a, target)
								);
							});
							while (cards.length) {
	...
	dcqingshi: {
		...
		content: function () {
							...
							for (var i = 0; i < cards.length; i++) {
								...
								for (var j = i + 1; j < cards.length; j++) {
									if (
										...
										get.position(cards[i]) +
											get.position(cards[j]) !==
											"ss" &&
			...
			if (result.control != "cancel2") {
				...
				game.log(player, "选择了", "#y" + result.control);
				var index =
					["选项一", "选项二", "选项三"].indexOf(result.control) + 1;
		...
		subSkill: {
			ex: {
				...
				filter: function (event, player) {
					return (
						...
						player.storage.dcqingshi_ex.some((info) => {
							return (
								info[0] == event.player && info[1] == event.card
							);
						})
					);
				},
				...
				content: function () {
					...
					for (
						var i = 0;
						i < player.storage.dcqingshi_ex.length;
						i++
					) {
			...
			effect: {
				...
				content: function () {
					...
					if (cards.length) {
						player
							.gain(cards, "gain2")
							.gaintag.addArray(["dczhizhe", "dczhizhe_clear"]);
	...
	dccaizhuang: {
		...
		tempCache() {
			...
			let sortedSuits = Object.fromEntries(
	...
	dchuayi: {
		...
		content: function () {
			...
			if (color == "red" || color == "black")
				player.addTempSkill("dchuayi_" + color, {
					player: "phaseBegin",
				});
		},
		subSkill: {
		...
		content: function () {
			...
			if (
				player.hasHistory("useCard", (evt) => {
					return (
						evt.getParent(2).name == "dcjianzheng" &&
	...
	dcfumou: {
		...
		content: function () {
			...
			("step 1");
			player
				.chooseTarget(get.prompt2("dcfumou"), [
					1,
					player.getDamagedHp(),
				])
				.set("ai", (target) => {
	...
	dcquanshou: {
		...
		content: function () {
			...
			var draw = Math.min(
				5,
				trigger.player.maxHp - trigger.player.countCards("h")
			);
			trigger.player
				.chooseControl()
				.set("choiceList", [
					(draw > 0
						? "摸" + get.cnNumber(draw) + "张牌，然后"
						: "令") + "你本回合使用【杀】的次数上限-1",
					"当你本回合使用牌被抵消后，" +
						get.translation(player) +
						"摸一张牌",
		...
		subSkill: {
			...
			respond: {
				...
				onremove: true,
				marktext:
					'<span style="text-decoration: line-through;">守</span>',
				...
				async content(event, trigger, player) {
					...
					await trigger.player
						...
						.set(
							...
							将一张牌当做${get.translation(card.nature) || ""}【${get.translation(
								card.name
							)}】使用`
						)
						.set("norestore", true)
	...
	dcdanyi: {
		...
		filter: function (event, player) {
			...
			var evt = lib.skill.dcjianying.getLastUsed(
				player,
				event.getParent()
			);
		...
		content: function () {
			...
			player.draw(
				trigger.targets.filter((target) => evt.targets.includes(target))
					.length
			);
		},
		mod: {
		...
		filter: function (event, player, name) {
			if (player == _status.currentPhase)
				return (
					...
					player.getExpansions("dcwangyuan").length <
						game.countPlayer2()
				);
		...
		content: function () {
			...
			var card = get.cardPile2((cardx) => {
				...
				return (
					...
					!cards.some(
						(cardxx) =>
							get.name(cardx, false) == get.name(cardxx, false)
					)
				);
			}, "random");
			if (card)
				...
				.set("ai", (button) => {
					...
					if (
						...
						ui.selected.buttons.length ==
							player.getExpansions("dcwangyuan").length - 1
					)
						return 0;
	...
	dcyuandi: {
		...
		onremove: () => {
			if (
				!game.hasPlayer(
					(i) => i.hasSkill("dcyuandi", null, null, false),
					true
				)
			)
				...
				player
					...
					.set("ai", (target) => {
						...
						return list.reduce((p, c) => {
							return (
								p +
								get.effect(target, { name: c }, player, player)
							);
						}, 0);
					});
			("step 1");
			...
			effect: {
				target: function (card, player, target, current) {
					if (get.type(card) == "delay" && current < 0) {
						...
						if (current.getSeatNum() > target.getSeatNum())
							return 0.1;
					}
				},
			},
	...
	dcaichen: {
		...
		onremove: function (player) {
			if (
				!game.hasPlayer(
					(current) =>
						current.hasSkill("dcaichen", null, null, false),
					true
				)
			)
		...
		filter: function (event, player, name) {
			...
			if (
				...
				player.hasHistory(
					"lose",
					(evtx) =>
						evtx.getParent("dcluochong", true) == evt &&
						evtx != event
				)
			)
				return false;
		...
		subSkill: {
			choose: {
				...
				content: function () {
					...
					if (
						game.hasPlayer(
							(current) => current.countCards("h") == 0
						)
					)
						choices.push("选项一");
					else
	...
	dcjinjie: {
			...
			const result = await player
				...
				.set(
					...
					(() => {
						...
						const unusefulCount = player.countCards(
							"he",
							(card) => {
								return (
									lib.filter.cardDiscardable(
		...
		async content(event, trigger, player) {
			...
			if (num > 0) {
				next = player
					.chooseToDiscard(
						`尽节：是否弃置${get.cnNumber(
							num
						)}张牌，令${get.translation(target)}回复1点体力？`,
						num,
						"he"
					)
					.set("ai", (card) => {
				...
				next = player
					.chooseBool(
						`尽节：是否令${get.translation(target)}回复1点体力？`
					)
	...
	dcjue: {
		...
		async content(event, trigger, player) {
			...
			let maxLimit = Math.min(
				lib.skill.dcjue.getCards().length,
				target.maxHp
			);
			while (maxLimit--) {
				...
				for (const name of list) {
					...
					let targets = [player, target].filter((current) =>
						player.canUse(card, current)
					);
					if (targets.length) {
	...
	dcyuanmo: {
		...
		content: function () {
			...
			if (result.control == "选项一") {
				...
				var targetsx = game.filterPlayer((current) =>
					player.inRange(current)
				);
	...
	dcxialei: {
		...
		filter: function (event, player) {
			...
			if (event.name != "cardsDiscard") {
				...
				return player.hasHistory("lose", (evtxx) => {
					return (
						evtx == (evtxx.relatedEvent || evtxx.getParent()) &&
						evtxx.cards2.length > 0
					);
				});
			}
		},
		async content(event, trigger, player) {
			...
			if (cards.length == 1) result = { bool: true, links: cards };
				...
				result = await player
					.chooseButton(["霞泪：获得其中的一张牌", cards], true)
					.forResult();
			if (result.bool) {
			...
			player.judge((result) => {
				if (get.color(result) == "red")
					return (
						_status.event
							.getParent()
							.player.countMark("dcxialei_clear") / 2
					);
				return 2;
	...
	dclianji: {
		...
		content: function () {
			...
			var card = get.cardPile2(function (card) {
				return (
					get.subtype(card) == "equip1" &&
					targets[0].hasUseTarget(card)
				);
			}, "random");
			if (card) {
				if (
					card.name == "qinggang" &&
					!lib.inpile.includes("qibaodao")
				) {
					card.remove();
			...
			game.updateRoundNumber();
			targets[0]
				.chooseToUse(get.translation(player) + "对你发动了【连计】", {
					name: "sha",
				})
				.set("targetRequired", true)
				.set("complexSelect", true)
			...
			if (result.bool) {
				...
				if (
					card.length &&
					result.targets.filter((target) => target.isIn()).length > 0
				) {
					event.card = card;
					targets[0]
						.chooseTarget(
	...
	dcmoucheng: {
		...
		filter: function (event, player) {
			return (
				player.hasSkill("dclianji_1") && player.hasSkill("dclianji_2")
			);
		},
		content: function () {
			...
			wake: {
				...
				async content(event, trigger, player) {
					...
					if (name === "phaseBegin") {
							...
							for (let i = 0; i < storage[0].length; i++) {
								...
								if (target?.isIn()) {
									if (target.hp != storage[1][i]) {
										...
										const next = target.changeHp(
											storage[1][i] - target.hp
										);
						...
						player.storage[event.name][0] = player.storage[
							event.name
						][1] = [];
						player[
							player.storage[event.name][2].length
								? "unmarkSkill"
								: "removeSkill"
						](event.name);
					}
			...
			all: {
				mod: {
					aiOrder(player, card, num) {
						if (
							num <= 0 ||
							!game.hasPlayer((t) => t.marks["dcwumei_wake"])
						)
							return;
						if (
							get.tag(card, "recover") &&
							...
							if (
								...
								game.hasPlayer((cur) => {
									return (
										...
										get.effect(cur, card, player, player) >
											0
									);
								})
							)
				...
				(!player.hasSkill("dczhanmeng_choice0") &&
					!game.hasPlayer2((current) => {
						...
						for (let i = history.length - 2; i >= 0; i--) {
							...
							const list = history[i].useCard.map(
								(evt) => evt.card.name
							);
			...
			delay: {
				...
				filter(event, player, name) {
					...
					if (name == "phaseBeginStart") {
						return !list.some(
							(evt) => evt["dczhanmeng_" + player.playerid]
						);
					}
					for (var evt of list) {
						if (
	...
	dcjingzao: {
		...
		filter: function (event, player) {
			if (
				3 +
					player.countMark("dcjingzao_add") -
					player.countMark("dcjingzao_ban") <=
				0
			)
				return false;
			return game.hasPlayer((current) =>
	...
	dclianzhi: {
		...
		filter: function (event, player) {
			return player.getStorage("dclianzhi").filter((i) => i && i.isIn())
				.length;
		},
		content: function () {
			player.recover();
			...
			reproach: {
				...
				content: function () {
					...
					if (result.bool) {
						...
						target.addMark(
							"dclingfang",
							Math.max(1, player.countMark("dclingfang"))
						);
					}
				},
			},
	...
	dcfengying: {
		...
		filter: function (event, player) {
			...
			if (
				mark <= 0 ||
				!player.hasCard((card) => get.number(card) <= mark, "hs")
			)
				return false;
		...
		hiddenCard: function (player, name) {
			...
			return (
				...
				player.hasCard(
					(card) =>
						get.number(card) <= player.countMark("dclingfang"),
					"hs"
				)
			);
		},
			...
			backup: function (links, player) {
				return {
					...
					precontent: function () {
						...
						player.markAuto("dcfengying_used", [
	...
	dcshouze: {
		...
		content: function () {
			...
			("step 1");
			var card = get.discardPile(
				(card) => get.color(card, false) == "black",
				"random"
			);
			if (card) player.gain(card, "gain2");
			player.loseHp();
		...
		check: function (event, player) {
			...
			if (
				current.countCards(
					"hs",
					(card) =>
						current.canUse(card, player) &&
						get.effect(player, card, current, player) < 0
				) >= 2
			)
				return false;
			return true;
	...
	olddcxiangmian: {
		...
		filter: function (event, player) {
			return (
				!player.getStorage("olddcxiangmian").includes(event.player) &&
				player != event.player
			);
		},
		logTarget: "player",
		...
		subSkill: {
			countdown: {
				...
				content: function () {
					...
					for (var i = 0; i < storage.length / 3; i++) {
						if (
							storage[i * 3 + 1] == get.suit(trigger.card, player)
						) {
							storage[i * 3 + 2]--;
						}
					}
					...
					for (var i = 0; i < storage.length / 3; i++) {
						if (storage[i * 3 + 2] <= 0) {
							...
							player.storage["olddcxiangmian_countdown"].splice(
								i * 3,
								3
							);
							if (
								!player.getStorage("olddcxiangmian_countdown")
									.length
							) {
	...
	dcxiangmian: {
		...
		filterTarget: function (card, player, target) {
			return (
				!player.getStorage("dcxiangmian").includes(target) &&
				player != target
			);
		},
		content: function () {
				...
				content: function () {
					...
					for (var i = 0; i < storage.length / 3; i++) {
						if (storage[i * 3 + 2] <= 0) {
							...
							player.storage["dcxiangmian_countdown"].splice(
								i * 3,
								3
							);
							if (
								!player.getStorage("dcxiangmian_countdown")
									.length
							) {
	...
	dctianji: {
		...
		filter: function (event, player) {
			...
			return (
				evt && evt.name == "judge" && event.cards.filterInD("d").length
			);
		},
		content: function () {
			...
			for (var fn of func) {
				var cardx = get.cardPile2((cardxx) => {
					if (
						get[fn](card, player) == get[fn](cardxx, player) &&
						!cards.includes(cardxx)
					) {
						return true;
					}
				}, "random");
			...
			player
				...
				.set(
					...
					skills.map((i) => {
						...
						switch (player.getStorage("dctongguan")[i]) {
							...
							case 2:
								info =
									' style="text-decoration:line-through; opacity:0.3;"';
								break;
						}
						return (
							'<div class="skill">「' +
							...
							"（已选过" +
							get.cnNumber(
								player.getStorage("dctongguan")[i] || 0
							) +
							"次）" +
							"</div>"
						);
				...
				.set("ai", function () {
					...
					if (
						...
						/(当【过河拆桥】使用|((弃置|获得)\S{1,5}其他角色\S{1,7}牌|))/.test(
							str
						)
					)
	...
	dcmengjie: {
		...
		filter: function (event, player) {
			...
			if (
				...
				(target.hasSkill("dctongguan_wuyong") &&
					target.getHistory("sourceDamage").length) ||
			...
			game.countPlayer2((current) => {
				...
				if (
					...
					current.hasHistory("lose", (evt) => {
						if (evt.type == "discard") {
							if (
								(evt.discarder || evt.getParent(2).player) !=
								target
							)
								return false;
	...
	dchuace: {
		...
		onChooseToUse: function (event) {
			...
			var list = lib.inpile.filter(function (i) {
				return (
					get.type(i) == "trick" &&
					lib.filter.filterCard({ name: i }, event.player, event)
				);
			});
			if (!list.length) {
		...
		chooseButton: {
			...
			prompt: function (links, player) {
				return (
					"将一张手牌当做【" + get.translation(links[0][2]) + "】使用"
				);
			},
		},
		ai: {
				...
				mod: {
					targetEnabled: function (card, player, target) {
						if (
							...
							return;
						var state1 = lib.skill.dcyaoyi.getZhuanhuanji(
							player,
							true
						);
						if (state1 == "none") return;
						if (
							lib.skill.dcyaoyi.getZhuanhuanji(target, true) ==
							state1
						)
							return false;
					},
	...
	dcshoutan: {
		...
		filter: function (event, player) {
			...
			return (
				player.countCards("h") > 0 && !player.getStat("skill").dcshoutan
			);
		},
		selectCard: function () {
			...
			if (gains.length) {
				...
				player
					...
					.set("ai", function (button) {
						...
						var getn = function (card) {
							return (
								...
								ui.selected.buttons.filter(
									(button) => button.link.name == card.name
								).length
							);
						};
						...
						if (
							card.name == "tao" &&
							getn(card) >= player.getDamagedHp()
						)
							return 0;
						if (
							card.name == "sha" &&
							getn(card) >= player.getCardUsable("sha")
						)
							return 0;
						return val;
					});
		...
		chooseButton: {
			dialog: function (event, player) {
				...
				dialog.add([
					[
						[
							"gain",
							"令一名体力值不大于你的其他角色交给你一张手牌",
						],
						[
							"sha",
							"视为对一名手牌数不大于你的其他角色使用一张【杀】",
						],
					],
					"textbutton",
				]);
			...
			check: function (button) {
				...
				if (
					...
					game.hasPlayer(function (current) {
						return (
							lib.skill.dcfengyan_gain.filterTarget(
								null,
								player,
								current
							) &&
				...
				if (
					...
					game.hasPlayer(function (current) {
						return (
							lib.skill.dcfengyan_sha.filterTarget(
								null,
								player,
								current
							) &&
			...
			refuse: {
				...
				filter: function (event, player) {
					var storage1 = event.player.getStorage("dcfudao_deadmark"),
						storage2 = player.getStorage("dcfudao_effect");
					return (
						storage1.some((i) => storage2.includes(i)) &&
	...
	dcyishu: {
		...
		filter: function (event, player) {
			...
			return (
				!player.isPhaseUsing() &&
				player.hasSkill("dchuishu", null, null, false)
			);
		},
		content: function () {
			"step 0";
		...
		content: function () {
			...
			if (_status.characterlist) {
				...
				for (var i = 0; i < _status.characterlist.length; i++) {
					...
					if (
						...
						(lib.character[name][0] == "female" ||
							lib.character[name][0] == "double")
					)
						list.push(name);
				}
				...
				list = get.charactersOL(function (i) {
					return (
						...
						(lib.character[i][0] != "female" &&
							lib.character[i][0] != "double")
					);
				});
			} else {
			...
			var chooseButton = function (list, skills) {
				...
				for (var i = 0; i < skills.length; i++) {
					...
					table.appendChild(td);
					td.innerHTML =
						"<span>" + get.translation(skills[i]) + "</span>";
					td.addEventListener(
						lib.config.touchscreen ? "touchend" : "click",
						function () {
							if (_status.dragged) return;
			...
			if (result.bool && player.canMoveCard()) {
				...
				for (var i = 0; i < players.length; i++) {
					for (var j = i + 1; j < players.length; j++) {
						if (
							!players[i].inRange(players[j]) ||
							!players[i].inRangeOf(players[j])
						) {
							goon = false;
							break;
						}
		...
		content: function () {
			if (!player.hasEquipableSlot(5) || player.getEquip("dagongche")) {
				...
				if (evt && evt.phaseList)
					evt.phaseList.splice(evt.num + 1, 0, "phaseUse|dcwanglu");
			} else {
	...
	dcgeyuan: {
		...
		filter: function (event, player) {
			...
			for (var i of cards) {
				if (
					lib.skill.dcgeyuan.filterNumber(
						player,
						get.number(i, false)
					)
				)
		...
		content: function () {
			...
			if (card) {
				...
				if (
					player.getStorage("dcgeyuan").length >
					player.getStorage("dcgeyuan_homura").length
				) {
		...
		intro: {
			...
			mark: function (dialog, storage, player) {
				...
				for (var i = 0; i < list.length; i++) {
					...
					core.appendChild(td);
					td.style.left =
						centerX + radius * Math.sin(radian * i) + "px";
					td.style.top =
						centerY - radius * Math.cos(radian * i) + "px";
				}
	...
	caiyi: {
		...
		filter: function (event, player) {
			if (player.storage.caiyi_info) {
				if (
					player.storage.caiyi_info[player.storage.caiyi ? 1 : 0]
						.length >= 4
				)
					return false;
			}
		...
		content: function () {
			...
			("step 3");
			var index2 = ["选项一", "选项二", "选项三", "选项四"].indexOf(
				result.control
			);
			player.storage.caiyi_info[event.index].push(index2);
			if (index2 == 3) {
				var list = player.storage.caiyi_info[event.index].filter(
					function (i) {
						return (
							i != 3 &&
							lib.skill.caiyi.filterx[event.index][i](target)
	...
	dcjinggong: {
		...
		subSkill: {
			base: {
				...
				filter: function (event, player) {
					return (
						event.skill == "dcjinggong" && event.targets.length > 0
					);
				},
				content: function () {
					trigger.baseDamage = Math.min(
						5,
						get.distance(player, trigger.targets[0])
					);
				},
			},
		},
	...
	dcxiaojuan: {
		...
		check: function (event, player) {
			...
			return (
				Math.floor(target.countCards("h") / 2) >=
				Math.floor(player.countCards("h") / 2)
			);
		},
		content: function () {
			"step 0";
			...
			give: {
				...
				content: function () {
					...
					player
						...
						.set("ai", function (target) {
							return (
								get.attitude(_status.event.player, target) + 100
							);
						});
					("step 1");
					if (result.bool) {
	...
	shawu: {
		...
		content: function () {
			...
			player
				...
				.set(
					"prompt2",
					"弃置两张手牌，或移去一枚“沙”并摸两张牌，然后对该角色造成1点伤害"
				)
				.set("ai", function () {
					var player = _status.event.player,
			...
			if (result.control == "cancel2") {
				...
				player.chooseToDiscard("h", true, 2).logSkill = [
					"shawu",
					target,
				];
			}
	...
	qianlong: {
		...
		content: function () {
			...
			next.set("processAI", function (list) {
				...
				if (
					!player.storage.juetao &&
					player.hasSkill("juetao") &&
					player.hasSha()
				) {
					let gain,
						bottom,
	...
	juetao: {
		...
		content: function () {
			"step 0";
			player
				.chooseTarget(get.prompt2("juetao"), lib.filter.notMe)
				.set("ai", function (target) {
					...
					if (
						...
						target.getEquip("renwang")
					)
						return 0.3 * att;
					if (
						target.getEquip("rewrite_tengjia") ||
						target.getEquip("rewrite_renwang")
					)
						return 0.2 * att;
					if (
						target.hasSkillTag(
	...
	jinjian: {
		...
		filter: function (event, player) {
			return (
				!event.jinjian_source2 && !player.hasSkill("jinjian_source2")
			);
		},
		...
		subSkill: {
			player: {
				...
				filter: function (event, player) {
					return (
						!event.jinjian_player2 &&
						!player.hasSkill("jinjian_player2")
					);
				},
				prompt2: "令即将受到的伤害-1",
				content: function () {
		...
		content: function () {
			...
			player
				...
				.set("ai", function (target) {
					...
					if (
						target.isLinked() &&
						get.effect(target, { name: "tiesuo" }, player, player) >
							0
					)
						return 1.6 * att;
					if (
						ui.selected.targets.length >=
						Math.sqrt(1 + player.maxHp)
					)
						return 0;
	...
	tianyun: {
		...
		filter: function (event, player) {
			return (
				event.player.getSeatNum() == game.roundNumber &&
	...
	xinzhoufu: {
		...
		filterTarget: function (card, player, target) {
			return (
				target != player && !target.getExpansions("xinzhoufu2").length
			);
		},
		check: function (card) {
			return 6 - get.value(card);
		...
		subSkill: {
			judge: {
				...
				filter: function (event, player) {
					return (
						!event.directresult &&
						player.getExpansions("xinzhoufu2").length
					);
				},
				content: function () {
		...
		filter: function (event, player) {
			return (
				...
				!player.hasHistory("gain", function (evt) {
					...
					return (
						evtx &&
						evtx.name == "xinyingbing" &&
						evtx._trigger.target == event.target
					);
				})
			);
		},
	...
	syxiongyi: {
		...
		async content(event, trigger, player) {
			...
			if (_status.characterlist.includes("xushi")) {
				if (
					player.name2 &&
					get.character(player.name2)[3].includes("syxiongyi")
				) {
	...
	yuqi: {
		...
		filter: function (event, player) {
			...
			return (
				event.player.isIn() &&
				get.distance(player, event.player) <= list[0]
			);
		},
		logTarget: "player",
		content: function () {
				...
				[
					[
						"交给" +
							...
							(event.list[2] > 1
								? "<br>至多" +
								  get.cnNumber(event.list[2]) +
								  "张"
								: "") +
							"</div>",
					],
					[
						'交给自己<div class="text center">至多' +
							get.cnNumber(event.list[3]) +
							"张</div>",
					],
				],
			]);
			...
			next.set("processAI", function (list) {
				var cards = list[0][1].slice(0).sort(function (a, b) {
					...
					target = _status.event.getTrigger().player;
				var info = lib.skill.yuqi.getInfo(_status.event.player);
				var cards1 = cards.splice(
					0,
					Math.min(info[3], cards.length - 1)
				);
				var card2;
		...
		init(player, skill) {
			...
			player.addTip(
				skill,
				get.translation(skill) + " " + list.slice().join(" ")
			);
		},
				...
				.set("ai", function () {
					...
					if (
						...
						game.hasPlayer(function (current) {
							return (
								current != player &&
								get.distance(player, current) > info[0]
							);
						})
					)
						return 0;
	...
	mengqing: {
		...
		filter: function (event, player) {
			return (
				game.countPlayer((current) => current.isDamaged()) > player.hp
			);
		},
		juexingji: true,
		skillAnimation: true,
			...
			while (selected.length < num) {
				...
				if (result.bool) {
					...
					game.log(
						...
						"#g【玉陨】",
						"的",
						"#y选项" + get.cnNumber(1 + choice, true)
					);
					switch (choice) {
						case 0:
							await player.draw(2);
			...
			sha: {
				mod: {
					cardUsableTarget: function (card, player, target) {
						if (
							card.name == "sha" &&
							player.getStorage("yuyun_sha").includes(target)
						)
							return Infinity;
					},
					targetInRange: function (card, player, target) {
			...
			if (
				...
				player
					.chooseTarget(
						...
						function (card, player, target) {
							return (
								target.countDiscardableCards(player, "j") > 0
							);
						}
					)
					.set("ai", function (target) {
	...
	yaner: {
		...
		filter: function (event, player) {
			...
			if (
				...
				current == player ||
				!current.isIn() ||
				!current.isPhaseUsing()
			)
				return false;
			var evt = event.getl(current);
			return (
				evt && evt.hs && evt.hs.length && current.countCards("h") == 0
			);
		},
		usable: 1,
		logTarget: function () {
			...
			player
				...
				.set("ai", function (target) {
					var player = _status.event.player,
						att = get.attitude(player, target);
					if (
						target.hasSkill("maixie") ||
						target.hasSkill("maixie_defend")
					)
						att /= 3;
					if (target != player)
						att /= Math.pow(
	...
	zhuihuan2: {
		...
		content: function () {
			if (player.hp < trigger.source.hp) trigger.source.damage();
				...
				trigger.source.discard(
					trigger.source.getCards("h").randomGet()
				);
		},
		mark: true,
		intro: {
			...
			player
				...
				.set("ai", function (target) {
					...
					var getv = function (list, target) {
						...
						val =
							getv(hs, target) -
							getv(player.storage.xingzuo2, target);
					if (num < 3)
		...
		chooseButton: {
			...
			prompt: function (links, player) {
				return (
					"将" +
					get.translation(
						player.getCards("h", { color: "black" })[0]
					) +
					"当做" +
					get.translation(links[0][2]) +
					"使用"
		...
		content: function () {
			...
			if (
				...
				(function () {
					for (var card of cards) {
						if (
							get.suit(card, target) == "heart" ||
							get.name(card, target) == "jiu"
						)
							return false;
					}
					return true;
	...
	pianchong: {
		...
		content: function () {
			...
			player.popup(
				result.control,
				result.control == "red" ? "fire" : "thunder"
			);
	...
	pianchong2: {
		...
		filter: function (event, player) {
			...
			for (var i of evt.cards2) {
				if (
					player
						.getStorage("pianchong2")
						.includes(get.color(i, player))
				)
					return true;
			}
			return false;
		...
		content: function () {
			...
			while (cardsx.length) {
				...
				var card = get.cardPile2(function (card) {
					return (
						!cards.includes(card) &&
						get.color(card, false) != get.color(precard, false)
					);
				});
				if (card) cards.push(card);
				else break;
			...
			check: function (button) {
				...
				switch (button.link) {
					case 0: {
						...
						return (
							(Math.min(target.hp, player.maxHp) - player.hp) * 2
						);
					}
					case 1: {
						var target = game.findPlayer(function (current) {
		...
		backups: [
			{
				...
				ai: {
					...
					result: {
						player: function (player, target) {
							return (
								Math.min(target.hp, player.maxHp) - player.hp
							);
						},
					},
				},
	...
	rezhongjian: {
		...
		filter: function (event, player) {
			if (
				player.getStat().skill.rezhongjian &&
				!player.hasSkill("recaishi2")
			)
				return false;
			return game.hasPlayer(function (current) {
				return lib.skill.rezhongjian.filterTarget(
					null,
					player,
					current
				);
		...
		content: function () {
			...
			player
				...
				.set(
					"prompt",
					"忠鉴：为" + get.translation(target) + "选择获得一项效果"
				)
				.set("choiceList", [
		...
		ai: {
			...
			result: {
				player: function (player, target) {
					if (get.attitude(player, target) == 0) return false;
					var sgn = get.sgn(
						(get.realAttitude || get.attitude)(player, target)
					);
					if (
						game.countPlayer(function (current) {
							return (
	...
	rezhongjian2: {
		...
		filter: function (event, player, name) {
			...
			var logTarget =
				name == "damageSource" ? event.source : event.player;
			return (
				logTarget &&
				logTarget.isIn() &&
				player.storage.rezhongjian2[num].includes(logTarget)
			);
		},
		...
		content: function () {
			...
			var target =
				event.triggername == "damageSource"
					? trigger.source
					: trigger.player;
				...
				if (player == game.me || player.isUnderControl()) {
					...
					dialog.addText(
						get.translation(player) +
							"共选择了" +
							get.cnNumber(
								storage[0].length + storage[1].length
							) +
							"人"
					);
				}
			...
			return (
				...
				player.getHistory("gain", function (evt) {
					return (
						...
						evt.cards.length ==
							2 + get.sgn(event.player.hp - player.hp)
					);
				}).length < num
			);
		},
			...
			game.getGlobalHistory("cardMove", function (evt) {
				...
				for (var i = 0; i < evt.cards.length; i++) {
					...
					list.remove(
						get.suit(
							card,
							evt.cards2 && evt.cards2.includes(card)
								? evt.player
								: false
						)
					);
	...
	lvli: {
		...
		chooseButton: {
			dialog: function (event, player) {
				...
				return ui.create.dialog(
					event.lvli6 ? get.prompt("lvli") : "膂力",
					[list, "vcard"]
				);
			},
			filter: function (button, player) {
	...
	spzhuilie: {
		...
		async content(event, trigger, player) {
			const next = player.judge(function (card) {
				...
				return ["equip1", "equip4", "equip3", "equip6"].includes(type)
					? 6
					: -6;
			});
	...
	souying: {
		...
		direct: true,
		filter: function (event, player, name) {
			if (!player.countCards("he") || player.hasSkill("souying2"))
				return false;
			if (
				!event.targets ||
				event.targets.length != 1 ||
	...
	hmxili: {
		...
		content: function () {
			...
			player
				...
				.set("logSkill", ["hmxili", trigger.player]).ai = function (
				card
			) {
				return 9 - get.value(card);
			};
	...
	wlcuorui: {
		...
		async content(event, trigger, player) {
			...
			const str = ["identity", "guozhan"].includes(get.mode())
				? "另一名其他角色"
				: "对手";
			...
			if (
				game.hasPlayer(function (current) {
					return (
						filter(current) &&
						current.countCards("e", { color: get.color(card) })
					);
				})
			)
				list.push("弃置装备");
				...
				result2 = await player
					...
					.set(
						...
						"挫锐：展示" +
							...
							str +
							"装备区内至多两张" +
							get.translation(get.color(card)) +
							"牌"
					)
					.set(
						"resultx",
			...
			if (result2.control == "展示手牌") {
				...
				const result3 = await player
					...
					.set("ai", (button) => {
						let color =
							get.color(button.link) == _status.event.color;
			...
			while (i++ < 2) {
				var card = get.cardPile(
					function (card) {
						...
						return (
							list.length == 0 ||
							get.subtype(card) != get.subtype(list[0])
						);
					},
					false,
					"random"
	...
	pytianjiang_move: {
		...
		filterTarget: function (event, player, target) {
			return (
				target != player && target.canEquip(ui.selected.cards[0], true)
			);
		},
		prepare: "give",
		discard: false,
		...
		content: function () {
			...
			("step 1");
			if (
				cards[0].name.indexOf("pyzhuren_") == 0 &&
				!player.getCards("e").includes(cards[0])
			)
				player.draw(2);
		},
		ai: {
	...
	pyzhuren: {
		...
		check: function (card) {
			var player = _status.event.player;
			var name =
				"pyzhuren_" + card[card.name == "shandian" ? "name" : "suit"];
			if (
				!lib.card[name] ||
				(_status.pyzhuren && _status.pyzhuren[name])
			) {
		...
		content: function () {
			...
			if (
				num > 12 ||
				cards[0].name == "shandian" ||
				get.isLuckyStar(player)
			)
				rand = 1;
			var name =
				"pyzhuren_" +
				cards[0][cards[0].name == "shandian" ? "name" : "suit"];
			if (
				!lib.card[name] ||
				_status.pyzhuren[name] ||
				Math.random() > rand
			) {
				player.popup("杯具");
	...
	pyzhuren_diamond: {
		...
		content: function () {
			...
			next.ai = function (card) {
				if (_status.event.goon)
					return 30 / (1 + _status.event.target.hp) - get.value(card);
				return -1;
			};
			next.set(
				"goon",
	...
	pyzhuren_club: {
		...
		subSkill: {
			lose: {
				...
				filter: (event, player) => {
					return (
						player.isDamaged() && !player.hasSkillTag("unequip2")
					);
				},
				getIndex(event, player) {
					const evt = event.getl(player);
	...
	tuiyan: {
		...
		content: function () {
			...
			game.log(
				player,
				"观看了牌堆顶的" + get.cnNumber(cards.length) + "张牌"
			);
			...
			for (var i = 0; i < Math.min(trigger.num, list.length); i++) {
				var card = get.cardPile(function (cardx) {
					...
					if (cardx.name != list[Math.min(i, list.length - 1)][2])
						return false;
					if (
						get.nature(cardx, false) !=
						list[Math.min(i, list.length - 1)][3]
					)
						return false;
					return true;
				});
	...
	mingjie: {
		...
		check: function () {
			return (
				ui.cardPile.hasChildNodes() &&
				get.color(ui.cardPile.firstChild) != "black"
			);
		},
		content: function () {
			"step 0";
			...
			if (Array.isArray(result)) {
				...
				if (get.color(result) != "red") {
					...
					player.chooseBool("是否继续发动【命戒】？").ai =
						function () {
