...
const skills = {
	...
	zc26_bagua_skill: {
		...
		async content(event, trigger, player) {
			...
			const result = await player.judge("zc26_bagua", card => (get.color(card) === "red" ? 1.5 : -0.5)).set("judge2", result => result.bool).forResult();
			if (result.bool > 0) {
				trigger.untrigger();
		...
		subSkill: {
			effect: {
				...
				content() {
					...
					if (bodies.length == 1) {
						event._result = { bool: true, targets: bodies };
					} else {
						player
	...
	changandajian_destroy: {
		...
		content() {
			...
			for (var i of evt.es) {
				...
				if (i.name == "changandajian_equip2") {
					recover++;
				}
			}
			if (recover > 0) {
	...
	tianren: {
		...
		content() {
			...
			for (var i of trigger.cards) {
				var owner = false;
				if (trigger.hs && trigger.hs.includes(i)) {
	...
	pingxiang: {
		...
		content() {
			...
			"step 2";
			if (result.bool && event.num < 9) {
				event.goto(1);
			} else {
				player.removeSkills("jiufa");
			}
			"step 3";
	...
	kuangfeng: {
		...
		async cost(event, trigger, player) {
			const { bool, targets, links: cost_data } = await player.chooseButtonTarget({
				createDialog: [get.prompt2(event.skill), player.getExpansions("qixing")],
				selectButton: 1,
				filterTarget: true,
				ai1(button) {
	...
	drlt_duorui: {
		...
		content() {
			...
			var func = function (skill) {
				var info = get.info(skill);
				if (!info || info.charlotte || info.persevereSkill || info.hiddenSkill || info.zhuSkill || info.juexingji || info.limited || info.dutySkill || (info.unique && !info.gainable) || lib.skill.drlt_duorui.bannedList.includes(skill)) {
					return false;
				}
				return true;
			};
			...
			if (event.skills.length > 0) {
				player.chooseButton(["请选择要获得的技能", [event.skills, "skill"]], true).set("ai", function () {
					return Math.random();
				});
			} else {
				event.finish();
