...
const dynamicTranslates = {
	...
	pejiechu(player) {
		const bool = player.getStorage("pejiechu", false);
		let yang = "阳：出牌阶段，你可以视为使用一张【顺手牵羊】，此牌结算完成后目标角色可以对你使用一张【杀】",
			yin = "阴：你成为【杀】的目标时，可以弃置一张手牌改变此【杀】的花色和属性";
		if (bool) {
			yin = `<span class='bluetext'>${yin}</span>`;
		}
		else {
	...
	jdjuqi(player) {
		if (player.storage.jdjuqi) {
			return '转换技。阴：准备阶段，你摸三张牌；其他角色的准备阶段，其可以展示并交给你一张黑色手牌。<span class="bluetext">阳：准备阶段，你令你本回合使用牌无次数限制且造成的伤害+1；其他角色的准备阶段，其可以展示并交给你一张红色手牌。</span>';
		}
	...
	tylongnu(player) {
		...
		if (player.hasSkill("tylongnu_yin")) {
			yin = "<span class='legendtext'>" + yin + "</span>";
		} else if (!player.storage.tylongnu && !player.hasSkill("tylongnu_yang")) {
			yin = "<span class='bluetext'>" + yin + "</span>";
		}
		str += yin;
		let yang = "阳：减少1点体力上限，然后此阶段内你可以将锦囊牌当无次数限制的雷【杀】使用或打出。";
		if (player.hasSkill("tylongnu_yang")) {
			yang = "<span class='legendtext'>" + yang + "</span>";
		} else if (player.storage.tylongnu && !player.hasSkill("tylongnu_yin")) {
			yang = "<span class='firetext'>" + yang + "</span>";
		}
		str += yang;
		return str;
	...
	yyyanggu(player) {
		if (player.storage.yyyanggu) {
			return '转换技。阴：当你受到伤害后，你可以回复1点体力；<span class="bluetext">阳：你可以将一张手牌当作【声东击西】使用</span>。';
		}
		return '转换技。<span class="bluetext">阴：当你受到伤害后，你可以回复1点体力</span>；阳：你可以将一张手牌当作【声东击西】使用。';
	},
	hm_shice(player) {
		if (!player.storage.hm_shice) {
			return '转换技，<span class="bluetext">阴：当你受到属性伤害时，若你的技能数不大于伤害来源，你可以防止此伤害并视为使用一张【火攻】</span>；阳：当你不因此技能使用牌指定唯一目标后，你可以令其弃置装备区任意张牌，然后此牌额外结算X次（X为其装备区的牌数）。';
		}
