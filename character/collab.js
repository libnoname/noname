...
const skills = {
  ...
  dcbeijin: {
    ...
    ai: {
      order: 20,
      result: { player: (player2) => 1 - (player2.hasSkill("dcbeijin_buff") && player2.hasCard((card2) => card2.hasGaintag("dcbeijin_effect"), "h")) }
    },
    locked: false,
    mod: {
      ...
      effect: {
        ...
        mod: {
          ...
          aiOrder(player2, card2, num) {
            if (player2.hasSkill("dcbeijin_buff") && typeof card2 === "object") {
              if (get.itemtype(card2) === "card" || card2.cards?.some((card3) => card3.hasGaintag("dcbeijin_effect"))) {
                return num + 100;
              }
