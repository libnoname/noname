...
const skills = {
  ...
  oljuhun: {
    ...
    getCombo(cards2) {
      if (cards2.length >= 2 && cards2.length <= 4 && cards2.every((card) => lib.skill.oljuhun.getRank(card) === lib.skill.oljuhun.getRank(cards2[0]))) {
        return [null, null, "pair", "triple", "bomb"][cards2.length];
      }
      const numbers = cards2.map(lib.skill.oljuhun.getRank).sort((a, b) => a - b);
    ...
    canCompleteCombo(selected, available) {
      ...
      const selectedCounts = /* @__PURE__ */ new Map();
      ...
      for (const [number, count] of counts) {
        for (let size = 2; size <= Math.min(4, count); size++) {
          templates.push(/* @__PURE__ */ new Map([[number, size]]));
        }
      }
      for (let start = 3; start <= 10; start++) {
        const straight = /* @__PURE__ */ new Map();
    ...
    paintCards(cards2, owner, restoreAtEnd) {
      ...
      const data = cards2.map((card) => [card, oljuhunSuitMap[get.suit(card, false)] || get.suit(card, false), get.number(card)]);
      game.broadcastAll(
        (data2, owner2, restoreAtEnd2) => {
          for (const node of [owner2.node.handcards1, owner2.node.handcards2]) {
            node.cardMod ??= {};
            node.cardMod.oljuhun = (card) => card.classList.contains("oljuhun") ? ["扑克牌", "仅可用于牛头马面的拘魂组合；离开手牌区时恢复原牌。"] : void 0;
          }
          const symbols = { heart: "♥", diamond: "♦", spade: "♠", club: "♣" };
          const displayNumbers = { 1: "A", 11: "J", 12: "Q", 13: "K" };
          for (const [card, suit, number] of data2) {
            card._oljuhunOwner = owner2;
            card._oljuhunEnd = restoreAtEnd2;
          ...
          ui.oljuhunSortControl = ui.create.control("理牌", () => {
            ...
            game.broadcastAll((player) => {
              ...
              for (const hand of [player.node.handcards1, player.node.handcards2]) {
                ...
                let index = 0;
                for (const card of nodes.map((card2) => isSortable(card2) ? sorted[index++] : card2)) {
    ...
    subSkill: {
      play: {
        ...
        filterCard(card, player) {
          ...
          const selected = [.../* @__PURE__ */ new Set([...ui.selected.cards || [], card])];
        ...
        async content(event, trigger, player) {
          ...
          if (type === "pair") {
            const list = get.inpileVCardList((info) => info[0] === "basic" && player.hasUseTarget({ name: info[2], nature: info[3], isCard: true }, void 0, false));
            if (list.length) {
            ...
            const targets = [.../* @__PURE__ */ new Set([player.getNext(), player.getPrevious()])].filter((current) => current !== player && current.hasCards("he")).sortBySeat();
            if (targets.length) {
            ...
            const result = await player.chooseTarget({
              ...
              filterTarget(card, player2, target) {
                return target.hasCards("h", (card2) => !lib.skill.oljuhun.isPoker(card2));
              },
              ai(target) {
          ...
          if (!player.getStorage("oldianbu_used").includes(type)) {
            ...
            await useEvent;
            const gains = player.getHistory("gain", (event2) => {
              let current = event2;
              while (current) {
                if (current === useEvent) {
                  return true;
            ...
            game.broadcastAll(
              (cards3) => {
                ...
                for (const card of cards3) {
                  ...
                  const nodes = /* @__PURE__ */ new Set([card, card.clone]);
                  if (id !== null) {
