...
const skills = {
	...
	dcmurui: {
		...
		async content(event, trigger, player) {
			...
			player
				.when({ player: ["useCardAfter", "dcmurui"] })
			...
			backup(result, player) {
				return {
					...
					async content(event, trigger, player) {
						...
						await player.draw(num);
						if (num >= 1) {
							await player.removeSkills("dctanluan");
						}
						if (num >= 2 && player.countCards("h")) {
							await player.chooseToDiscard("h", true);
						}
		...
		choices: {
			...
			recast_self: {
				...
				async content(player, num = 1) {
					...
					const { bool, cards } = await player.chooseCard("重铸" + num + "张牌", "he", num, lib.filter.cardRecastable, true).set("ai", lib.skill.zhiheng.check).forResult();
					if (bool) {
	...
	dcdingji: {
		...
		async content(event, trigger, player) {
			let result;
			result = await player.chooseTarget(get.prompt2("dcdingji")).set("ai", target => {
		...
		async content(event, trigger, player) {
			...
			while (cards.length) {
				...
				if (cards.length == 1) {
					...
					result = await player.chooseCardButton("良秀：请选择要分配的牌", cards, [1, cards.length], true).set("ai", button => {
						if (!ui.selected.buttons.length) {
				...
				return {
					...
					async precontent(event, trigger, player) {
						...
						if (!player.countMark("dcshizong")) {
							player.when({ global: "phaseAfter" }).then(() => delete player.storage.dcshizong);
						}
	...
	dcjianshu: {
		...
		content() {
			...
			if (result.bool) {
				...
				if (cards.length > 0) {
					targets[0].discard(cards.randomGet());
				}
				targets[1].loseHp();
			} else if (result.tie) {
				targets[0].loseHp();
				...
				if (cards.length > 0) {
					targets[1].discard(cards.randomGet());
				}
				targets[0].loseHp();
			}
		},
		...
		content() {
			...
			if (result.bool && target.isIn()) {
				...
				if (!hs.length) {
					...
					player.chooseCard("he", true, "选择交给" + get.translation(target) + get.cnNumber(num) + "张牌", "（已得到牌的点数和：" + numx + "）", num);
				}
			} else {
				event.finish();
			}
