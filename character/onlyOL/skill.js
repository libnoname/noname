...
const skills = {
		...
		subSkill: {
			buff: {
				...
				async content(event, trigger, player) {
					...
					const storage = player.storage.olsbchoulie_buff;
					const vcard = new lib.element.VCard({ name: "sha" });
					if (player.canUse(vcard, storage[0], false))
						await player.useCard(vcard, storage, false);
				},
			},
			use: {
				charlotte: true,
		...
		subSkill: {
			buff: {
				...
				async content(event, trigger, player) {
					...
					for (const i of list) {
						await lib.skill.olsbnilan.choice[i].apply(
							this,
							arguments
						);
					}
	...
	olsbjueya: {
		...
		hiddenCard: function (player, name) {
			...
			if (
				player.storage.olsbjueya &&
				!player.storage.olsbjueya.includes(name)
			)
				return !player.countCards("h");
			return false;
		},
		...
		chooseButton: {
			dialog: function (event, player) {
				const list = [];
				const cardname = lib.inpile.filter(
					(c) => get.type(c) == "basic"
				);
				const storage = player.storage.olsbjueya;
		...
		async content(event, trigger, player) {
			...
			game.broadcastAll(function (player) {
				...
				if (!player.node.jiu && lib.config.jiu_effect) {
					player.node.jiu = ui.create.div(
						".playerjiu",
						player.node.avatar
					);
					player.node.jiu2 = ui.create.div(
						".playerjiu",
						player.node.avatar2
					);
				}
			...
			buff: {
				...
				filter(event, player) {
					if (event.player != player.storage.olsbxieyong_jiu[0])
						return false;
					return (
						!event.targets || !event.targets.includes(event.player)
					);
				},
				direct: true,
				async content(event, trigger, player) {
	...
	olsblixian: {
		mod: {
			cardEnabled(card, player, result) {
				const evt = get.event();
				if (
					get.itemtype(card) == "vcard" &&
					Array.isArray(card.cards)
				) {
					if (
						card.cards.some((c) => c.hasGaintag("olsblixian")) &&
						!["olsblixian_sha", "olsblixian_shan"].includes(
							evt.skill
						)
					)
						return false;
				...
				if (
					card.hasGaintag("olsblixian") &&
					!["olsblixian_sha", "olsblixian_shan"].includes(evt.skill)
				)
					return false;
			},
		},
			...
			sha: {
				...
				viewAsFilter: function (player) {
					if (
						!player.countCards(
							"hs",
							lib.skill["olsblixian_sha"].filterCard
						)
					)
						return false;
				},
	...
	olsbxutu: {
		...
		filter(event, player) {
			return (
				...
				!player.getExpansions("olsbxutu").length
			);
		},
		async content(event, trigger, player) {
						...
						.set("filterButton", function (button) {
							if (ui.selected.buttons.length)
								return (
									...
									get.position(ui.selected.buttons[0].link)
								);
							return true;
						})
						.set("cards1", xs)
				...
				async content(event, trigger, player) {
					...
					if (bool) {
						const { result } = await player
							...
							.set(
								"prompt",
								`将${get.translation(xs)}交给一名其他角色`
							);
						await result.targets[0].gain(xs, "draw");
						await player
							.addToExpansion(get.cards(3), player, "giveAuto")
							.gaintag.add("olsbxutu");
					}
				},
			},
		},
			...
			if (num >= 0) {
				...
				if (
					...
					if (result?.bool && result.targets?.length) {
						...
						trigger
							.getParent()
							.triggeredTargets1.addArray(result.targets);
						game.log(
							result.targets,
							"成为了",
							trigger.card,
	...
	olsbbaojing: {
		...
		async content(event, trigger, player) {
			...
			let index = await player
				...
				.set(
					"prompt",
					"令" + get.translation(target) + "的攻击范围+1或-1"
				)
				.forResult("index");
		...
		subSkill: {
			effect: {
				...
				intro: {
					markcount(storage = {}) {
						const num = Object.keys(storage).reduce(
							(sum, i) => sum + storage[i],
							0
						);
						if (!num) return num;
				...
				mod: {
					attackRange(player, num) {
						const storage =
							player.storage["olsbbaojing_effect"] || {};
						const numx = Object.keys(storage).reduce(
							(sum, i) => sum + storage[i],
							0
						);
						if (!numx) return;
				...
				filter(event, player) {
					return player.storage?.["olsbbaojing_effect"]?.[
				...
				content() {
					...
					player[
						Object.keys(player.storage[event.name]).length
							? "markSkill"
							: "removeSkill"
					](event.name);
				},
	...
	olsbjigu: {
		...
		filter(event, player) {
			...
			if (event.name == "cardsDiscard") {
				...
				return (
					evt2.name == "useCard" &&
					evt2.player != event.getParent("phaseUse")?.player
				);
			}
			...
			if (trigger.name == "cardsDiscard") {
				...
				next.set("processAI", (list) => {
					let player = get.player(),
						...
						cards2 = cards.splice(
							0,
							player.getExpansions("olsbjigu").length
						);
					return [cards2, cards];
				});
				const {
			...
			if (trigger.name == "phaseJieshu") {
				...
				next.set("filterButton", (button) => {
					const { link } = button,
						player = get.player();
					return (
						(link == "lose" && player.maxHp > 0) ||
						(link == "discard" &&
							player.countExpansions("olsbjigu") > 1)
					);
				});
				next.set("ai", (button) => {
					const { link } = button,
					...
					const cards =
						player.countExpansions("olsbjigu") <= 2
							...
							: await player
									...
									.set(
										"ai",
										(button) => 6 - get.value(button.link)
									)
									.forResultLinks();
	...
	olguanbian: {
		...
		async content(event, trigger, player) {
			if (
				...
				["olxiongni", "olfengshang"].includes(trigger.name)
			)
		...
		intro: {
			content:
				"<li>手牌上限+#<br><li>计算与其他角色的距离+#<br><li>其他角色计算与你的距离+#",
		},
	},
	olxiongni: {
		audio: 6,
		...
		async cost(event, trigger, player) {
			...
			event.result = await player
				...
				.set("logSkill", [
					skillName,
					get.info(skillName).logTarget(trigger, player),
				])
				.forResult();
		},
		popup: false,
		logTarget: (event, player) =>
			game.filterPlayer((target) => target != player).sortBySeat(),
		async content(event, trigger, player) {
			player.changeSkin(
				{ characterName: "ol_sb_dongzhuo" },
				"ol_sb_dongzhuo_shadow1"
			);
			const suit = get.suit(event.cards[0]);
			for (const target of event.targets) {
				const bool = await target
					.chooseToDiscard(
						`弃置一张${get.translation(
							suit
						)}牌，否则${get.translation(player)}对你造成1点伤害`,
						"he",
						(card, player) => {
	...
	olfengshang: {
		...
		filter(event, player) {
			const cards =
				event.name == "chooseToUse"
					? event.olfengshang_cards || []
					: get.info("olfengshang").getCards(player);
			if (
				!lib.suit.some(
					(suit) =>
						cards.filter((card) => get.suit(card) == suit).length >
						1
				)
			)
				return false;
			return !player.hasSkill(
				"olfengshang_" +
					(event.name === "chooseToUse" ? "used" : "round"),
				null,
				null,
				false
			);
		...
		onChooseToUse(event) {
			if (!game.online && !event.olfengshang_cards) {
				event.set(
					"olfengshang_cards",
					get.info("olfengshang").getCards(event.player)
				);
			}
		},
		async content(event, trigger, player) {
			player.addTempSkill(
				event.name + (trigger ? "_round" : "_used"),
				trigger ? "phaseAfter" : "phaseUseAfter"
			);
			...
			const cards = !trigger
				? event.getParent(2).olfengshang_cards
				: get.info(event.name).getCards(player);
			let result;
			while (Object.keys(given_map).length < 2 && cards.length) {
							...
							if (!Object.values(map).flat().length)
								return (
									get
										...
										.filter(
											(card) =>
												get.suit(card) == get.suit(link)
										).length > 1
								);
							return (
								get.suit(link) ==
								get.suit(Object.values(map).flat()[0])
							);
						})
						.set("ai", (button) => {
				...
				result = await player
					.chooseTarget(
						"选择获得" + get.translation(toGive) + "的角色",
						true,
						(card, player, target) => {
							return !get.event().getParent().given_map[
								target.playerid
							];
						}
					)
			...
			if (
				!player.hasHistory(
					"gain",
					(evt) => evt.getParent(2) == event
				) &&
				player.hasUseTarget({ name: "jiu", isCard: true }, true, false)
			) {
	...
	olzhibin: {
		...
		getNum(player) {
			...
			game.countPlayer2((current) => {
				if (current != player && current.group == "qun") {
					num += current.getAllHistory(
						"useCard",
						(evt) => get.color(evt.card) == "black"
					).length;
				}
			});
			return num;
		...
		filter(event, player) {
			...
			return (
				get.info("olzhibin").filterx(player, num) ||
				get.info("olzhibin").filtery(player, num) ||
				get.info("olzhibin").filterz(player, num)
			);
		},
		filterx(player, num) {
			return (
			...
			return (
				...
				!game.getAllGlobalHistory(
					...
					(evt) =>
						...
						evt.player == player &&
						evt.getParent().name == "olzhibin" &&
						evt.addSkill.includes("benghuai")
				).length
			);
		},
		zhuSkill: true,
	...
	olsbbojue: {
		...
		async content(event, trigger, player) {
					...
					humans.map((current) => {
						return new Promise(async (resolve, reject) => {
							if (current.isOnline()) {
								...
								const next = lib.skill.olsbbojue.chooseCard(
									current,
									targets,
									eventId
								);
			...
			if (locals.length > 0) {
				for (const current of locals) {
					const result = await lib.skill.olsbbojue
						.chooseCard(current, targets)
						.forResult();
					if (result?.bool && result.cards?.length)
		...
		ai: {
			...
			result: {
				player(player, target) {
					return (
						get.effect(player, { name: "guohe" }, target, player) +
		...
		chooseCard(player, targets, eventId) {
			...
			return player
				...
				.set(
					...
					"若你与" +
						str +
						"的手牌数之和的变化值为：0，你与其依次弃置对方一张牌；2，你与其依次视为对对方使用一张【杀】"
				)
				.set("ai", (card) => {
					return 0;
				})
			...
			let history = player.getHistory(
				...
				(evt) => {
					return (
						...
						evt.getParent("phaseDraw").player !== player
					);
				},
				event
			);
				...
				filter(event, player) {
					...
					let history = game.getGlobalHistory(
						...
						(evt) => {
							if (
								evt.name !== "loseAsync" &&
								(event.name !== "lose" || evt.player !== player)
							)
								return false;
							if (
								evt.type != "discard" ||
								evt.getlx === false ||
								evt.getParent("phaseDiscard").player === player
							)
								return false;
					...
					return (
						history.length >= 2 &&
						event.getl(player).cards2.includes(history[1])
					);
				},
				forced: true,
				usable: 1,
				...
				async cost(event, trigger, player) {
					...
					if (trigger.name != "phase") {
						...
						let bool =
							...
							player.hasSkill("oljiushi", null, false);
						player.markAuto("oljiushi_gain", trigger.cards);
						if (
							bool &&
							player.getStorage("oljiushi_gain").length >=
								player.maxHp
						) {
							const result = await player
								.chooseBool("是否发动【酒诗】，将武将牌翻面？")
								.forResult();
							event.result = result;
						}
					} else
						player.unmarkAuto(
							"oljiushi_gain",
							player.getStorage("oljiushi_gain")
						);
				},
				async content(event, trigger, player) {
					await player.turnOver();
	...
	olsbjinming: {
		...
		async cost(event, trigger, player) {
			...
			for (let i = 0; i < choiceList.length; i++) {
				if (!player.getStorage("olsbjinming").includes(i + 1)) {
					choiceList[
						i
					] = `<span style="text-decoration: line-through;">${choiceList[i]}</span>`;
				}
			}
			const result = (event.result = await player
				.chooseButton([
					get.prompt2("olsbjinming"),
					[choiceList.slice(0, 2), "tdnodes"],
					[choiceList.slice(2, 4), "tdnodes"],
				])
				.set("filterButton", (button) => {
					const player = get.player();
					return player
						.getStorage("olsbjinming")
						.includes(parseInt(button.link.slice(0, 1)));
				})
		...
		async content(event, trigger, player) {
			...
			player.storage.olsbjinming_used =
			...
			used: {
				...
				intro: {
					...
					content(storage, player) {
						...
						return `最后一次发动〖矜名〗所选选项为${storage.slice(
							0,
							1
						)}`;
					},
	...
	olsbxiaoshi: {
		...
		filter(event, player) {
			if (
				!player.isPhaseUsing() ||
				player
					.getStorage("olsbxiaoshi_used")
					.includes(event.getParent("phaseUse")) ||
				!player.storage.olsbjinming_used
			)
				return false;
		...
		async content(event, trigger, player) {
			...
			player.markAuto("olsbxiaoshi_used", [
				trigger.getParent("phaseUse"),
			]);
		...
		subSkill: {
			...
			effect: {
				...
				async cost(event, trigger, player) {
					const num = parseInt(
						player.storage.olsbjinming_used.slice(0, 1)
					);
					event.result = await player
						.chooseTarget(
			...
			give: {
				...
				prompt() {
					...
					let list = game.filterPlayer((target) => {
						return (
							target != player &&
							target.hasZhuSkill("olsbyanliang", player) &&
							!target.hasSkill("olsbyanliang_used")
						);
					});
				...
				filter(event, player) {
					...
					return player.hasCard((card) => {
						return lib.skill.olsbyanliang_give.filterCard(
							card,
							player
						);
					}, "he");
			...
			player
				...
				.step(async (event, trigger, player) => {
					const bool = await player
						...
						.set(
							"choice",
							get.effect(player, sha, target, player) > 0
						)
						.forResultBool();
		...
		async cost(event, trigger, player) {
			event.result = await player
				.chooseToDiscard(
					...
					`弃置一张装备牌并将武将牌翻面，然后${get.translation(
						trigger.player
					)}回复至1点体力`,
					{ type: "equip" },
					"he"
				...
				.set("ai", (card) => {
					const player = get.player();
					if (
						get.attitude(player, get.event().getTrigger().player) >
						3
					) {
	...
	oljianmie: {
		...
		async content(event, trigger, player) {
					...
					humans.map((current) => {
						return new Promise(async (resolve, reject) => {
							if (current.isOnline()) {
								...
								const next = lib.skill.oljianmie.chooseControl(
									current,
									targets,
									eventId
								);
			...
			if (locals.length > 0) {
				for (const current of locals) {
					const result = await lib.skill.oljianmie
						.chooseControl(current, targets)
						.forResult();
					if (result && result.control)
						map[current.playerid] =
							result.control == "none2" ? "none" : result.control;
				}
			}
			delete event._global_waiting;
			for (const i of targets) i.hideTimer();
			const cards_player = player
				.getDiscardableCards(player, "h")
				.filter((card) => get.color(card) == map[player.playerid]);
			const cards_target = target
				.getDiscardableCards(target, "h")
				.filter((card) => get.color(card) == map[target.playerid]);
			...
			if (cards_player.length != cards_target.length) {
				const user =
					cards_player.length > cards_target.length ? player : target;
	...
	olsbliwen: {
		...
		async content(event, trigger, player) {
			while (
				player.hasMark("olsbliwen") &&
				game.hasPlayer(
					(t) => t != player && t.countMark("olsbliwen") < 5
				)
			) {
				const result = await player
					.chooseTarget(
			...
			const targets = game
				...
				.sort(
					(a, b) =>
						b.countMark("olsbliwen") - a.countMark("olsbliwen")
				);
			...
			for (const target of targets) {
				const result = await target
					...
					}, '###立文###<div class="text center">使用一张手牌，或移去所有“贤”标记并令' +
		...
		subSkill: {
			gain: {
				...
				content() {
					player.addMark(
						"olsbliwen",
						["useCard", "respond"].includes(trigger.name)
							? 1
							: Math.min(3, 5 - player.countMark("olsbliwen"))
					);
				},
				mod: {
	...
	olsbzhengyi: {
		...
		filter(event, player) {
			...
			return game.hasPlayer(
				(target) =>
					target != event.player && target.hasMark("olsbliwen")
			);
		},
		logTarget(event, player) {
		...
		async content(event, trigger, player) {
			const targets = game.filterPlayer(
				(target) =>
					target != trigger.player && target.hasMark("olsbliwen")
			);
			let humans = targets.filter(
				(current) => current === game.me || current.isOnline()
			);
					...
					humans.map((current) => {
						return new Promise(async (resolve, reject) => {
							if (current.isOnline()) {
								...
								const next = lib.skill.olsbzhengyi.chooseBool(
									current,
									trigger,
									eventId
								);
			...
			if (locals.length > 0) {
				for (const current of locals) {
					const result = await lib.skill.olsbzhengyi
						.chooseBool(current, trigger)
						.forResult();
	...
	olganlu: {
		...
		async content(event, trigger, player) {
			const num = Math.abs(
				event.targets[0].countCards("e") -
					event.targets[1].countCards("e")
			);
	...
	olzishou: {
		...
		async content(event, trigger, player) {
			...
			player
				...
				.filter(
					(evt) =>
						evt.getParent() == trigger.getParent() &&
						player.hasHistory(
							"sourceDamage",
							(evtx) => evtx.player != player
						) &&
	...
	olqieting: {
		...
		filter(event, player) {
			...
			return (
				...
				!target.hasHistory(
					"useCard",
					(evt) => evt.targets && evt.targets.some((i) => i != target)
				)
			);
		},
		async cost(event, trigger, player) {
	...
	olsbhongtu: {
		...
		prompt2:
			"你可以摸三张牌，展示三张手牌，令一名其他角色选择是否使用其中一张牌并令你随机弃置其中另一张牌。若使用牌的点数于三张牌中满足以下条件，其获得如下技能或效果直到其下一个回合的回合结束：唯一最大：〖飞军〗；不为最大且不为最小：〖潜袭〗；唯一最小：手牌上限+2。若其未以此法使用牌，你对其与你各造成1点火焰伤害。",
		check(event, player) {
			if (
				game.hasPlayer((current) => {
					return (
			...
			if (
				game.hasPlayer((current) => {
					return (
						get.damageEffect(current, player, player, "fire") >
							eff &&
		...
		async content(event, trigger, player) {
			...
			const [cards, targets] = await player
				.chooseCardTarget({
					...
					prompt2:
						"你选择的角色须选择是否使用其中的一张牌，并令你随机弃置其中的另一张牌。",
					position: "h",
					filterCard: true,
					selectCard: 3,
					filterTarget: lib.filter.notMe,
					...
					ai1(card) {
						...
						if (
							game.hasPlayer((current) => {
								return (
									get.attitude(get.player(), current) < 0 &&
									!current.hasUseTarget(card)
								);
							})
						)
							return 100 - val;
					...
					ai2(target) {
						...
						return Math.max(
							...ui.selected.cards.map(
								(card) => target.getUseValue(card) * att
							)
						);
					},
				})
			...
			await player.showCards(
				cards,
				`${get.translation(player)}对${get.translation(
					target
				)}发动了【鸿图】`
			);
			const links = await target
				.chooseButton([
					`鸿图：是否使用${get.translation(
						player
					)}展示的其中一张牌？`,
					cards,
				])
			...
			if (!links || !links.length) {
				...
				const restCards = cards.filter((card) => {
					return (
						...
						get.position(card) === "h" &&
						lib.filter.cardDiscardable(card, player, "olsbhongtu")
					);
				});
				if (restCards.length) {
			...
			limit: {
				...
				content() {
					player.storage.olsbhongtu_limit = [
						player.storage.olsbhongtu_limit[1],
						0,
					];
					if (!player.storage.olsbhongtu_limit[0])
	...
	olsbqiwu: {
		...
		async cost(event, trigger, player) {
			event.result = await player
				.chooseToDiscard(
					get.prompt("olsbqiwu"),
					`你可以弃置一张红色牌，防止${get.translation(
						trigger.source
					)}对你造成的${trigger.num}点伤害。`,
					"chooseonly",
					{ color: "red" },
					"he"
				)
				...
				.set(
					"goon",
					get.damageEffect(player, trigger.source, player) < 0
				)
				.forResult();
		},
		async content(event, trigger, player) {
				...
				.chooseCardTarget({
					...
					ai1(card) {
						if (
							!_status.event.goon &&
							game.countPlayer(
								(target) => target != _status.event.player
							) > 1
						)
							return 0;
		...
		async content(event, trigger, player) {
			...
			if (
				...
				while (
					num > 0 &&
					player.hasCard(
						(card) => !card.hasGaintag("olsujian_given"),
						"he"
					)
				) {
					const {
	...
	olchunlao: {
		...
		filter(event, player) {
			...
			return game.hasPlayer((target) => {
				if (
					![player.getPrevious(), player, player.getNext()].includes(
						target
					)
				)
					return false;
		...
		async content(event, trigger, player) {
			const cards = game
				...
				.flat()
				.unique();
			const gain = cards.slice(
				0,
				Math.min(
					cards.length,
					9 - player.getExpansions("olchunlao").length
				)
			);
			if (gain.length) {
			...
			save: {
				...
				filter(event, player) {
					...
					return (
						...
						event.dying &&
						event.dying.hp <= 0 &&
						player.getExpansions("olchunlao").length >= num
					);
				},
				async content(event, trigger, player) {
					const target = event.targets[0];
		...
		async content(event, trigger, player) {
			const {
				...
				.set("list", [
					[
						"本次弃置的牌",
						get.info("olzongxuan").getCards(trigger, player),
					],
					["牌堆顶"],
				])
		...
		getCards(event, player) {
			...
			for (const target of [player, player.getPrevious()]) {
				...
				if (
					evt &&
					evt.cards2 &&
					evt.cards2.some((i) => get.position(i) == "d")
				) {
					if (
						target == player ||
						target
							.getHistory("lose", (evt) => {
								return (
									evt.type == "discard" && evt.getlx !== false
								);
							})
							.indexOf(event) == 0
					) {
						cards.addArray(
							evt.cards2.filter((i) => get.position(i) == "d")
						);
					}
				}
			}
	...
	olzhiyan: {
		...
		filter(event, player) {
			return (
				event.player == player || event.player == player.getPrevious()
			);
		},
		direct: true,
		async content(event, trigger, player) {
		...
		async content(event, trigger, player) {
			const {
				...
				.chooseCardTarget({
					...
					filterCard(card, player) {
						return (
							get.color(card) ==
								get.color(get.event().getTrigger().card) &&
							lib.filter.cardDiscardable(card, player)
						);
					},
					position: "he",
			...
			if (bool) {
				...
				trigger
					...
					.excluded.addArray(
						game.filterPlayer(
							(i) => trigger.targets.includes(i) && target != i
						)
					);
			}
		},
	...
	olsbyufeng: {
		...
		filter(event, player) {
			const card =
				get.cardPile("sizhaojian", "field") ||
				game.createCard2("sizhaojian", "diamond", 6);
			return (
				(event.name != "phase" || game.phaseNumber == 0) &&
				...
				mod: {
					cardEnabled(card, player) {
						...
						if (
							!evt ||
							!evt.respondTo ||
							!storage.includes(evt.respondTo[1])
						)
							return;
	...
	olsbshishou: {
		...
		filter(event, player) {
			...
			return game.hasPlayer((target) => {
				...
				const evt = event.getl(target);
				return (
					evt && evt.player == target && evt.es && evt.es.length > 0
				);
			});
		},
		direct: true,
			...
			effect: {
				...
				async cost(event, trigger, player) {
					...
					if (targets.length == 1) {
						...
						const bool = await player
							.chooseBool(
								get.prompt("olxianzhen_effect", target),
								"令" +
									get.translation(target) +
									"也成为" +
									get.translation(trigger.card) +
						...
						event.result = await player
							.chooseTarget(
								get.prompt("olxianzhen_effect"),
								"令任意名【陷阵】拼点成功的目标角色也成为" +
									get.translation(trigger.card) +
									"的目标",
								(card, player, target) => {
									const trigger = get.event().getTrigger();
					...
					effect: {
						player_use(card, player, target, current, isLink) {
							if (
								isLink ||
								!target ||
								player._olxianzhen_effect_temp
							)
								return;
							if (
								!player
	...
	olsbduoshou: {
		...
		mod: {
			targetInRange(card, player, target) {
				if (
					get.color(card) == "red" &&
					!player.hasSkill("olsbduoshou_used")
				)
					return true;
			},
		},
				...
				mod: {
					cardname: function (card) {
						if (
							...
							card.hasGaintag("olsbfumeng_buff")
						)
							return "sha";
					},
				},
	...
	olsbguidao: {
		...
		filterCard(card, player) {
			...
			if (
				ui.selected.cards.filter(
					(cardx) => get.name(cardx, player) == "sha"
				).length < num &&
				get.name(card, player) != "sha"
			)
				return false;
		...
		async content(event, trigger, player) {
			...
			player.getHistory("custom").push({
				olsbguidao_num:
					cards.filter((card) => get.name(card, player) == "sha")
						.length + 1,
			});
			const card = new lib.element.VCard({
				name: "juedou",
	...
	olsbzhuri: {
			...
			if (result.bool) {
				...
				if (result2.bool) {
					...
					var result3 = yield player
						...
						.set(
							"list",
							list.slice(trigger.getParent().num, list.length)
						);
		...
		subSkill: {
			block: {
				...
				mark: true,
				marktext:
					'<span style="text-decoration: line-through;">日</span>',
	...
	olsbranji: {
		...
		content: function* (event, map) {
			...
			player
				...
				.then(() => player.removeSkill("olsbranji_norecover"));
		},
		derivation: ["kunfenx", "zhaxiang"],
		getList: function (event) {
			return event
				.getParent()
				.phaseList.map((list) => list.split("|")[0]);
		},
			...
			return player
				...
				.reduce((list, evt) => {
					return list.add(
						evt.getParent(
							lib.phaseName.find(
								(name) => evt.getParent(name).name == name
							)
						)
					);
				}, []).length;
	...
	oljiangchi: {
		...
		direct: true,
		logAudio: (index) =>
			typeof index === "number" ? "rejiangchi" + index + ".mp3" : 2,
		content: function* (event, map) {
			var player = map.player;
			var choiceList = [
			...
			if (result.control != "cancel2") {
				player.logSkill("oljiangchi", null, null, null, [
					result.control == "摸牌，-1" ? 1 : 2,
				]);
	...
	olqiaoshui: {
		...
		async content(event, trigger, player) {
			...
			if (result.bool)
				player.addTempSkill("olqiaoshui_target", {
		...
		subSkill: {
			used: {
				...
				mark: true,
				marktext:
					'<span style="text-decoration: line-through;">说</span>',
				intro: { content: "被迫闭嘴" },
			},
			target: {
				audio: "olqiaoshui",
