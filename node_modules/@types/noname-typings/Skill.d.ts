...
declare interface SkillHookTrigger {
	 ...
	 * @param player
	 * @param triggername
	 * @param triggernameList
	 */
	after?(event: GameEventPromise, player: Player, triggername: string, triggernameList: string[]): boolean;
	/**
	 * 【hookTrigger相关】
	 *
...
declare interface Skill {
	...
	*/
	 ...
	 *
	 * @deprecated 与此的相关判断请改为使用seatRelated:'changeSeat'
	 */
	changeSeat?: boolean;
	 ...
	 * @param player
	 * @param name 触发事件的第一个名称，为event.triggername或event.triggernameList[0]，目前在lib.filter.filterTrigger中才传该值，但是基本不调用此值，最大用处仅用于兼容旧版技能
	 * @param target v1.10.11 触发的目标
	 * @param names 触发事件的所有名称数组集合，为event.triggernameList，目前只有在lib.filter.filterTrigger中才传该值，即被动触发，主动触发不检测该值，目前暂未完善
	 */
	filter?(event: GameEventPromise, player: Player, name?: string, target?: Player, names?: string[]): boolean | void | null;
	/**
	 * 选择的目标武将牌上出现什么字。
	 *
	...
	*/
	 ...
	 * @param target v1.10.11 触发的目标
	 */
	logTarget?: string | ((event?: GameEventPromise, player?: Player, triggername?: string, target?: Player, triggernameList?: string[]) => string | Player | Player[] | null | undefined);
	/**
	 ...
	 * 无参，简洁写法；
	 */
	check?: ((card: Card) => number | boolean | void) | ((event: GameEventPromise, player: Player, triggername?: string, target?: Player, triggernameList?: string[]) => number | boolean | void) | (() => number | boolean | void);
