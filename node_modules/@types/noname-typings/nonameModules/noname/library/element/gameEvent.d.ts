...
export class GameEvent implements PromiseLike<Omit<GameEvent, "then">> {
	 ...
	 */
	callHandler(
		type: Parameters<typeof this.hasHandler>[0],
		event: GameEvent,
		option: {
			state?: "begin" | "end";
		}
	 ...
	 * }) => void)[]}
	 */
	getHandler(type?: Parameters<typeof this.hasHandler>[0]): ((
		event: GameEvent,
		option: {
			state?: "begin" | "end";
		}
	...
	_cancelled: boolean;
	neutralize(event?: GameEvent): Promise<Omit<GameEvent, "then">>;
	/**
	 * @type { boolean }
	 */
	...
	/**
	 * 事件当前交互的对话框喵
	 * 一般是chooseButton类用的多喵
	 */
	dialog?: Dialog;
	/**
	 * 表示当前事件不允许使用全选按钮喵（true没有全选按钮）
	 */
	noChooseAll?: boolean;
	/**
	 * 当前事件牌的全选按钮喵
	 */
	cardChooseAll?: Control;
	/**
	 * 当前事件按钮的全选按钮喵
	 */
	buttonChooseAll?: HTMLDivElement;
	ai1: any;
	ai2: any;
	restore(): this;
	fakeforce: any;
     ...
     * @returns { Promise<Exclude<Result[T], undefined>[]> }
     */
	forResult<T extends keyof Result>(): Promise<Partial<Result>>;
	/**
     * 获取 Result 对象中的信息。
     * @example
     * ```js
