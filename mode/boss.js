...
export default () => {
	return {
		...
		start() {
			...
			event.current = null;
			var list = [];
			if (lib.storage.current == undefined) {
				lib.storage.current = "boss_hundun";
			}
...
characterIntro: {
			...
			boss_yanluowang:"第五殿阎罗王包拯，东北方度仙上圣天尊化冥府五殿最胜耀灵真君阎罗天子，神居纠纶宫，阎罗天子包，正月初八日诞辰，位列垦宫，尊居丑位，执掌镬汤地狱，威张煮溃之权，专司大海之底，东北沃焦石下叫唤大地狱，并十六诛心小狱。前本居第一殿，因怜屈死，屡放还阳伸雪，降调此殿。凡解到此殿者，押赴望乡台，令之闻见世上本家，因罪遭殃各事，随即推入此狱，细查曾犯何恶，再发入诛心十六小狱，钩出其心，掷与蛇食，铡其身首（包公即善于用铡刀），受苦满日，另发别殿。",
		    ...
		    boss_mengpo:"孟婆神孟姜女，常驻在忘川河黄泉路奈何桥旁，醧忘台为了能忘记这些痛苦万分的记忆，熬制了能使人忘记记忆的孟婆汤。她在奈何桥畔熬制孟婆汤，让参与轮回的阴魂们忘记前世的一切。高大如方丈，四周的廊房有一百零八间。有一条通道，通向东方，仅一尺四寸宽。所有奉令押解到的男女鬼魂，在各廊房中，都有杯子，招各鬼饮下此汤，多饮少饮不论。",
			...
			boss_baihu: "白虎也是战神、杀伐之神。白虎具有避邪、禳灾、祈丰及惩恶扬善、发财致富、喜结良缘等多种神力。而它是四灵之一，当然也是由星宿变成的。是由二十八星宿之的西方七宿（奎、娄、胃、昂、毕、觜、参）。所以是西方的代表，而它的白，是因是西方，西方在五行中属金，色是白的。所以它叫白虎不是因它是白色，而是从五行中说的了。白虎原为古老神话中的西方之神，从先秦时代开始它就是代表少昊与西方七宿的神兽，而到了汉代五行学说兴起，它的象征含义又多了庚辛与秋季，四象合于五行，西方白虎又多了象征五行中金行的意义。",
			...
			boss_xuanwu: "玄武是一种由龟和蛇组合成的一种灵物。玄武的本意就是玄冥，武、冥古音是相通的。武，是黑的意思；冥，就是阴的意思。玄冥起初是对龟卜的形容：龟背是黑色的，龟卜就是请龟到冥间去诣问袓先，将答案带回来，以卜兆的形式显给世人。因此，最早的玄武就是乌龟。二十八宿的北方七宿（斗、牛、女、虚、危、室、壁），以后，玄冥的含义不断地扩大。龟生活在江河湖海（包括海龟），因而玄冥成了水神；乌龟长寿，玄冥成了长生不老的象征；最初的冥间在北方，殷商的甲骨占卜即‘其卜必北向’，所以玄冥又成了北方神。",
			boss_shuishenxuanming: "玄冥一词指水神、冬神、北方之神，而道家则用来形容“道”的，道教称肾之神。到汉代，中国民间用来指阴间，九泉；同时用于汉代郊祀歌名，歌词为：“玄冥陵阴，蛰虫盖臧……”取首二字为歌名。到唐代以后，就有用来称为北方的，也有用来称为冬季的。而到了现代，“玄冥”则成为各种武侠小说里武功怪异人的代名词了。但实际上，“冥”是商族领袖之一。因其”勤其官而水死“，故被后人奉为水神，称“玄冥”。",
			...
			chooseCharacter(func) {
				...
				next.setContent(function () {
					...
					for (i in lib.character) {
						...
						if (lib.character[i].isAiForbidden) {
							continue;
						}
			...
			boss_taowu: {
				...
				randchoice(name, list) {
					if (Math.random() > 1 / 3) {
						...
						var arr = ["shen_caocao", "shen_simayi", "shen_guanyu", "shen_zhugeliang", "shen_zhaoyun", "shen_zhouyu", "shen_lvmeng", "shen_lvbu", "shen_liubei", "shen_luxun", "shen_ganning", "ol_zhangliao", "shen_zhenji", "shen_caopi", "key_kagari", "key_shiki", "db_key_hina", "shen_sunquan", "junk_zhangjiao"];
				...
				control(type, control) {
					if (type == "cancel") {
						if (!control.classList.contains("glow")) {
							return;
						}
						...
						control.backup2 = ui.create.buttons(["shen_caocao", "shen_simayi", "shen_guanyu", "shen_zhugeliang", "shen_zhaoyun", "shen_zhouyu", "shen_lvmeng", "shen_lvbu", "shen_liubei", "shen_luxun", "shen_ganning", "ol_zhangliao", "shen_zhenji", "shen_caopi", "key_kagari", "key_shiki", "db_key_hina", "shen_sunquan", "junk_zhangjiao"], "character", control.backup1);
						return control;
					}
				},
				init() {
				...
				content() {
					...
					player
						...
						.set("ai", function (target) {
							...
							if (target.countDiscardableCards(_status.event.player, "he") >= _status.event.getParent().num) {
								att = att * 2;
							}
							return -att;
						});
	...
	*/
					...
					effect: {
						target(card, player, target) {
							...
							if (get.tag(card, "damage") || get.tag(card, "recover")) {
								...
								if (target.isDying()) {
									if (player.isEnemyOf(target) && player.hp >= -1) {
										return [0, 0, 0, 1];
									}
									return "zeroplayertarget";
								}
		...
		translate: {
			...
			*/
			...
			"#boss_heisheng": "此绳索，是你无法逃脱的噩梦。",
			"#boss_shengfu": "想跑？痴人说梦！",
