...
export default {
	...
	gz_luanchang: {
		...
		filter(event, player) {
			...
			return event.player.countCards("h") && event.player.hasUseTarget(card) && game.hasPlayer2(current => {
	...
	gz_yanxi: {
		...
		async content(event, trigger, player) {
			...
			const result = await player
				.chooseTarget("宴席：展示并获得一名角色被你选择的牌", (card, player, target) => {
			...
			if (result?.bool) {
				...
				if (name != card.name) {
					await player.gain(cards.filter(cardx => cardx != card), "giveAuto");
				}
			}
		},
	},
	...
	gz_chengxi: {
		...
		async content(event, trigger, player) {
			...
			if (result?.targets?.length && game.hasPlayer2(current => {
	...
	gz_zhefu: {
		...
		filter(event, player) {
			return player != _status.currentPhase && get.type(event.card) == "basic" && game.hasPlayer(current => {
		...
		async cost(event, trigger, player) {
			event.result = await player
				...
				.set("numx", game.countPlayer(target => target.identity == player.identity))
				.set("ai", target => {
					const player = get.player();
	...
	gz_yidu: {
		...
		filter(event, player) {
			return get.tag(event.card, "damage") > 0.5 && event.targets.some(target => {
		...
		async content(event, trigger, player) {
			...
			while(true) {
				...
				if (result.bool) {
					...
					const result2 = await player.chooseTarget("乘流：对一名装备区牌数少于你的角色造成1点伤害", (card, player, target) => {
						return target.countCards("e") < player.countCards("e");
					})
					.forResult();
					if (result2.bool) {
			...
			use: {
				...
				chooseButton: {
					...
					backup(links, player) {
						...
						game.broadcastAll((skill, name, info) => {
						...
						}, "gz_jiaping_use_backup", links[0], info);
						return info;
					},
					prompt(links, player) {
	...
	gz_shujuan: {
		...
		filter(event, player) {
			...
			if (event.name == "equip" && event.card.name == "jilinqianyi") {
				...
				if (history?.everything?.filter(evt => {
	...
	gz_xiace: {
		...
		viewAsFilter(player) {
			...
			if (event) {
				return num > target.getHistory("useCard", evt => {
	...
	gz_limeng: {
		...
		async content(event, trigger, player) {
			const { cards, targets: [target1, target2] } = event;
			await player.discard(cards);
			if (target2) {
