	...
	return {
		...
		start() {
			...
			if (playback) {
				...
				if (!["normal", "yingbian", "old", "free"].includes(_status.mode)) {
					_status.mode = "normal";
				}
				//决定牌堆
				switch (_status.mode) {
			...
			if (_status.connectMode) {
				...
				if (!["normal", "yingbian", "old"].includes(_status.mode)) {
					_status.mode = "normal";
				}
			...
			showYexings() {
				...
				next.setContent(function () {
					...
					if (!_status.yexinjia_list) {
						_status.yexinjia_list = ["夏", "商", "周", "秦", "汉", "隋", "唐", "宋", "辽", "金", "元", "明"];
					}
					source
					...
					if (result.control) {
						...
						text = _status.yexinjia_list.randomRemove();
					}
					lib.group.push(text);
					...
					target
						...
						.set(
							...
							(function () {
								...
								if (game.players.length <= 2 * fs) {
									return false;
								}
								if (source.getFriends(true).length + fs > game.players.length / 2) {
									return true;
								}
								if (target.isDamaged() || target.countCards("h") < 4) {
									return false;
								}
								return true;
							})()
			...
			chooseCharacter() {
				...
				next.setContent(function () {
					...
					for (i in lib.character) {
						...
						if (lib.character[i].hasHiddenSkill) {
							continue;
						}
						event.list.push(i);
					}
					...
					if (result && result.control) {
						game.me.trueIdentity = result.control;
					}
					if (event.choosen) {
			...
			chooseCharacterOL() {
				...
				next.setContent(function () {
					...
					for (var i in lib.characterPack.mode_guozhan) {
						...
						if (lib.config.guozhan_banned && lib.config.guozhan_banned.includes(i)) {
							continue;
						}
						list.push(i);
					}
