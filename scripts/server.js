...
wss.on("connection", function (ws) {
	...
	ws.on("message", function (message) {
		...
		if (messageStr == "heartbeat") {
			...
			// Manually construct the message to avoid double-stringification
			// The message is already a JSON string, so we only need to wrap it
			// in the onmessage envelope without stringifying it again
			try {
				var parsed = JSON.parse(messageStr);
				var envelope = ["onmessage", this.wsid, parsed];
				this.owner.send(JSON.stringify(envelope));
			} catch (e) {
				this.owner.close();
			}
		} else {
...
console.log("WebSocket multiplayer server started on port 8080");
