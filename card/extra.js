...
const extra = {
  ...
  card: {
    ...
    jiu: {
      ...
      ai: {
        ...
        order(item, player) {
          ...
          if (usable < 2 && player.hasCard((i) => get.name(i, player) === "zhuge", "hs")) {
            usable = Infinity;
          }
        ...
        result: {
          target: (player, target, card) => {
            ...
            for (const i in effs) {
              ...
              target.needsToDiscard() > Math.max(0, 3 - target.hp) || !effs[i].target.mayHaveShan(player, "use")) {
    ...
    huogong: {
      ...
      async content(event, trigger, player) {
        const target = event.target;
        const showPosition = event.showPosition === void 0 ? "h" : event.showPosition;
        if (!target.hasCards(showPosition)) {
          return event.finish();
        }
        event.chooseToShow ??= async (event2, player2, target2) => {
          ...
          if (target2.countCards(showPosition2) === 1) {
            ...
            result3 = await target2.chooseCard(true, showPosition2, "请选择【火攻】要展示的牌", filterShow).set("ai", (card) => {
        ...
        event.chooseToDiscard ??= async (event2, player2, target2) => {
          const discardPosition = event2.discardPosition === void 0 ? "h" : event2.discardPosition;
          const cards2 = event2.cards2;
          const filterDiscard = event2.filterDiscard === void 0 ? { suit: get.suit(cards2[0]) } : event2.filterDiscard;
          const result3 = await player2.chooseToDiscard(discardPosition, filterDiscard).set("ai", (card) => {
      ...
      ai: {
        ...
        result: {
          ...
          target(player, target) {
            if (target.hasSkill("huogong2") || !target.hasCards("h")) {
              return 0;
            }
      ...
      ai: {
        value(card, player, index, method) {
          ...
          if (player.getEquips("tengjia").includes(card)) {
            ...
            if (game.hasPlayer((current2) => current2 !== player && get.attitude(current2, player) < 0 && current2.hasSkillTag("fireAttack", null, null, true))) {
              return 0;
            }
            return 6;
          }
        ...
        equipValue(card, player) {
          ...
          let num = 4 - game.countPlayer((current) => {
  ...
  skill: {
    ...
    muniu_skill: {
      ...
      onChooseToUse(event) {
        ...
        const cards = event.player.getVCards("e", (card) => card.name === "muniu" && !card?.storage?.used);
      ...
      chooseButton: {
        ...
        backup(links, player) {
          return {
            ...
            lose: false,
            delay: false,
            prepare(cards, player2) {
              player2.$give(1, player2, false);
            },
            async content(event, trigger, player2) {
              await player2.loseToSpecial(event.cards, "muniu");
              let dx = 0;
              for (const [i, card] of [...event.cards].entries()) {
                if (card._selfDestroyed || !card.hasGaintag("muniu") || get.position(card) !== "s") {
                  card.remove();
                  event.cards.splice(i - dx, 1);
                  ++dx;
                }
              }
              const muniu = player2.getVCards("e").find((card) => card.vcardID === lib.skill.muniu_skill_backup.muniu);
              ...
              const players = game.filterPlayer((current) => {
                return current.canEquip(muniu) && current !== player2 && !current.isTurnedOver() && get.attitude(player2, current) >= 3 && get.attitude(current, player2) >= 3;
              });
              players.sort(lib.sort.seat);
              const choice = players[0];
              const next = player2.chooseTarget("是否移动木牛流马？", (card, player3, target) => !target.isMin() && player3 !== target && target.canEquip(_status.event.muniu)).set("muniu", muniu);
              next.set("ai", (target) => target === _status.event.choice ? 1 : -1);
              next.set("choice", choice);
              const result = await next.forResult();
              if (result?.bool && result?.targets?.length) {
                const card = player2.getCards("e", (i) => i[i.cardSymbol] === muniu)[0];
                if (card) {
                  await result.targets[0].equip(card);
                  player2.$give(card, result.targets[0]);
      ...
      mod: {
        cardEnabled2(card, player) {
          ...
          const cards = player.getVCards("e", (card2) => card2.name === "muniu");
    ...
    jiu: {
      ...
      async content(event, trigger, player) {
        ...
        if (lib.skill.jiu2.filter(trigger, player)) {
          game.broadcastAll((player2) => player2.removeSkill("jiu"), player);
