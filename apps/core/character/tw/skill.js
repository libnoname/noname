...
const skills = {
  ...
  twwuhun: {
    ...
    async content(event, trigger, player) {
      ...
      const num = game.countPlayer(
        (current) => current !== player && current.hasMark("twwuhun"),
      );
      if (num === 0) {
        return;
      }
      ...
      next.set(
        "filterTarget",
        (card, _player, target) =>
          target !== player && target.hasMark("twwuhun"),
      );
  ...
  twshelie: {
    ...
    subSkill: {
      ...
      jingce: {
        ...
        filter(event, player) {
          if (
            player.hasSkill("twshelie_round") ||
    ...
    async content(event, trigger, player) {
      ...
      if (result.bool) {
        ...
        if (
          ...
          player.line(target);
          player.addTempSkill("twgongxin3", {
            player: ["twgongxin3After", "phaseAfter"],
          });
  ...
  twgongxin2: {
    mod: {
      cardEnabled2(card, player) {
        ...
        if (
          color != "unsure" &&
          player.getStorage("twgongxin2").includes(color)
        ) {
          return false;
        }
      },
  ...
  twtaoluan: {
    ...
    filter(event, player) {
      return (
        ...
        player.countDiscardableCards(
          player,
          "he",
          (card) => get.suit(card) == event.result.suit,
        ) > 0
      );
    },
    async cost(event, trigger, player) {
      ...
      event.result = await player
        ...
        .set(
          "goon",
          trigger.result.judge * get.attitude(player, trigger.player) <= 0,
        )
        .forResult();
    },
    async content(event, trigger, player) {
      ...
      if (evt.name === "phaseJudge") {
        ...
        trigger.next = trigger.next.filter(
          (next) => next.name !== "judgeCallback",
        );
        ...
        const cards = evts
          .flatMap((e) => e.cards)
          .filter((card) => get.position(card, true) === "o");
      ...
      draw: {
        ...
        filter(event, player) {
          return event.cards?.some((card) =>
            player.getStorage("twshiji_draw").includes(get.suit(card)),
          );
        },
        async content(event, trigger, player) {
  ...
  twzhengjun: {
    ...
    async content(event, trigger, player) {
      ...
      const check = (evt) =>
        evt.getParent().name == event.name ||
        (evt.getParent().name == "draw" && evt.getParent(2).name == event.name);
      if (
        player
          .getRoundHistory("gain", (evt) => check(evt) && evt.cards.length > 0)
          .flatMap((evt) => evt.cards).length >= 2
      ) {
  ...
  twshelun: {
    ...
    filter(event, player) {
      return (
        player.countCards("h") &&
        game.hasPlayer(
          (current) => current != player && player.inRange(current),
        )
      );
    },
    filterTarget(card, player, target) {
      ...
      await player
        ...
        .set("callback", async function (event, trigger, player) {
          ...
          if (result.bool && result.opinion) {
            ...
            if (opinion && ["red", "black"].includes(opinion)) {
              player.logSkill("twshelun", targets, null, null, [
                opinion == "red" ? 3 : 4,
              ]);
              if (opinion == "red") {
        ...
        .set("ai", (card) => {
          ...
          const color =
            ...
            targets.reduce(
              (sum, target) => sum + get.damageEffect(target, player, player),
              0,
            ) > 0
              ? "black"
              : "red";
  ...
  twjishan: {
    ...
    filter(event, player, name) {
      ...
      return game.hasPlayer(
        (current) =>
          current.isMinHp() &&
          player.getStorage("twjishan").includes(current) &&
          current.isDamaged(),
      );
    },
    async cost(event, trigger, player) {
      ...
      if (triggername == "damageBegin4") {
        const result = await player
          .chooseBool(
            get.prompt(skill, target),
            "失去2点体力并防止此伤害，然后你与其各摸一张牌",
          )
        ...
        event.result = await player
          .chooseTarget(
            ...
            (card, player, target) => {
              return (
                target.isMinHp() &&
                player.getStorage("twjishan").includes(target) &&
                target.isDamaged()
              );
            },
          )
    ...
    async cost(event, trigger, player) {
      event.result = await player
        ...
        .set("ai", (target) => {
          ...
          const eff2 = [
            1,
            get.recoverEffect(player, player, target) +
              get.effect(player, { name: "draw" }, player, target) * num,
          ];
          return [
            eff3,
  ...
  twzhenglve2: {
    ...
    mod: {
      cardUsableTarget(card, player, target) {
        if (
          player.isPhaseUsing() &&
          !player.hasSkill("twzhenglve3") &&
          target.hasMark("jsrgzhenglve_mark")
        ) {
          return true;
        }
      },
  ...
  twguanhuo: {
    ...
    subSkill: {
      ...
      effect: {
        ...
        filter(event, player) {
          return (
            event.card?.storage?.twguanhuo &&
            !game.hasPlayer2((current) =>
              current.hasHistory("damage", (evt) => evt.card == event.card),
            )
          );
        },
        forced: true,
      ...
      ex: {
        ...
        filter(event, player) {
          return (
            event.card?.name == "huogong" && event.getParent().type == "card"
          );
        },
        forced: true,
        popup: false,
  ...
  twjuxia: {
    ...
    filter(event, player) {
      ...
      return (
        event.player !== player &&
        lib.skill.jsrgjuxia.countSkill(event.player) >
  ...
  twanguo: {
    ...
    subSkill: {
      ...
      move: {
        ...
        filter(event, player) {
          return (
            ...
            game.hasPlayer(
              (current) =>
                !current.hasMark("twanguo_mark") && current != player,
            )
          );
        },
        async cost(event, trigger, player) {
          const targets = game.filterPlayer((current) =>
            current.hasSkill("twanguo_mark"),
          );
          const prompt2 =
            targets.length == 1
          ...
          const result = await player
            .chooseTarget(
              get.prompt("twanguo"),
              prompt2,
              targets.length == 1 ? 1 : 2,
              (card, player, target) => {
                if (
                  ui.selected.targets.length == 0 &&
                  _status.event.targets.length > 1
                ) {
            ...
            .set("ai", (target) => {
              ...
              return (
                get.attitude(player, _status.event.targets[0]) <
        ...
        async content(event, trigger, player) {
          ...
          let target1, target2;
          if (targets.length == 1) {
            target1 = game.filterPlayer((current) =>
              current.hasSkill("twanguo_mark"),
            )[0];
            target2 = targets[0];
          } else {
          ...
          for (const key in map) {
            ...
            target2.markAuto(
              "twanguo_effect",
              target1.getStorage("twanguo_effect"),
            );
      ...
      draw: {
        ...
        usable: 1,
        prompt:
          "摸等同于一名拥有“安国”标记的角色已损失体力值张牌，并交给其等量的牌",
        filter(event, player) {
          return game.hasPlayer(
            (target) => target.hasMark("twanguo_mark") && target.isDamaged(),
          );
        },
        filterTarget(card, player, target) {
      ...
      dying: {
        ...
        filter(event, player) {
          ...
          if (event.player == player) {
            return game.hasPlayer(
              (target) => target.getStorage(skill).length > 0,
            );
          }
          ...
          if (trigger.player == player) {
              ...
              .chooseButtonTarget({
                ...
                filterButton(button) {
                  return game.hasPlayer((target) =>
                    target.getStorage("twanguo_effect").includes(button.link),
                  );
                },
                complexTarget: true,
                filterTarget(card, player, target) {
            ...
            const result = await player
              ...
              .set("filterButton", (button) => {
                return get
                  .event()
                  .targetx.getStorage("twanguo_effect")
    ...
    subSkill: {
      effect: {
        ...
        async content(event, trigger, player) {
          ...
          if (
            !target.hasHistory("sourceDamage", (evt) => evt.num) &&
  ...
  twmingce: {
    ...
    filter(event, player) {
      return (
        ...
        player.getStorage("twmingce_used").length < 2
      );
    },
    filterCard: true,
    check(card) {
    ...
    async content(event, trigger, player) {
      ...
      storage.forEach((num) => {
        ...
        choiceList[num] =
          '<span style="opacity:0.5">' + choiceList[num] + "</span>";
      });
      let result;
      if (choices.length == 1) {
  ...
  twfushu: {
    ...
    filter(event, player) {
      ...
      return (
        event.filterCard({ name: "tao", isCard: true }, player, event) &&
        player.countCards("h") &&
        !player.hasSkillTag("noCompareSource")
      );
    },
    ...
    prompt:
      "选择一张手牌与牌堆顶的一张牌拼点：若你赢，你视为使用一张【桃】；若你没赢，你下次受到的伤害值+1",
    ...
    ai: {
      ...
      skillTagFilter(player, tag, arg) {
        return (
          player.countCards("h") > 0 && !player.hasSkillTag("noCompareSource")
        );
      },
      order: 7,
      result: {
  ...
  twxiumu: {
    ...
    filter(event, player) {
      return game.hasPlayer(
        (current) => current.countCards("h") && current != player,
      );
    },
    async cost(event, trigger, player) {
      event.result = await player
    ...
    async content(event, trigger, player) {
      ...
      if (
        ...
        const result = await target
          .chooseCard(
            `修睦：请选择点数之和大于等于13的手牌与${get.translation(player)}的手牌交换`,
            [1, Infinity],
            true,
            "h",
          )
          ...
          .set("ai", (card) => {
            ...
            if (num < 13) {
              ...
              return (
                Math.ceil(get.number(card, player) / 4) * (6 - get.value(card))
              );
            }
            return 0;
          })
  ...
  twhuiyu: {
    ...
    async cost(event, trigger, player) {
      const targetprompt =
        Math.random() > 0.25 ? ["代替", "被代替"] : ["替罪羊", "躺赢狗"];
    ...
    async content(event, trigger, player) {
      ...
      target.addTip(
        skill,
        get.translation(skill) + " " + get.translation(source),
      );
    ...
    subSkill: {
      effect: {
        ...
        logTarget(event, player) {
          const source = game.findPlayer((target) =>
            player.getStorage("twhuiyu_effect").includes(target),
          );
          return source;
        },
        forced: true,
        filter(event, player) {
          return (
            lib.skill.twhuiyu_effect.logTarget(event, player)?.isIn() &&
            player.isPhaseUsing()
          );
        },
        content() {
  ...
  twbeixing: {
    ...
    check(event, player) {
      return (
        ...
        event.player.countDiscardableCards(player, "h")
      );
    },
    logTarget: "player",
    async content(event, trigger, player) {
      ...
      if (targets.length && player.hp > target.hp) {
        ...
        const result = await player
          .chooseTarget(
            `狈行：你可令一名角色获得${get.translation(target)}的一张手牌`,
            (card, player, target) => {
              return (
  ...
  twhanhong: {
    ...
    chooseButton: {
      ...
      chooseControl(event, player) {
        const choices = lib.suit.filter(
          (s) => !player.getStorage("twhanhong_used").includes(s),
        );
      ...
      check(event, player) {
        return lib.suit
          .filter((s) => !player.getStorage("twhanhong_used").includes(s))
          .randomGet();
      },
  ...
  twhuazhang: {
    audio: 3,
    logAudio: (index) =>
      typeof index === "number" ? "twhuazhang" + index + ".mp3" : 2,
    trigger: {
      player: "phaseUseEnd",
    },
    filter(event, player) {
      return (
        ...
        player.hasCard((card) => player.canRecast(card), "h")
      );
    },
    check: () => true,
    async content(event, trigger, player) {
  ...
  twqianxiong: {
    ...
    async content(event, trigger, player) {
      ...
      const result2 = await player
        .chooseTarget(
          `潜凶：将${get.translation(card)}正面向下置于一名角色的武将牌上`,
          true,
        )
      ...
      await target
        .addToExpansion(card, player, "give")
        .set("gaintag", ["twqianxiong"]);
    },
    intro: {
      markcount: "expansion",
      content(storage, player, skill) {
    ...
    subSkill: {
      damage: {
        ...
        filter(event, player) {
          ...
          return target
            .getExpansions("twqianxiong")
            .some((card) => card.name == event.card.name);
        },
        logTarget: "player",
        content() {
      ...
      effect: {
        ...
        async content(event, trigger, player) {
          ...
          if (control == "选项一") {
            ...
            while (
              target.isAlive() &&
              cards.some((card) => player.hasUseTarget(card))
            ) {
  ...
  twzhengshi: {
    audio: 3,
    logAudio: (index) =>
      typeof index === "number" ? "twzhengshi" + index + ".mp3" : 1,
    derivation: ["twjunsi"],
    forced: true,
    locked: false,
    trigger: {
      ...
      change: {
        ...
        filter(event, player) {
          ...
          return event.name == "die"
            ? event.player.hasSkill("twjunsi")
            : game.roundNumber == 1;
        },
        async cost(event, trigger, player) {
          event.result = await player
            .chooseTarget(
              `争適：令一名角色〖隽嗣〗的摸牌数或弃牌数+1或-1`,
              (card, player, target) => {
        ...
        async content(event, trigger, player) {
          ...
          const result = await player
            .chooseButton(
              [
                ...
                [
                  list.slice(0, 2).map((item, index) => [index, item]),
                  "tdnodes",
                ],
                [
                  list.slice(2, 4).map((item, index) => [index + 2, item]),
                  "tdnodes",
                ],
              ],
              true,
            ...
            .set("filterButton", function (button) {
              ...
              if (num % 2 == 0) {
                return (
                  get.event().target.getStorage("twjunsi")[num / 2 - 1] > 0
                );
              }
              return true;
            })
          ...
          target.storage.twjunsi[Math.ceil(num / 2) - 1] +=
            num % 2 == 1 ? 1 : -1;
  ...
  twjunsi: {
    ...
    subSkill: {
      source: {
        ...
        filter(event, player) {
          const bool = !game.hasPlayer(
            (target) => target != player && target.hasSkill("twjunsi"),
          )
            ? true
            : event.player.hasSkill("twjunsi");
          if (bool) {
        ...
        filter(event, player) {
          ...
          const bool = !game.hasPlayer(
            ...
            ? true
            : event.source.hasSkill("twjunsi");
          if (bool) {
            return (
              player.getStorage("twjunsi")[1] > 0 &&
      ...
      damage: {
        ...
        filter(event, player) {
          return (
            event.notLink() &&
            event?.card?.name == "sha" &&
            event.getParent("useCard").targets?.includes(event.player)
          );
        },
        logTarget: "player",
        content() {
          const num = game
            .getGlobalHistory("everything", (evt) => {
              return (
                evt.name == "useCard" && evt?.card?.name == "sha" && evt.targets
              );
            })
            .reduce((list, evt) => {
  ...
  twmiyong: {
    ...
    filterCard: (card, player) =>
      ...
      !player.getStorage("twmiyong_effect").includes(card),
      ...
      effect: {
        ...
        getSha(event, player) {
          ...
          const cards = event.cards.filter(
            (card) => card.storage.twmiyong_effect,
          );
          if (!cards.length) {
            return gain;
          }
          ...
          game.checkGlobalHistory("cardMove", (evt) => {
            ...
            if (
              ...
              ["lose", "loseAsync"].includes(evt.name)
            ) {
              if (
          ...
          gain.addArray(
            cards.filter((card) => {
              return [use, respond, discard].some(
                (list) =>
                  list
                    .filter((evt) => evt.cards.includes(card))
                    .indexOf(event) == 0,
              );
            }),
          );
  ...
  twjinglve: {
    ...
    filter(event, player) {
      ...
      return game.hasPlayer((target) =>
        lib.skill.twjinglve.filterTarget(null, player, target),
      );
    },
    filterTarget(card, player, target) {
  ...
  twjinglve3: {
    ...
    filter(event, player) {
      ...
      return (
        get.cardPile(card, "filed") ||
        game.hasPlayer((target) => target.getCards("h").includes(card))
      );
    },
    forced: true,
    logTarget: "player",
    async content(event, trigger, player) {
      if (trigger.name == "useCard") {
        ...
        await player.gain(
          card,
          ...(get.owner(card) ? [get.owner(card), "give"] : ["gain2"]),
        );
      }
      player.removeSkill("twjinglve2");
    },
  ...
  twsbxingshang: {
    ...
    async cost(event, trigger, player) {
      if (trigger.name != "die") {
        event.result = await player
          .chooseTarget(
            `###${get.prompt(event.skill)}###将一名角色区域里的牌数弃置至与其体力值相同`,
            (card, player, target) => {
      ...
      do {
        const prompt1 =
          ...
          prompt2 =
            give_map.size > 0
              ? `行殇：将${get.translation(cards)}分配给一名角色`
              : `行殇：是否令一名角色获得${get.translation(cards)}？`;
        const result =
          cards.length > 1
            ? await player
                .chooseButtonTarget({
        ...
        .chooseButton([
          ...
          [
            [
              [
                "fengyin",
                `令其非锁定技失效并获得${get.poptip("twyuanbu")}直到其下个回合结束`,
              ],
  ...
  twsbsongwei: {
    ...
    zhuSkill: true,
    getPhases() {
      let evts = game.getAllGlobalHistory(
        "everything",
        (evt) => evt.name == "phase",
      );
      const evt = evts
    ...
    async content(event, trigger, player) {
      ...
      const num = Math.min(
        3,
        game.countPlayer((current) =>
          evts.every((evt) => evt.player != current),
        ),
      );
      await player.draw(num);
    },
  ...
  twxingshang: {
    ...
    getList: [
      {
        ...
        filter: () =>
          game.hasPlayer(
            (target) => target.isLinked() || target.isTurnedOver(),
          ),
        filterTarget: {
          filterTarget: (card, player, target) =>
            target.isLinked() || target.isTurnedOver(),
        },
      ...
      },
      {
        ...
        prompt: () =>
          "令一名角色摸" +
          get.cnNumber(Math.min(5, Math.max(2, game.dead.length))) +
          "张牌",
        filter: () => true,
        filterTarget: {
          filterTarget: true,
          ...
          result: {
            player(player, target) {
              return (
                ...
                Math.min(5, Math.max(2, game.dead.length))
              );
            },
          },
        },
      },
      {
        cost: 5,
        prompt: () =>
          "令一名体力上限小于10的角色回复1点体力，增加1点体力上限，随机恢复一个废除的装备栏",
        ...
        async content(player, target) {
          ...
          let list = Array.from({ length: 13 }).map(
            (_, i) => "equip" + parseFloat(i + 1),
          );
      ...
      },
      {
        ...
        filter: () =>
          game.dead.some((target) =>
            target
              .getStockSkills(true, true)
              .some((i) => get.info(i) && !get.info(i).charlotte),
          ),
        filterTarget: {
          filterTarget(card, player, target) {
            if (!target.isDead()) {
        ...
        async content(player, target) {
          await player.changeSkills(
            target
              .getStockSkills(true, true)
              .filter((skill) => get.info(skill) && !get.info(skill).charlotte),
          ...
          result: {
            player(player, target) {
              return ["name", "name1", "name2"].reduce((sum, name) => {
                if (
                  !target[name] ||
                  !lib.character[target[name]] ||
                  (name == "name1" && target.name1 == target.name)
                ) {
                  return sum;
                }
    ...
    filter(event, player) {
      return get.info("twxingshang").getList.some((effect) => {
        return (
          player.countMark("twxingshang") >= effect.cost &&
          effect.filter(player)
        );
      });
    },
    usable: 2,
        ...
        return Math.max(
          ...game
            ...
            .map((target) => {
              game.broadcastAll(
                (effect) =>
                  (lib.skill["twxingshang_aiSkill"].ai = effect.ai || {}),
                effect,
              );
    ...
    subSkill: {
      ...
      gain: {
        ...
        filter(event, player) {
          if (
            player.countMark("twxingshang") >= get.info("twxingshang").getLimit
          ) {
            return false;
          }
          return (
            event.name == "die" ||
            !player.getHistory("custom", (evt) => evt.twxingshang).length
          );
        },
        forced: true,
        locked: false,
        async content(event, trigger, player) {
          player.addMark(
            ...
            Math.min(
              2,
              get.info("twxingshang").getLimit -
                player.countMark("twxingshang"),
            ),
          );
    ...
    getList: [
      {
        ...
        filter: (player) =>
          ...
          game.hasPlayer(
            (target) =>
              target != player &&
              !target.getStorage("twfangzhu_ban").includes("basic"),
          ),
        filterTarget: {
          filterTarget: (card, player, target) =>
            target != player &&
      ...
      },
      {
        ...
        filter: (player) =>
          game.hasPlayer(
            (target) =>
              target != player &&
              !target.getStorage("twfangzhu_ban").includes("trick"),
          ),
        filterTarget: {
          filterTarget: (card, player, target) =>
            target != player &&
      ...
      },
      {
        ...
        filter: (player) =>
          ...
          game.hasPlayer(
            (target) =>
              target != player &&
              !target.getStorage("twfangzhu_ban").includes("equip"),
          ),
        filterTarget: {
          filterTarget: (card, player, target) =>
            target != player &&
        ...
        ai: {
          result: {
            target(player, target) {
              return (
                -target
                  .getSkills(null, false)
                  .filter((i) => get.info(i) && !get.info(i).charlotte).length *
        ...
        mod: {
          cardEnabled(card, player) {
            ...
            if (
              ...
              (storage.length > 1 || !storage.includes(get.type2(card)))
            ) {
              return false;
            }
          },
  ...
  twsongwei: {
    ...
    filter(event, player) {
      ...
      return game.hasPlayer(
        (target) => target.group == "wei" && target != player,
      );
    },
    zhuSkill: true,
    forced: true,
    ...
    subSkill: {
      delete: {
        ...
        filter(event, player) {
          ...
          return game.hasPlayer((target) =>
            lib.skill.twsongwei.subSkill.delete.filterTarget(
              null,
              player,
              target,
            ),
        ...
        filterTarget(card, player, target) {
          return (
            target != player &&
            target.group == "wei" &&
            target.getStockSkills(false, true).length
          );
        },
        skillAnimation: true,
        animationColor: "thunder",
    ...
    async content(event, trigger, player) {
      ...
      var result = await player
        ...
        .set("ai", function (button) {
          if (
            ui.selected.buttons.length >= Math.max(3, game.countPlayer() / 2)
          ) {
            return 0;
          }
    ...
    subSkill: {
      kanpo: {
        ...
        filter(event, player) {
          return (
            event.player != player &&
            player.storage.twkanpo[1].includes(event.card.name)
          );
        },
        prompt2(event, player) {
          return (
          ...
          if (effect < 0) {
            if (event.card.name == "sha") {
              ...
              if (target == player) {
                ...
                return (
                  target.hp == 1 ||
                  (target.countCards("h") <= 2 && target.hp <= 2)
                );
              }
            } else {
              return true;
      ...
      if (
        [player, target].some((i) =>
          i.getCards("ej").some((card) => {
            ...
            return game.hasPlayer(
              (current) => current != player && target.canUse(juedou, current),
            );
          }),
        )
      ) {
              ...
              [
                ...
                [
                  [
                    ...
                    [
                      "huogong",
                      `${get.translation(player)}视为对你使用一张【火攻】`,
                    ],
                  ],
                  "textbutton",
                ],
      ...
      if (links[0] === "huogong") {
        ...
        const { links: cards } = await target
          ...
          .set("filterButton", (button) => {
            ...
            const juedou = get.autoViewAs({ name: "juedou" }, [card]);
            return game.hasPlayer(
              (current) => current != target && player.canUse(juedou, current),
            );
          })
          .forResult();
        await target
  ...
  twchihui: {
    ...
    async cost(event, trigger, player) {
      ...
      const { bool, links } = await player
        ...
        .set("filterButton", (button) => {
          ...
          if (
            Boolean(ui.selected.buttons.length) ==
            (typeof link == "number")
          ) {
            return false;
          }
          if (ui.selected.buttons.length) {
            return (
              link == "equip" || target.countDiscardableCards(player, "hej")
            );
          }
          return true;
        })
                    ...
                    game.hasPlayer(
                      (current) =>
                        (get.realAttitude || get.attitude)(player, current) <
                          0 && get.distance(player, current) > 1,
                    ) &&
                    !bool
                  ) {
                    return 0;
    ...
    async content(event, trigger, player) {
      ...
      if (links[1] == "discard") {
        ...
        const equip = get.cardPile2(
          (card) => get.subtype(card) == `equip${links[0]}`,
        );
        if (equip) {
  ...
  twhuangzhu: {
    ...
    filter(event, player) {
      ...
      return (
        player.getStorage("twhuangzhu_effect").length &&
    ...
    async cost(event, trigger, player) {
      if (trigger.name == "phaseZhunbei") {
        ...
        const { control } = await player
          ...
          .set(
            "prompt2",
            "从牌堆或弃牌堆中随机获得一张对应副类别的装备牌，并记录其牌名",
          )
          .set("ai", () => {
            return get
              .event()
        ...
        const storage2 = player
          ...
          .map((equip) => equip[2]);
        let virtualList = {};
        let disabled = [1, 2, 3, 4, 5]
          .filter((num) => player.countDisabledSlot(num))
          .map((num) => "equip" + num);
        ...
        let chooseList = [];
        chooseList.push(
          '###煌烛###<div class="text center">为至多两个已废除的装备栏选择或替换牌名</div>',
        );
        for (let i of disabled) {
          ...
          let equips = storage
            .slice()
            .filter((name) => get.subtypes(name).includes(i));
        ...
        const { bool, links } = await player
          ...
          .set("filterButton", (button) => {
            ...
            if (ui.selected.buttons.length) {
              if (
                get.subtype(ui.selected.buttons[0].link[2]) ==
                get.subtype(button.link[2])
              ) {
                return false;
              }
            }
          ...
          .set("ai", (button) =>
            get.equipValue({ name: button.link[2] }, get.player()),
          )
          .forResult();
        event.result = {
          bool: bool,
      ...
      if (trigger.name == "phaseZhunbei") {
        ...
        player.unmarkAuto(
          ...
          player
            .getStorage(equip)
            .filter((name) =>
              subtypes.some((t) => get.subtypes(name[2]).includes(t)),
            ),
        );
      ...
      equip: {
          ...
          globalFrom(from, to, distance) {
            return (
              ...
              from
                ...
                .reduce(
                  (sum, name) =>
                    sum + (lib.card[name[2]]?.distance?.globalFrom || 0),
                  0,
                )
            );
          },
          globalTo(from, to, distance) {
            return (
              ...
              to
                ...
                .reduce(
                  (sum, name) =>
                    sum + (lib.card[name[2]]?.distance?.globalTo || 0),
                  0,
                )
            );
          },
          attackRange(from, distance) {
            return (
              ...
              from
                ...
                .reduce(
                  (sum, name) =>
                    sum + (lib.card[name[2]]?.distance?.attackFrom || 0),
                  0,
                )
            );
          },
          attackTo(from, to, distance) {
            return (
              ...
              to
                ...
                .reduce(
                  (sum, name) =>
                    sum + (lib.card[name[2]]?.distance?.attackTo || 0),
                  0,
                )
            );
          },
        ...
        filter(event, player) {
          ...
          return player
            .getStorage("twhuangzhu_equip")
            .some((name) =>
              event.slots.some((t) => get.subtypes(name[2]).includes(t)),
            );
        },
        forced: true,
        popup: false,
        content() {
          ...
          player.unmarkAuto(
            ...
            player
              ...
              .filter((name) =>
                trigger.slots.some((t) => get.subtypes(name[2]).includes(t)),
              ),
          );
  ...
  twliyuan: {
    ...
    filter(event, player) {
      return player.countCards("hes", (card) =>
        get.subtypes(card).some((i) => player.hasDisabledSlot(i)),
      );
    },
    position: "hes",
    precontent() {
  ...
  twjifa: {
    ...
    async content(event, trigger, player) {
      ...
      await player.changeSkills(
        ["twchihui", "twfuxi"],
        ["twhuangzhu", "twliyuan", "twjifa"].remove(control),
      );
    },
  },
  //幻刘封
  twchenxun: {
    ...
    filter(event, player) {
      return game.hasPlayer((current) =>
        player.canUse({ name: "juedou", isCard: true }, current),
      );
    },
    async cost(event, trigger, player) {
      event.result = await player
        ...
        .set("ai", (target) => {
          ...
          let eff = get.effect(target, { name: "juedou" }, player, player),
            shas =
              target.mayHaveSha(player, "respond", null, "count") -
              player.mayHaveSha(player, "respond", null, "count"),
            att = get.attitude(player, target);
          if (shas > 0) {
    ...
    async content(event, trigger, player) {
      const target = event.targets[0],
        juedou = get.autoViewAs({
          name: "juedou",
          isCard: true,
          storage: { twchenxun: true },
        });
  ...
  twzenhui: {
    ...
    filter(event, player) {
      ...
      if (
        card.name != "sha" &&
        (get.type(card, null, false) != "trick" ||
          ...
          player
            ...
            .set("ai", function () {
              ...
              return (target.countGainableCards(player, "hej")
                ? get.effect(target, { name: "shunshou" }, player, player)
                : 0) > get.effect(target, trigger.card, player, player)
                ? 1
                : 0;
            });
        }
  ...
  twdanji: {
    ...
    filter(event, player) {
      ...
      if (zhu && zhu.isZhu) {
        if (
          lib.translate[zhu.name].indexOf("刘备") != -1 ||
          (zhu.name2 && lib.translate[zhu.name2].indexOf("刘备") != -1)
        ) {
          return false;
        }
      }
  ...
  twxingbu: {
    ...
    content() {
      ...
      var num = 0,
        list = [
          ...
          [
            "扶匡东柱",
            "出牌阶段使用的第一张牌结算完成后，弃置一张牌并摸两张牌",
          ],
          [
            "五星连珠",
            "摸牌阶段多摸两张牌，出牌阶段使用【杀】的次数+1，跳过弃牌阶段",
          ],
        ];
      for (var i of cards) {
      ...
      player
        ...
        .set("ai", function (target) {
          ...
          switch (num) {
            case 1:
              return (
                -get.sgn(att) * target.countCards("hs", { name: "sha" }) - 1
              );
            case 2:
              return att;
            case 3:
    ...
    subSkill: {
      ...
      effect2: {
        ...
        marktext: "匡",
        intro: {
          content: "出牌阶段使用的第一张牌结算完成后，弃置#张牌并摸#*2张牌",
        },
        audio: "xingbu",
        trigger: { player: "useCardAfter" },
        filter(event, player) {
        ...
        content() {
          ...
          player.chooseToDiscard(
            player.countMark("twxingbu_effect2"),
            "he",
            true,
          );
          ("step 1");
      ...
      effect3: {
        ...
        intro: {
          content:
            "摸牌阶段多摸#*2张牌，出牌阶段使用【杀】的次数+#，跳过弃牌阶段",
        },
        mod: {
          cardUsable(card, player, num) {
            if (card.name == "sha") {
  ...
  twlifeng: {
    ...
    filterTarget(card, player, target) {
      ...
      let cards = ui.selected.cards,
        num = Math.abs(
          get.number(cards[0], player) - get.number(cards[1], player),
        );
  ...
  old_twlifeng: {
    ...
    filter(event, player) {
      ...
      return player.countCards(
        "he",
        (card) => get.number(card, player) != get.number(cardx, player),
      );
    },
    filterCard(card, player) {
      if (ui.selected.cards.length) {
        return (
          get.number(card, player) != get.number(ui.selected.cards[0], player)
        );
      }
      return true;
    },
  ...
  twniwo: {
    ...
    async cost(event, trigger, player) {
      event.result = await player
        ...
        .set("ai", (target) => {
          return (
            -get.attitude(get.player(), target) / (target.countCards("h") + 1)
          );
        })
        .forResult();
    },
    async content(event, trigger, player) {
      ...
      const result = await player
        ...
        .set("filterOk", () => {
          ...
          return (
            buttons.filter((i) => get.owner(i.link) == get.player()).length *
              2 ==
            buttons.length
          );
        })
        .set(
          ...
          (function () {
            ...
            return result.concat(
              target.getCards("h").randomGets(result.length),
            );
          })(),
        )
        .set("ai", (button) => {
  ...
  twguihan: {
    ...
    filter(event, player) {
      return game.hasPlayer((target) =>
        lib.skill.twguihan.filterTarget(null, player, target),
      );
    },
    filterTarget(card, player, target) {
    ...
    async content(event, trigger, player) {
      ...
      const { index } = await player
        ...
        .set("choiceList", [
          ...
          "获得牌堆顶第" + get.cnNumber(num) + "张牌下的两张牌",
        ])
        .set("prompt", "归汉：请选择一项")
        .set("ai", () => {
          const player = get.player(),
  ...
  twrenxian: {
    ...
    filter(event, player) {
      return player.countCards(
        "h",
        (card) => get.name(card) !== "shan" && get.type(card) === "basic",
      );
    },
    usable: 1,
    filterTarget: lib.filter.notMe,
    filterCard: (card) =>
      get.name(card) !== "shan" && get.type(card) === "basic",
    selectCard: -1,
    lose: false,
    discard: false,
    delay: false,
    ...
    async content(event, trigger, player) {
      ...
      target.addTempSkill("twrenxian_phase", {
        player: "twrenxian_phaseAfter",
      });
    ...
    mod: {
      aiOrder(player, card, num) {
        ...
        if (
          ...
          cards.includes(card) &&
          game.hasPlayer(
            (target) => target != player && get.attitude(player, target) > 0,
          )
        ) {
          return 0;
        }
            ...
            if (
              [card]
                ...
                .some(
                  (c) =>
                    get.itemtype(c) === "card" &&
                    !player.getStorage("twrenxian_mark").includes(c),
                )
            ) {
              return false;
            }
  ...
  twciyin: {
    ...
    filter(event, player) {
      ...
      return (
        player == target ||
        player.getStorage("beOfOneHeartWith").includes(target)
      );
    },
    async content(event, trigger, player) {
      ...
      const gain = cards.filter((card) =>
        ["heart", "spade"].includes(get.suit(card)),
      );
      if (!gain.length) {
        return;
      }
      ...
      heart: {
        ...
        filter(event, player) {
          ...
          const history = game.getAllGlobalHistory(
            "everything",
            (evt) => evt.name == "twciyin_heart" && evt.player == player,
          );
          const limit = history.map((evt) => evt.cost_data).flat();
          return (
            !limit.includes("选项一") ||
            (!limit.includes("选项二") && player.countCards("h") < player.maxHp)
          );
        },
        getIndex(event, player) {
          ...
          const limit = history.map((evt) => evt.cost_data).flat();
          return (
            Math.floor(player.getExpansions("twciyin").length / 3) -
            limit.length
          );
        },
        async cost(event, trigger, player) {
          ...
          if (!limit.includes("选项一")) {
            ...
            choiceList[0] =
              '<span style="opacity:0.5;">' + choiceList[0] + "</span>";
          }
          if (
            !limit.includes("选项二") &&
            ...
            choiceList[1] =
              '<span style="opacity:0.5;">' + choiceList[1] + "</span>";
          }
          const control =
            choices.length == 1
              ? choices[0]
    ...
    async content(event, trigger, player) {
      ...
      if (_status.characterlist) {
        ...
        list = get.charactersOL(
          (name) => !["shu", "qun"].includes(lib.character[name][1]),
        );
      } else {
        list = get.gainableCharacters((info) =>
          ["shu", "qun"].includes(info[1]),
        );
      }
      ...
      const filter = (skill) => {
        ...
        return (
          ...
          !info.dutySkill &&
          ["【杀】", "【闪】"].some((str) =>
            get.plainText(translation).includes(str),
          )
        );
      };
    ...
    subSkill: {
      record: {
        ...
        async cost(event, trigger, player) {
          ...
          const { bool, links } = await player
            ...
            .set("filterButton", (button) => {
              return (
                !ui.selected.buttons.some(
                  (buttonx) => buttonx.link[2] == "sha",
                ) || button.link[2] != "sha"
              );
            })
            .set("ai", (button) => {
        ...
        async content(event, trigger, player) {
          const names = event.cost_data.map((link) => [
            { name: link[2], nature: link[3] },
          ]);
        ...
        async content(event, trigger, player) {
          ...
          while (storage.length) {
            const name = storage.shift(),
              card = get.autoViewAs({
                name: name[0].name,
                nature: name[0].nature,
                isCard: true,
              });
            if (
              (!get.info(card).notarget ||
      ...
      huan: {
        ...
        filter(event, player, name) {
          ...
          return (
            name == "useCardAfter" ||
            (name == "useCard1" && event.addCount !== false)
          );
        },
        forced: true,
        async content(event, trigger, player) {
          if (event.triggername == "useCard1") {
            ...
            player.unmarkAuto(
              ...
              player
                .getStorage("twbeiding")
                .filter((i) => i[0].name == trigger.card.name),
            );
          }
        },
      },
      ...
      lose: {
        ...
        filter(event, player) {
          ...
          return !player.hasHistory("useCard", (evt) =>
            evt.targets?.some((i) => i == event.player),
          );
        },
        forced: true,
        async content(event, trigger, player) {
    ...
    subSkill: {
      buff: {
        ...
        async content(event, trigger, player) {
          ...
          game.log(
            player,
            "防止此次了" + (trigger.name == "damage" ? "伤害" : "失去体力"),
          );
        },
        ai: {
          nofire: true,
  ...
  twhuanji: {
    ...
    chooseButton: {
      ...
      filter(button) {
        return (
          !ui.selected.buttons.some((buttonx) => buttonx.link[2] == "sha") ||
          button.link[2] != "sha"
        );
      },
      check(button, player) {
  ...
  twqinghan: {
    ...
    filter(event, player) {
      ...
      return (
        ...
        game.hasPlayer((current) => player.canCompare(current, true))
      );
    },
    filterTarget(card, player, target) {
    ...
    async content(event, trigger, player) {
      ...
      if (result.bool && cards.length) {
        const { links } = await player
          .chooseButton([
            `请选择你要对${get.translation(target)}使用的牌`,
            [cards, "vcard"],
          ])
          .set("ai", (button) => {
            const player = get.player(),
  ...
  twzhihuan: {
    ...
    async cost(event, trigger, player) {
      ...
      const choices = [];
      const choiceList = [
        `获得${get.translation(target)}装备区一张牌`,
        `获得并使用一张牌堆或弃牌堆中空置装备栏对应类别的装备牌`,
      ];
        ...
        for (let i = 1; i < 7; i++) {
          if (player.hasEmptySlot(i)) {
            const sub = "equip" + i,
              equip = get.cardPile(
                (card) =>
                  get.subtype(card, false) == sub && player.hasUseTarget(card),
              );
            if (equip) {
              player.$gain2(equip);
              await game.delayx();
  ...
  rexianyuan: {
      ...
      if (!target.countCards("h")) {
        ...
        choice = (
          await player
            ...
            .set("ai", () =>
              get.attitude(get.player(), get.event().getTrigger().player) > 0
                ? 1
                : 0,
            )
            .forResult()
      ...
      if (choice == 0) {
        const result = await player
          .choosePlayerCard(
            ...
            [1, num],
            true,
            '###仙援###<div class="text center">将其中至多' +
              get.cnNumber(num) +
              "张牌置于牌堆顶（先选择的在上）</div>",
          )
          .forResult();
    ...
    subSkill: {
      give: {
        ...
        filter(event, player) {
          return (
            player.hasMark("rexianyuan") &&
            game.hasPlayer((i) =>
              lib.skill.rexianyuan.subSkill.give.filterTarget(null, player, i),
            )
          );
        },
        filterTarget(card, player, target) {
          return (
            target != player &&
            target.countMark("rexianyuan") < lib.skill.rexianyuan.limit
          );
        },
        ...
        async content(event, trigger, player) {
          ...
          const gives = Array.from({
            length: player.countMark("rexianyuan"),
          ...
          if (gives.length == 1) {
            ...
            give = (
              await player
                ...
                .set(
                  "prompt",
                  "仙援：将任意枚“仙援”标记分配给" + get.translation(target),
                )
                .forResult()
            ).index;
          }
      ...
      gain: {
        ...
        content() {
          player.addMark(
            ...
            Math.min(
              2,
              lib.skill.rexianyuan.limit - player.countMark("rexianyuan"),
            ),
          );
        },
      },
    ...
    filter(event, player) {
      return (
        ...
        game.hasPlayer((target) =>
          lib.skill.twxianyuan.filterTarget(null, player, target),
        )
      );
    },
    filterTarget(card, player, target) {
      ...
      effect: {
        ...
        filter(event, player) {
          return event.player.hasCard(
            (card) => card.hasGaintag("twxianyuan_" + player.playerid),
            "h",
          );
        },
        prompt2(event, player) {
          const num =
            event.player.storage["twxianyuan_effect"][player.playerid];
          return (
            "观看其手牌并将其中至多" +
            get.cnNumber(num) +
            "张牌以任意顺序置于牌堆顶"
        ...
        async content(event, trigger, player) {
          ...
          const result = await player
            ...
            .set("processAI", (list) => {
              ...
              return [
                cards.slice(sgn > 0 ? 1 : num, cards.length),
                cards.slice(0, sgn > 0 ? 1 : num),
              ];
            })
            .forResult();
          if (result.bool) {
            ...
            game.log(
              target,
              "的" + get.cnNumber(cards.length) + "张牌被置入了",
              "#y牌堆顶",
            );
          ...
          const sum = targets.reduce(
            (num, target) =>
              ...
              target.countCards("h", (card) =>
                card.hasGaintag("twxianyuan_" + player.playerid),
              ),
            0,
          );
          for (const target of targets) {
          ...
          if (target?.isIn()) {
            const num = Math.min(
              ...
              player.countCards(
                "h",
                (card) =>
                  card.hasGaintag("twkuiduan_card") ||
                  get.name(card, player) == "sha",
              ),
            );
            if (num > 0) {
      ...
      damage: {
        ...
        filter(event, player) {
          return player.hasUseTarget(
            get.autoViewAs({ name: "juedou", isCard: true }),
            false,
            false,
          );
        },
  ...
  twjiezhan: {
    ...
    check(event, player) {
      ...
      if (
        player.hasSkill("twlongjin", null, null, false) &&
        !player.awakenedSkills.includes("twlongjin")
      ) {
        return true;
      }
      if (
        ...
        (player.countCards("hs", "sha") &&
          player.hasSkill("ollongdan", null, null, false))
      ) {
        return true;
      }
      return (
        get.effect(player, { name: "draw" }, player, player) +
        get.effect(event.player, { name: "sha" }, event.player, player)
      );
    },
    logTarget: "player",
    async content(event, trigger, player) {
    ...
    subSkill: {
      buff: {
        ...
        filter(event, player, name) {
          return (
            name == "phaseBefore" ||
            (name == "phaseAfter" && !player.countMark("twlongjin_buff"))
          );
        },
        forced: true,
        popup: false,
        ...
        async content(event, trigger, player) {
          ...
          if (
            event.triggername == "phaseAfter" &&
            !player.countMark(event.name)
          ) {
      ...
      event.result = await player
        .chooseTarget(
          ...
          `选择一名角色，然后你进行判定，并令其执行相应效果` +
            (history.length > 0
              ? `（上次选择的目标：${get.translation(history.at(-1).targets[0])}`
              : ``) +
            (judgeEvent.length > 0
              ? `；上次判定的结果：${get.translation(judgeEvent.at(-1).result.color)}）`
              : ``),
        )
        .set("ai", (target) => {
          const player = get.player(),
            att = get.attitude(player, target);
          if (
            player.getAllHistory("useSkill", (evt) => evt.skill == "twzongquan")
              .length
          ) {
  ...
  twguimou: {
    ...
    async content(event, trigger, player) {
      ...
      next.set("processAI", function (list) {
        const player = get.player(),
          target =
            get.event().getTrigger().name == "phaseZhunbei"
              ? player
              : player.next;
    ...
    subSkill: {
      buff: {
        ...
        async cost(event, trigger, player) {
          event.result = await player
            .chooseTarget(
              "令一名本回合未以此法选择的角色摸一张牌，然后其可以将此杀转移给自己",
              (card, player, target) => {
  ...
  twpiankuang: {
    ...
    filter(event, player) {
      ...
      return (
        event.card?.name == "sha" &&
        event.getParent().type == "card" &&
        player.hasHistory("sourceDamage", (evt) => evt.card?.name == "sha")
      );
    },
    forced: true,
    async content(event, trigger, player) {
  ...
  twshouzhu: {
    ...
    async content(event, trigger, player) {
      ...
      for (const current of targets) {
        ...
        game.log(
          current,
          "将" + get.cnNumber(bottom.length) + "张牌置于牌堆底",
        );
        if (discard.length) {
  ...
  beOfOneHeart: {
    ...
    async content(event, trigger, player) {
      ...
      const { targets } = await player
        .chooseTarget("请选择你的“同心”角色", lib.filter.notMe, [
          1,
          beOfOneHeartLimit,
        ])
        .set("ai", function (target) {
    ...
    async cost(event, trigger, player) {
      ...
      event.result = await player
        ...
        .set("ai", (target) => {
          ...
          return (
            get.attitude(player, target) *
            (player === target && player.needsToDiscard(1) ? 0.4 : 1)
          );
        })
        .forResult();
    },
  ...
  twcairu: {
    ...
    filter(event, player) {
      return ["huogong", "tiesuo", "wuzhong"].some((name) => {
        if (
          player.getStorage("twcairu_used").filter((namex) => namex == name)
            .length > 1
        ) {
          return false;
        }
    ...
    chooseButton: {
      ...
      prompt(links, player) {
        return (
          "将两张颜色不同的牌当【" + get.translation(links[0][2]) + "】使用"
        );
      },
    },
    subSkill: { backup: {} },
  ...
  xunde: {
    ...
    check(event, player) {
      return (
        get.attitude(player, event.player) > 0 &&
        (!event.source || get.attitude(player, event.source) < 0)
      );
    },
    content() {
      "step 0";
              ...
              for (const target of targets) {
                const historyx =
                  target.actionHistory[
                    target.actionHistory.length -
                      (player.actionHistory.length - i)
                  ];
                if (!historyx) {
                  continue;
                }
  ...
  twylyanshi: {
    ...
    enable: ["chooseToUse", "chooseToRespond"],
    filter(event, player) {
      return ["juedou", "binglinchengxiax"]
        .concat(get.zhinangs())
        .some((name) => {
    ...
    chooseButton: {
      ...
      prompt(links, player) {
        return (
          "将一张【杀】当作" + "【" + get.translation(links[0][2]) + "】使用"
        );
      },
    },
    subSkill: { backup: {} },
    hiddenCard(player, name) {
      if (
        ...
        !player.countCards(
          "hs",
          (card) => _status.connectMode || get.name(card) == "sha",
        )
      ) {
        return false;
      }
      ...
      player
        ...
        .step(async (event, trigger, player) => {
          ...
          for (const i of targets) {
            if (
              i.isIn() &&
              i.hasCard((card) => card.hasGaintag("twjuexing"), "h")
            ) {
              list.push([
                i,
                i.getCards("h", (card) => card.hasGaintag("twjuexing")),
              ]);
            }
          }
          if (list.length) {
            await game
              .loseAsync({ lose_list: list })
          ...
          if (listx.length) {
            await game
              .loseAsync({ gain_list: listx, animate: "draw" })
              .setContent("gaincardMultiple");
          }
  ...
  twxiayong: {
    ...
    filter(event, player) {
      ...
      if (
        evt &&
        evt.targets &&
        (event.player != player || player.countCards("h") > 0)
      ) {
    ...
    logAudio(event, player) {
      return (
        "twxiayong" +
        (lib.skill.twxiayong.audioname.includes(player.name)
          ? "_" + player.name
          : "") +
  ...
  twqiaosi: {
    ...
    check(event, player) {
      ...
      if (
        ...
        cards.some(
          (card) =>
            get.name(card, player) == "tao" || get.name(card, player) == "jiu",
        )
      ) {
        return true;
      }
    ...
    getCards(player) {
      ...
      for (const target of targets) {
        ...
        const history = target.getHistory(
          "lose",
          (evt) => evt.position == ui.discardPile,
        );
        if (history.length) {
  ...
  twbaizu: {
    ...
    async cost(event, trigger, player) {
      ...
      let targets = game.filterPlayer((target) =>
        filterTarget(null, player, target),
      );
  ...
  twhuajing: {
    ...
    intro: {
      markcount(storage, player) {
        return lib.skill.twhuajing.derivation.filter((skill) =>
    ...
    subSkill: {
      ...
      use: {
        ...
        enable: "phaseUse",
        filter(event, player) {
          return lib.skill.twhuajing.derivation.some((skill) =>
            player.hasMark(skill),
          );
        },
        filterCard: true,
        ...
        usable: 1,
        prompt:
          "展示至多四张手牌，然后根据这些牌含有的花色数于本回合获得等量你拥有的“武”标记的效果",
        async content(event, trigger, player) {
          await player.showCards(
            event.cards,
            get.translation(player) + "发动了【化境】",
          );
          const skills = lib.skill.twhuajing.derivation.filter((skill) =>
            player.hasMark(skill),
          );
          const gainSkills = skills.randomGets(
            Math.min(
              skills.length,
              event.cards.reduce(
                (list, cardx) => list.add(get.suit(cardx, player)),
                [],
              ).length,
            ),
          );
      ...
      blocker: {
        ...
        async content(event, trigger, player) {
          ...
          player.disableSkill(
            "twhuajing_blocker",
            lib.skill.twhuajing.getSkills(player),
          );
        },
  ...
  twdengjian: {
    ...
    filter(event, player) {
      return (
        event.player != player &&
        lib.skill.twdengjian.getCards(player, event.player).length
      );
    },
      ...
      let cards = target
        .getHistory("useCard", (evt) => {
          return (
            ...
            evt.cards
              .filterInD("d")
              .some((card) => get.name(card, false) == "sha") &&
        ...
        .reduce(
          (list, evt) =>
            list.addArray(
              evt.cards
                .filterInD("d")
                .filter((card) => get.name(card, false) == "sha"),
            ),
          [],
        );
      if (cards.length) {
        ...
        for (let i = history.length - 1; i >= 0; i--) {
          for (let evt of history[i].gain) {
            if (evt.getParent().name == "twdengjian") {
              ...
              cards = cards.filter(
                (cardx) => get.color(cardx) != get.color(card),
              );
              if (!cards.length) {
                break;
              }
    ...
    async content(event, trigger, player) {
      ...
      /*const {bool}=await player.chooseToDiscard(get.prompt('twdengjian'),'he')
			...
			.forResult();
			if(bool) */ await player
        .gain(cards.randomGet(), "gain2")
        .gaintag.add("twdengjianx");
    },
      ...
      if (player.hasSkill("twxinshou_0") && player.hasSkill("twxinshou_1")) {
        const { bool, targets } = await player
          .chooseTarget((card, player, target) => {
            return (
              target != player &&
              !target.hasSkill("twdengjian", null, null, false)
            );
          })
          .set("ai", (target) => {
            const player = get.event().player;
        ...
        if (bool) {
          ...
          target.addAdditionalSkills(
            "twxinshou_" + player.playerid,
            "twdengjian",
          );
          player.popup("登剑");
          target.popup("登剑");
        ...
        if (control == "给牌") {
          ...
          const { bool, targets } = await player
            ...
            .set("ai", (target) => {
              ...
              if (cards.length) {
                cards.sort(
                  (a, b) => get.value(a, player) - get.value(b, player),
                );
  ...
  twjieqiu: {
    ...
    filter(event, player) {
      return game.hasPlayer((target) =>
        lib.skill.twjieqiu.filterTarget(null, player, target),
      );
    },
    filterTarget(card, player, target) {
    ...
    ai: {
      ...
      result: {
        target(player, target) {
          return (
            -target.countCards("e") - (get.attitude(player, target) < 0 ? 1 : 0)
          );
        },
      },
    },
    ...
    subSkill: {
      ...
      buff: {
        ...
        async content(event, trigger, player) {
          ...
          for (const target of targets) {
            const result = await target
              .chooseBool("劫囚：是否执行一个额外回合？")
              .set("frequentSkill", "twjieqiu")
              .forResult();
            if (result.bool) {
              target.popup("劫囚");
  ...
  twenchou: {
    ...
    filterTarget(card, player, target) {
      return (
        target != player && target.countCards("h") && target.hasDisabledSlot()
      );
    },
    usable: 1,
    async content(event, trigger, player) {
  ...
  twzhongyi: {
    ...
    filter(event, player) {
      ...
      return player.getHistory(
        "sourceDamage",
        (evt) => evt.card && evt.card == event.card,
      ).length;
    },
    forced: true,
    async content(event, trigger, player) {
      ...
      const num2 =
        1 + player.getAllHistory("custom", (evt) => evt.twzhongyi).length;
      let choice = ["摸牌"],
      ...
      return (
        ...
        game.hasPlayer(
          (target) =>
            ...
            player.canUse(event.card, target),
        )
      );
    },
    ...
    ai: {
      ...
      result: {
        target(player, target) {
          ...
          if (
            Math.min(13, hs[0].number + player.getDamagedHp()) > ts[0].number
          ) {
            return -get.damageEffect(target, player, player);
          }
          return 0;
  ...
  twhuzhong: {
    ...
    content() {
      ...
      var choiceList = [
        ...
        "弃置" +
          get.translation(target) +
          "一张手牌，若此【杀】造成伤害，则你本阶段可以额外使用一张【杀】",
      ];
      if (target.countCards("h")) {
        list.unshift("其弃置");
      } else {
  ...
  twfenwang: {
    ...
    content() {
      ...
      if (event.triggername == "damageBegin2") {
        ...
        player
          .chooseToDiscard("h", "弃置一张手牌，或令此伤害+1")
          .set("ai", function (card) {
            return 8 - get.value(card);
          });
      }
  ...
  twchengxi: {
    ...
    filter(event, player) {
      return game.hasPlayer((target) =>
        lib.skill.twchengxi.filterTarget(null, player, target),
      );
    },
    filterTarget(card, player, target) {
      if (
        player.getStorage("twchengxi_used").includes(target) ||
        target == player
      ) {
        return false;
      }
    ...
    async content(event, trigger, player) {
      ...
      if (!player.storage.twchengxi_used) {
        player
          .when("phaseUseAfter")
          .step(async () => delete player.storage.twchengxi_used);
      }
  ...
  twshenyi: {
    ...
    async cost(event, trigger, player) {
      ...
      const list = get.inpileVCardList((info) => {
        return (
          ...
          !player.getStorage(event.skill).includes(info[2])
        );
      });
      const dialog = [
      ...
      const { bool, links } = await player
        ...
        .set("ai", (button) => {
          ...
          if (
            ["tao", "jiu", "caochuan", "wuxie"].includes(name) &&
            get.event().getRand() > 0.4
          ) {
            return value * 2;
          }
          return value;
    ...
    async content(event, trigger, player) {
      ...
      const card = get.cardPile2(
        (card) =>
          get.name(card, false) == name && get.nature(card, false) == nature,
      );
      if (card) {
        ...
        const card = get.cardPile2(
          (card) => get.type2(card) == get.type2(name),
        );
        if (card) {
          const next = player.addToExpansion([card], "gain2");
          next.gaintag.add(skillName);
      ...
      if (target != player && player.countCards("h")) {
        ...
        const next = player
          .chooseToGive(
            target,
            `伸义：是否将任意张手牌交给${get.translation(target)}？`,
      ...
      draw: {
          ...
          return game
            .filterPlayer((target) => {
              ...
              return Object.values(evt.gaintag_map)
                .flat()
                .includes(`twshenyi_${player.playerid}`);
            })
            .sortBySeat();
        },
        ...
        async content(event, trigger, player) {
          ...
          const num = Object.values(evt.gaintag_map)
            ...
            .reduce(
              (numx, tag) =>
                numx + (tag.includes(`twshenyi_${player.playerid}`) ? 1 : 0),
              0,
            );
          if (num) {
            await player.draw(num);
  ...
  twquanqian: {
    ...
    filter(event, player) {
      return (
        event.player != player &&
        game.hasPlayer((current) => current.countCards("h") > 0)
      );
    },
    check(event, player) {
      if (get.attitude(player, event.player) > 0) {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player
        .chooseToGive(
          ...
          [1, Infinity],
          "allowChooseAll",
          `是否交给${get.translation(target)}任意张牌？`,
        )
        ...
        .set("ai", (card) => {
          ...
          if (
            ui.selected.cards.every(
              (cardx) => get.type2(cardx) != get.type2(card),
            )
          ) {
      ...
      sunben: {
        ...
        async content(event, trigger, player) {
          ...
          if (!player.countMark(event.name)) {
            player.removeSkill(event.name);
            if (
              player.hasSkill("twquanqian", null, null, false) &&
              !player.hasSkill("twquanqian")
            ) {
              player.popup("劝迁");
  ...
  twrouke: {
    ...
    check(event, player) {
      const getD = (current) =>
        get.effect(current, { name: "draw" }, player, player);
  ...
  old_twquanqian: {
    ...
    async content(event, trigger, player) {
      ...
      if (result2?.control) {
        await player.gain(
          target.getCards("h", { suit: result2.control }),
          target,
          "give",
        );
      }
    ...
    subSkill: {
      sunben: {
        ...
        content() {
          "step 0";
          player.addMark(
            "old_twquanqian_sunben",
            trigger.getl(player).hs.length,
            false,
          );
          ("step 1");
          if (player.countMark("old_twquanqian_sunben") >= 6) {
            player.removeSkill("old_twquanqian_sunben");
            if (
              player.hasSkill("old_twquanqian", null, null, false) &&
              !player.hasSkill("old_twquanqian")
            ) {
              player.popup("劝迁");
              player.restoreSkill("old_twquanqian");
              game.log(player, "恢复了技能", "#g【劝迁】");
            }
          }
        },
  ...
  twlijian: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player
        .chooseToMove(
          "力谏：请分配" + get.translation(target) + "和你获得的牌",
          true,
          "allowChooseAll",
        )
        .set("list", [
        ...
        .set("processAI", function (list) {
          ...
          switch (get.sgn(att)) {
            ...
            case -1:
              var num =
                Math.ceil(cards.length / 2) + (cards.length % 2 == 0 ? 1 : 0);
    ...
    subSkill: {
      sunben: {
        ...
        content() {
          ...
          ("step 1");
          if (
            player.countMark("twlijian_sunben") >=
            player.countMark("twlijian_sunben_limit")
          ) {
            player.removeSkill("twlijian_sunben");
            if (
              player.hasSkill("twlijian", null, null, false) &&
              !player.hasSkill("twlijian")
            ) {
              player.popup("力谏");
        ...
        filter(event, player, name, card) {
          ...
          return player.hasHistory("lose", (evt) => {
            ...
            return tags?.some((tag) => {
              return (
                tag.startsWith("twchungang_") &&
                tag.slice(11) !== player.playerid
              );
            });
          });
        },
  ...
  old_twchungang: {
    ...
    subSkill: {
      global: {
        ...
        filter(event, player) {
          return !game.hasPlayer(
            (i) => i.hasSkill("old_twchungang", null, null, false),
            true,
          );
        },
        silent: true,
        ...
        ai: {
          effect: {
            target(card, player, target) {
              if (
                (get.tag(card, "gain") || 0) < 2 &&
                (get.tag(card, "draw") || 0) < 2
              ) {
                return;
              }
  ...
  twpolu: {
    ...
    async cost(event, trigger, player) {
      const num =
        player.getAllHistory("useSkill", (evt) => evt.skill == event.skill)
          .length + 1;
      event.result = await player
        .chooseTarget(
          [1, Infinity],
          get.prompt(event.skill),
          `令任意名角色摸${get.cnNumber(num)}张牌`,
        )
        .set("forceDie", true)
        .set("ai", (target) => {
          const player = get.player();
    ...
    subSkill: {
      global: {
        ...
        filter(event, player) {
          if (
            event.type != "wuxie" ||
            (player.group != "wei" && player.group != "qun")
          ) {
            return false;
          }
  ...
  twniju: {
    ...
    async content(event, trigger, player) {
      ...
      const lose_list = trigger.lose_list
        .slice()
        .sort((a, b) => lib.sort.seat(a[0], b[0]));
      dialog.push(
    ...
    subSkill: {
      change: {
        ...
        filter(event, player) {
          ...
          if (
            (player !== event.player || event.iwhile) &&
            player !== event.target
          ) {
            return false;
          }
          return event.lose_list.some((list) => {
            ...
            return (
              list[0] === player && storage.some((s) => cards.includes(s[2]))
            );
          });
        },
        charlotte: true,
    ...
    mod: {
      playerEnabled(card, player, target) {
        ...
        if (
          ...
          player.storage.twchongwangx.includes(target)
        ) {
          return false;
        }
      },
      targetEnabled(card, player, target) {
        ...
        if (
          get.tag(card, "damage") > 0 &&
          target.storage.twchongwangx.includes(player)
        ) {
          return false;
        }
      },
  ...
  twxianfeng: {
    ...
    check(event, player) {
      ...
      let cnt = game.countPlayer(
        (current) => get.distance(player, current) === 2,
      );
      if (
        cnt > 2 ||
        (cnt === 2 && Math.abs(att) < 2) ||
        (cnt && Math.abs(att) < 1)
      ) {
        return true;
      }
    ...
    async content(event, trigger, player) {
      ...
      if (result?.bool) {
        if (!target.countGainableCards(player, "he")) {
          ...
          const result2 = await player
            .gainPlayerCard(target, "he", [1, 2], true)
            .forResult();
  ...
  twduoren: {
    ...
    prompt2(event, player) {
      ...
      for (const i of player.getSkills(null, false, false).filter((skill) => {
        ...
        return (
          info &&
          !info.charlotte &&
          !player.getStockSkills(true, true).includes(skill)
        );
      })) {
    ...
    filter(event, player) {
      return (
        event.player != player &&
        (player.hasAllHistory(
          "sourceDamage",
          (evt) => evt.player == event.player,
        ) ||
          player.hasAllHistory(
            "damage",
            (evt) => evt.source == event.player,
          )) &&
        !event.reserveOut
      );
    },
    ...
    subSkill: {
      hurt: {
        ...
        async content(event, trigger, player) {
          ...
          if (
            trigger.source?.isIn() &&
            trigger.source.countMark("twyanshi_mark") < 3
          ) {
            const target = trigger.source;
  ...
  twrenchou: {
    ...
    filter(event, player, name) {
      if (name == "die") {
        return (
          event.player == player &&
          player.getStorage("twyanshi").some((i) => i.isIn())
        );
      }
  ...
  twliexi: {
    ...
    chooseAi: (event, player) => {
      ...
      game.countPlayer((cur) => {
        ...
        can =
          eff -
          allcards.slice(0, cur.hp + 1).reduce((acc, val) => acc + val[0], 0);
        if (!wq.length) {
          //没武器只能凑数砸的
          if (can > 0) {
            targets.push([
              cur,
              can,
              allcards.slice(0, cur.hp + 1).map((i) => i[1]),
            ]);
          }
          return false;
        }
      ...
      if (targets.length) {
        player
          ...
          .set("ai", (target) => {
            return Math.max(
              0.1,
              target.hp - _status.event.player.countCards("h"),
            );
          })
          .set("targets", targets);
      } else {
      ...
      want: {
        ...
        chooseButton: {
          ...
          check(event, player) {
            ...
            targets.sort(
              (a, b) => get.attitude(player, b) - get.attitude(player, a),
            );
                ...
                if (bool) {
                  ...
                  if (cards.some((card) => get[fn](card, player) == type)) {
                    const card = get.cardPile(
                      (cardx) => get[fn](cardx, target) != type,
                    );
                    if (card) {
  ...
  twbingde: {
    ...
    filter(event, player) {
      return (
        player.countCards("he") &&
        player.getStorage("twbingde_clear").length < 4
      );
    },
    onChooseToUse(event) {
    ...
    chooseButton: {
      ...
      check(event, player) {
        ...
        const suit = lib.suit
          .filter(
            (suit) =>
              !player.getStorage("twbingde_clear").includes(suit) &&
              player.hasCard((card) => get.suit(card) == suit, "he"),
          )
          .sort((a, b) => {
            return map[b] - map[a];
          })[0];
      ...
      backup(result, player) {
        return {
          ...
          async content(event, trigger, player) {
            ...
            if (
              get.suit(event.cards[0], player) == suit &&
              player.getStat("skill").twbingde
            ) {
  ...
  twqingtao: {
    ...
    filter(event, player) {
      ...
      return (
        ...
        !player.hasHistory("custom", (evt) => evt.twqingtao)
      );
    },
    async cost(event, trigger, player) {
      event.result = await player
    ...
    async content(event, trigger, player) {
      ...
      if (
        get.name(cards[0]) == "jiu" ||
        get.type(cards[0], false, player) != "basic"
      ) {
        await player.draw();
      }
    },
    ...
    subSkill: {
      extra: {
        ...
        filter(event, player) {
          return (
            event.source &&
            event.source.hasSkill("twxiongjun") &&
            event.source != player
          );
        },
        logTarget: "source",
  ...
  baonvezhi: {
    ...
    change(player, num) {
      ...
      player[num > 0 ? "addMark" : "removeMark"](
        "baonvezhi",
        Math.abs(num),
        false,
      );
      ...
      player[player.countMark("baonvezhi") > 0 ? "markSkill" : "unmarkSkill"](
        "baonvezhi",
      );
    },
    filter(event, player) {
  ...
  twxiongjun: {
    ...
    content() {
      var targets = game
        .filterPlayer((current) => current.hasSkill("twxiongjun"))
        .sortBySeat();
      player.line(targets, "green");
  ...
  twxiongxi: {
    ...
    selectCard() {
      return (
        (lib.skill.baonvezhi.baonvezhi_max || 5) -
        _status.event.player.countMark("baonvezhi")
      );
    },
    check(card) {
      return 8 - get.value(card);
    ...
    ai: {
      ...
      result: {
        player(player, target) {
          ...
          if (
            player.hasSkill("twxiongjun") &&
            !player.storage.counttrigger?.twxiongjun
          ) {
            num += game.countPlayer((current) => {
  ...
  twjichou: {
    ...
    chooseButton: {
      ...
      backup(links, player) {
        ...
        var backup = get.copy(
          lib.skill["twjichou_" + (isUse ? "use" : "give")],
        );
        if (isUse) {
  ...
  twjilun: {
    ...
    async cost(event, trigger, player) {
      const num = Math.min(
        Math.max(1, player.getStorage("twjichou").length),
        5,
      );
      const list = player
        .getStorage("twjichou")
      ...
      if (list.length) {
        const result = await player
          ...
          .set("ai", (button) => {
            ...
            if (Array.isArray(link)) {
              if (
                player.getUseValue({ name: link[2], nature: link[3] }) >
                4 * val
              ) {
                return 1;
              }
        ...
        event.result = await player
          .chooseBool(get.prompt(event.skill), `摸${get.cnNumber(num)}张牌`)
          .forResult();
  ...
  twkunsi: {
    ...
    content() {
      ...
      player.useCard({ name: "sha", isCard: true }, target, false).animate =
        false;
      ("step 1");
      if (
      ...
      result: {
        target(player, target) {
          if (
            ...
            !target.mayHaveShan(player, "use") &&
            get.effect(target, { name: "sha", isCard: true }, player, player) >
              0
          ) {
            return -1;
          } else if (
  ...
  twxiongzheng: {
    ...
    filter(event, player) {
      return game.hasPlayer(
        (target) => !player.getStorage("twxiongzheng").includes(target),
      );
    },
    async cost(event, trigger, player) {
      event.result = await player
    ...
    subSkill: {
      ...
      effect: {
        ...
        filter(event, player) {
          ...
          return game.hasPlayer(
            (target) =>
              player.getStorage("twxiongzheng_mark").includes(target) ||
        ...
        async cost(event, trigger, player) {
          ...
          const list2 = game.filterPlayer(
            (target) =>
              !list.includes(target) && player.canUse(sha, target, false),
          );
          let choiceList = [
            [
              "sha",
              `视为对任意名本轮未对${get.translation(target)}造成过伤害的角色依次使用一张【杀】`,
            ],
            [
              "draw",
      ...
      result: {
        player(player, target) {
          ...
          return (
            ...
            Math.min(
              2,
              targetx.countCards("h", { name: "sha" }) + map[targetx.playerid],
            )
          );
        },
      },
  ...
  twyimou: {
    ...
    async content(event, trigger, player) {
      ...
      if (player.countCards("h") && player !== target) {
        ...
        choiceList[2] =
          '<span style="opacity:0.5">' + choiceList[2] + "</span>";
      }
      const { control } = await player
        .chooseControl(list)
        .set("prompt", "毅谋：请选择一项")
    ...
    ai: {
      ...
      skillTagFilter(player, tag, arg) {
        ...
        for (var i = 0; i < ui.cardPile.childNodes.length; i++) {
          var card = ui.cardPile.childNodes[i];
          if (
            get.type(card, null, false) == "basic" &&
            !list.includes(card.name)
          ) {
            list.push(card.name);
          }
        }
      ...
      if (result.control == "选项一") {
        ...
        target.discard(
          target.getCards("he", (card) => {
            return (
              event.cardType.includes(get.type2(card)) && !cards.includes(card)
            );
          }),
        );
      }
    ...
    subSkill: {
      damage: {
        ...
        filter(event, player) {
          ...
          return (
            evt.name == "useCard" && history[history.indexOf(evt) - 1].twxuhe
          );
        },
        content() {
          if (trigger.name != "useCard") {
  ...
  twjiexun: {
    ...
    content() {
      ...
      var str = lib.suit
        .map((suit) => {
          return (
            get.translation(suit) + "：" + get.cnNumber(suits[suit] || 0) + "张"
          );
        })
        .join("；");
      player
        ...
        .set(
          "prompt2",
          get.skillInfoTranslation("twjiexun", player, false) + "<br>" + str,
        );
      ("step 1");
      if (result.control != "cancel2") {
      ...
      if (
        ...
        player
          ...
          .set(
            "choice",
            event.num2 >= 4 ? 0 : event.num2 <= 1 ? 1 : [0, 1].randomGet(),
          );
      } else {
        event.finish();
      }
  ...
  twluanlve: {
    ...
    viewAsFilter(player) {
      return (
        player.isPhaseUsing() &&
        player.countCards("hs", { name: "sha" }) >=
          player.countMark("twluanlve")
      );
    },
    filterCard(card, player) {
    ...
    mod: {
      playerEnabled(card, player, target) {
        if (
          !_status.event.twluanlve_ban ||
          !Array.isArray(_status.event.twluanlve_ban)
        ) {
          return;
        }
        if (
            ...
            const result =
              cards.length > 1
                ? await player
                    .chooseButtonTarget({
                      createDialog: [
                        `攸业：请选择要分配的牌${boolx ? `（至少分给${get.translation(_status.currentPhase)}一张）` : ""}`,
                        cards,
                      ],
                      selectButton: [1, Infinity],
                      forced: true,
  ...
  twweipo: {
    ...
    content() {
      ...
      player
        .chooseButton(
          ["危迫：令其获得一张智囊牌或【兵临城下】", [list, "vcard"]],
          true,
        )
        .set("ai", function (button) {
          return _status.event
            .getParent()
            .target.getUseValue({ name: button.link[2] });
        });
      ("step 2");
      if (result.bool) {
    ...
    ai: {
      ...
      result: {
        target(player, target) {
          ...
          return (
            (target.countCards("he") + 0.5) * Math.sqrt(Math.max(1, target.hp))
          );
        },
      },
    },
  ...
  twshangyi: {
    ...
    async content(event, trigger, player) {
      ...
      chooseButton.set("filterButton", function (button) {
        ...
        if (
          !ui.selected.buttons.length &&
          get.owner(button.link) != _status.event.target
        ) {
          return false;
        }
        if (
      ...
      if (result.bool) {
        if (result.links.length == 1) {
          const cards = (
            await target.modedDiscard(result.links, player).forResult()
          ).cards;
          ...
          if (
            get.color(card1, player) === "red" &&
            get.color(card2, target) === "red"
          ) {
            await player.draw();
          }
        }
  ...
  twgyshenxing: {
    ...
    filter(event, player) {
      return (
        player.countCards("he") >=
        Math.min(2, player.countMark("twgyshenxing_used"))
      );
    },
    selectCard() {
    ...
    prompt() {
      return (
        "弃置" +
        get.cnNumber(
          Math.min(2, _status.event.player.countMark("twgyshenxing_used")),
        ) +
        "张牌并摸一张牌"
      );
    },
  ...
  twbingyi: {
    ...
    content() {
      ...
      if (lib.skill.twbingyi.filterx(trigger, player)) {
        ...
        player.chooseBool(get.prompt("twbingyi"), "展示所有手牌").ai =
          function () {
            return false;
          };
      }
  ...
  twfenming: {
    ...
    filter(event, player) {
      return game.hasPlayer(function (target) {
        return (
          target != player && (target.countCards("he") || !target.isLinked())
        );
      });
    },
    direct: true,
    content() {
      ...
      player
        .chooseTarget(
          get.prompt2("twfenming"),
          function (card, player, target) {
            return (
              target != player &&
              (target.countCards("he") || !target.isLinked())
            );
          },
        )
        .set("ai", function (target) {
    ...
    mod: {
      cardUsable(card, player) {
        if (card.name == "sha") {
          ...
          if (
            suit === "unsure" ||
            player.getStorage("twgongji2").includes(suit)
          ) {
            return Infinity;
          }
        }
  ...
  twshuangren: {
    ...
    content() {
      ...
      if (result.bool) {
        ...
        target
          .chooseBool(
            "双刃：是否视为对" + get.translation(player) + "使用一张杀？",
          )
          .set(
            "choice",
  ...
  twenyuan1: {
    ...
    async cost(event, trigger, player) {
      ...
      if (
        (!target.countCards("h") || !target.countCards("e")) &&
        target.isDamaged()
      ) {
        list.push("回复1点体力");
        prompt2 += "或回复1点体力";
  ...
  twenyuan2: {
    ...
    sourceSkill: "twenyuan",
    prompt2: (event) =>
      `令${get.translation(event.source)}选择一项：1.失去1点体力；2.交给你一张手牌，若此牌的花色不为♥，你摸一张牌。`,
    async content(event, trigger, player) {
  ...
  twqianxi2: {
    ...
    mod: {
      cardEnabled2(card, player) {
        if (
          get.itemtype(card) == "card" &&
          player.getStorage("twqianxi2").includes(get.color(card)) &&
          get.position(card) == "h"
        ) {
          return false;
        }
  ...
  twqianxi3: {
    mod: {
      cardEnabled2(card, player) {
        if (
          get.itemtype(card) == "card" &&
          !player.getStorage("twqianxi3").includes(get.color(card)) &&
          get.position(card) == "h"
        ) {
          return false;
        }
  ...
  twzhengrong: {
    ...
    content() {
      ...
      if (result.bool) {
        player
          .addToExpansion(result.links, target, "give")
          .gaintag.add("twzhengrong");
      }
    },
    marktext: "荣",
    onremove(player, skill) {
  ...
  twhongju: {
    ...
    content() {
      ...
      if (player.countCards("h") == 0) {
        ...
        next.set("processAI", function (list) {
          var player = _status.event.player,
            ...
            cards2 = cards.splice(
              0,
              player.getExpansions("twzhengrong").length,
            );
          return [cards2, cards];
        });
      }
      ...
      if (result.bool) {
        ...
        player
          .addToExpansion(pushs, player, "giveAuto")
          .gaintag.add("twzhengrong");
    ...
    content() {
      ...
      player
        ...
        .set(
          ...
          player.needsToDiscard() ||
            player.getExpansions("twxingwu").length > 1,
        );
      ("step 1");
      if (result.bool) {
      ...
      player
        ...
        .set(
          "goon",
          game.hasPlayer(
            (current) => get.damageEffect(current, player, player) < 0,
          ),
        );
      ("step 4");
      if (result.bool) {
    ...
    content() {
      ...
      player
        .chooseTarget(get.prompt2("twxuewei"), function (card, player, target) {
          return (
            target != player && target != _status.event.getTrigger().player
          );
        })
        .set("ai", function (target) {
  ...
  twmingren: {
    ...
    subSkill: {
      change: {
        ...
        filter(event, player) {
          return (
            player.countCards("he") && player.getExpansions("twmingren").length
          );
        },
        async cost(event, trigger, player) {
          event.result = await player
  ...
  twzhenliang: {
    ...
    filter(event, player) {
      ...
      if (event.name == "chooseToUse") {
        if (
          player.storage.twzhenliang ||
          player.hasSkill("twzhenliang_used", null, null, false)
        ) {
          return false;
        }
        ...
        return (
          (event.num > 0 && event.player == player) ||
    ...
    content() {
      ...
      player
        .chooseTarget(
          ...
          function (card, player, target) {
            return (
              target != player && player.canUse("sha", target, false, false)
            );
          },
        )
        .set("ai", function (target) {
          var player = _status.event.player;
          if (
            player.hp + player.countCards("hs", { name: ["tao", "jiu"] }) <=
            1
          ) {
            return -1;
          }
  ...
  twfupan: {
    ...
    async content(event, trigger, player) {
      ...
      if (
        ...
        !game.hasPlayer((current) => {
          return (
            !(player.storage.twfupan[current.playerid] >= 2) &&
            player != current
          );
        })
      ) {
      ...
      const next = player.chooseCardTarget({
        ...
        filterTarget(card, player, target) {
          return (
            !(player.storage.twfupan[target.playerid] >= 2) && player != target
          );
        },
        ai1(card) {
          const player = get.player();
      ...
      next.set(
        ...
        next.targetprompt2.concat([
          (target) => {
            ...
            return !evt.player.storage.twfupan[target.playerid]
              ? "你摸两张牌"
              : "可对其<br>造成伤害";
          },
        ]),
    ...
    subSkill: {
      ...
      gain: {
        ...
        content() {
          ...
          target
            .chooseButton(
              ["选择获得一张“生”", player.getExpansions("jutu")],
              true,
            )
            .set("ai", function (button) {
  ...
  twruilian: {
    ...
    content() {
      "step 0";
      player
        .chooseTarget(get.prompt2("twruilian"))
        .set("ai", function (target) {
          var player = _status.event.player,
            att = get.attitude(player, target),
  ...
  twruilian2: {
    ...
    content() {
      ...
      player
        ...
        .set(
          "prompt",
          "睿敛：是否与" + get.translation(target) + "各获得一种类型的牌？",
        )
        .set("ai", function () {
          var player = _status.event.player,
          ...
          if (
            player.countCards("he", { type: "equip" }) < 2 &&
            list.includes("equip")
          ) {
            return "equip";
          }
      ...
      lose: {
        ...
        filter(event, player) {
          ...
          if (
            evt &&
            evt.cards2 &&
            evt.cards2.some((i) => get.subtype(i) == "equip1")
          ) {
            return true;
          }
          return false;
  ...
  twtanfeng: {
    ...
    filter(event, player) {
      return game.hasPlayer(function (current) {
        return (
          current != player && current.countDiscardableCards(player, "hej") > 0
        );
      });
    },
    direct: true,
      ...
      player
        ...
        .set("ai", function (target) {
          ...
          if (get.attitude(player, target) > 0) {
            ...
            !target.countCards("he") ||
            !target.canUse("sha", player)
          ) {
            if (
              target.hp + target.countCards("hs", { name: ["tao", "jiu"] }) <=
              1
            ) {
              num = 2;
            } else {
  ...
  twzhibian: {
    ...
    filter(event, player) {
      return game.hasPlayer(
        (current) => current != player && player.canCompare(current),
      );
    },
    direct: true,
    content() {
  ...
  twguoyi: {
    ...
    filter(event, player) {
      if (
        event.target == player ||
        (event.card.storage && event.card.storage.twguoyi)
      ) {
        return false;
      }
      return (
  ...
  twyizhu: {
    ...
    async content(event, trigger, player) {
      ...
      const result =
        hs.length > 2
          ? await player
              .chooseCard("he", true, 2, "选择两张牌洗入牌堆")
              .forResult()
          : {
              bool: true,
              cards: hs,
      ...
      if (result?.bool) {
        ...
        next.insert_index = function () {
          return ui.cardPile.childNodes[
            get.rand(0, Math.min(4, game.countPlayer()) * 2 - 2)
          ];
        };
    ...
    subSkill: {
      use: {
        ...
        content() {
          ...
          if (result.bool) {
            var target =
              result.targets[result.targets[0] == trigger.target ? 1 : 0];
  ...
  twgonghuan: {
    ...
    content() {
      ...
      player
        .damage(
          trigger.source ? trigger.source : "nosource",
          trigger.nature,
          trigger.num,
        )
        .set("card", trigger.card)
  ...
  twwanwei: {
    ...
    check(event, player) {
      return (
        get.attitude(player, event.player) > 0 && event.player.hp < player.hp
      );
    },
    usable: 1,
    logTarget: "player",
    ...
    subSkill: {
      effect: {
        ...
        async content(event, trigger, player) {
          ...
          await player.showCards(
            card,
            get.translation(player) + "挽危：牌堆底的牌",
            true,
          );
          if (player.hasUseTarget(card)) {
            await player.chooseUseTarget(card, true);
        ...
        return {
          ...
          content() {
            ...
            if (
              cards &&
              cards.length &&
              get.name(cards[0], player) == card[2]
            ) {
          ...
          ai: {
            result: {
              target(player, target) {
                ...
                return (
                  att / Math.sqrt(get.distance(player, target, "absolute"))
                );
              },
            },
          },
      ...
      effect: {
        ...
        content() {
          player.chooseToDiscard(
            "he",
            true,
            player.countMark("twchayi_effect"),
          );
  ...
  twkuanji: {
    ...
    async cost(event, trigger, player) {
      const cards =
        trigger.name != "cardsDiscard"
          ? trigger.getd(player, "cards2")
      ...
      const result = await player
        .chooseButtonTarget({
          ...
          ai2(target) {
            ...
            return (
              get.attitude(player, target) *
              get.value(ui.selected.buttons[0].link, target) *
  ...
  twchaofeng: {
    ...
    filter(event, player) {
      ...
      if (
        event.filterCard(
          get.autoViewAs({ name: "sha" }, "unsure"),
          player,
          event,
        )
      ) {
                  ...
                  ui.selected.targets.reduce((acc, p) => {
                    return (
                      ...
                      (player.canUse(card, p, false)
                        ? get.effect(p, card, player, player)
                        : 0) -
                      get.attitude(player, p)
                    );
                  }, 0)
                );
              }
              return (
                (tar.canUse(card, player, false)
                  ? get.effect(player, card, tar, player)
                  : 0) +
                eff -
                5 +
                ui.selected.targets.reduce((acc, p) => {
                  return (
                    acc +
                    (tar.canUse(card, p, false)
                      ? get.effect(p, card, tar, player)
                      : 0) -
                    get.attitude(player, p)
                  );
                }, 0)
    ...
    subSkill: {
      ...
      damage: {
        ...
        filter(event, player, name) {
          if (name == "useCard") {
            return (
              event.card.name == "sha" && player.hasMark("twchuanshu_mark")
            );
          }
        ...
        .set(
          ...
          (function () {
            ...
            var yimie = function () {
              ...
              key.push("caochuan");
              var bool1 =
                get.recoverEffect(target, player, player) > 0 ? 1 : -1;
              var bool2 =
                (att > 0 && !hit) ||
                (target.countCards("hs", { name: key }) &&
                  !trigger.getParent().directHit.includes(target))
  ...
  twsuizheng: {
    ...
    subSkill: {
      ...
      xianfu: {
        ...
        filter(event, player) {
          return (
            player.getStorage("twsuizheng").includes(event.player) &&
            event.player.isIn()
          );
        },
        forced: true,
    ...
    content() {
      ...
      player
        ...
        .set("filterButton", function (button) {
          ...
          if (ui.selected.buttons.length) {
            if (
              list.includes(ui.selected.buttons[0].link) &&
              list.includes(link)
            ) {
              return false;
            }
  ...
  twqianju: {
    ...
    filter(event, player) {
      return (
        get.distance(player, event.player) <= 1 && player.countCards("e") < 5
      );
    },
    forced: true,
    usable: 1,
        ...
        intro: {
          content(shadow) {
            ...
            return shadow
              .map(
                (list) =>
                  `${get.translation(list[0])}：${get.translation(list[1])}`,
              )
              .join("<br>");
          },
        },
        ...
        mod: {
          cardEnabled(card, player) {
            ...
            const hs = player.getCards("h", (card) =>
                suits.includes(get.suit(card, player)),
              ),
              cards = [card];
          ...
          cardRespondable(card, player) {
            return lib.skill.twgongsun_shadow2.mod.cardEnabled.apply(
              this,
              arguments,
            );
          },
  ...
  twzhengjian: {
    ...
    filter(event, player) {
      ...
      return (
        !player.hasSkill("twzhengjian_eff0") &&
        !player.hasSkill("twzhengjian_eff1")
      );
    },
    content() {
      "step 0";
    ...
    subSkill: {
      eff0: {
        ...
        filter(event, player) {
          ...
          return (
            player.storage.twzhengjian || event.player.countCards("he") > 0
          );
        },
        logTarget: "player",
        content() {
          ...
          if (player.storage.twzhengjian) {
            player
              .chooseBool(
                "征建：是否对" + get.translation(target) + "造成1点伤害？",
              )
              .set("ai", () => _status.event.goon)
              .set(
                "goon",
                get.damageEffect(target, player, _status.event.player) > 0,
              );
          } else {
            target.chooseCard(
              "he",
    ...
    content() {
      ...
      if (list.length == 1) {
        ...
        player
          ...
          .set(
            "prompt",
            "选择变更" + get.translation(target) + "一个技能的状态",
          )
  ...
  yuzhang: {
    ...
    content() {
      "step 0";
      var choiceList = [
        "令" + get.translation(trigger.source) + "本回合不能再使用或打出牌",
      ];
      if (trigger.source.countCards("he")) {
    ...
    subSkill: {
      skip: {
        ...
        prompt2(event, player) {
          ...
          str += ["准备", "判定", "摸牌", "出牌", "弃牌", "结束"][
            list.indexOf(event.name)
          ];
          str += "阶段";
          return str;
        },
  ...
  twlihuo2: {
    ...
    filter(event, player) {
      ...
      return game.hasPlayer(function (current) {
        return (
          !event.targets.includes(current) && player.canUse(event.card, current)
        );
      });
    },
    direct: true,
  ...
  twfengji: {
    ...
    content() {
      "step 0";
      player
        .chooseCard("he", get.prompt2("twfengji"))
        .set("ai", function (card) {
          var name = card.name,
            num = 0;
          for (var i = 0; i < ui.cardPile.childNodes.length; i++) {
  ...
  twbudao: {
    ...
    skillValue: {
      ...
      twzuhuo: (target, player) =>
        get.damageEffect(target, player, player) > 0 ? 0.1 : 1,
      twharvestinori: (target) => 0.9 + Math.random() / 5,
      twhuangjin: (target) => Math.random() / 5,
      twguimen: (target) =>
        Math.sqrt(Math.min(3, target.countCards("he", { suit: "spade" }))) *
        0.09,
      twzhouzu: (target) => {
    ...
    content() {
      ...
      player
        ...
        .set(
          ...
          skills.sort(
            (a, b) =>
              (map[b](target, player) || 0.5) - (map[a](target, player) || 0.5),
          )[0],
        );
      ("step 1");
      var skill = result.control;
      ...
      player
        ...
        .set("ai", function () {
          ...
          return (
            Math.max(
              1,
              Math.min(safe, 3, game.countPlayer(), player.getDamagedHp()),
            ) - 1
          );
        });
      ("step 1");
  ...
  twharvestinori: {
    ...
    content() {
      ...
      ("step 1");
      player.storage.twharvestinori_mahou = [
        result.index + 1,
        result.index + 1,
      ];
  ...
  twhuangjin: {
    ...
    filter(event, player) {
      return (
        event.card.name == "sha" && typeof get.number(event.card) == "number"
      );
    },
    content() {
      "step 0";
    ...
    content() {
      ...
      if (result.bool && result.links) {
        ...
        game.log(
          player,
          "选择",
          "#y" + get.translation(event.judgingSpade),
          "进行判定",
        );
        player.judge(function (result) {
      ...
      event.result = await player
        ...
        .set("ai", (card) => {
          ...
          if (attitude == 0 || result == 0) {
            ...
            const checkx = get.color(card, player) == get.color(judging);
            if (checkx > 0) {
              return checkx;
            }
            return 0;
    ...
    filter(event, player) {
      ...
      return (
        ...
        (player.hasSkill(skill, null, false, false) &&
          !lib.skill[skill].derivation.every((i) =>
            player.hasSkill(i, null, false, false),
          ))
      );
    },
    prompt2(event, player) {
    ...
    async content(event, trigger, player) {
      switch (game.roundNumber) {
        ...
        case 2:
          ...
          break;
        default:
          await player.changeSkills(lib.skill[event.name].derivation, [
            event.name,
          ]);
          break;
      }
  ...
  twlingfa_sha: {
    ...
    content() {
      ...
      trigger.player
        ...
        .set(
          "goon",
          get.damageEffect(trigger.player, player, trigger.player) < 0,
        )
        .set("ai", function (card) {
          if (!_status.event.goon) {
            return 0;
    ...
    intro: {
      content:
        "其他角色使用【杀】时，若其有牌，则其需弃置一张牌，否则你对其造成1点伤害。",
    },
  ...
  twlingfa_tao: {
    ...
    intro: {
      content:
        "其他角色使用【桃】结算结束后，若其有牌，则其需交给你一张牌，否则你对其造成1点伤害。",
    },
    sourceSkill: "twlingfa",
  },
  twzhian: {
    ...
    async cost(event, trigger, player) {
      ...
      if ((trigger.cards ?? []).length > 0) {
        ...
        if (lib.filter.canBeDiscarded(card, player, owner)) {
          choices.push("弃置卡牌");
        } else {
        ...
        if (
          player.hasCard(
            (card) => lib.filter.cardDiscardable(card, player, event.skill),
            "h",
          ) &&
      ...
      const result = await player
        ...
        .set("ai", () => {
          ...
          let choices = event.controls.slice();
          choices.sort((a, b) => getEffect(b) - getEffect(a));
          return choices[0];
        })
        .forResult();
  ...
  twyujue: {
    ...
    filter(event, player) {
      ...
      return game.hasPlayer(function (current) {
        ...
        return (
          !current.hasSkill("twyujue_effect0") ||
          !current.hasSkill("twyujue_effect1")
        );
      });
    },
    content() {
        ...
        if (
          game.hasPlayer(function (current) {
            return (
              ...
              current.countDiscardableCards(target, "he") > 0
            );
          })
        ) {
          target
  ...
  twyujue_give: {
      ...
      return [
        ...
        Math.max.apply(
          ...
          targets.map(function (target) {
            ...
            if (
              player.group == "qun" &&
              target.hasZhuSkill("twfengqi", player)
            ) {
              num = 4;
            }
    ...
    filterTarget(card, player, target) {
      ...
      return (
        num - target.countMark("twyujue_clear") >=
        Math.max(1, ui.selected.cards.length)
      );
    },
    selectTarget() {
      var player = _status.event.player;
    ...
    ai: {
      ...
      result: {
        target(player, target) {
          ...
          if (
            player.hasUnknown() &&
            (player.identity != "fan" || !target.isZhu)
          ) {
            return 0;
          }
  ...
  twjieyu: {
    ...
    check(event, player) {
      ...
      if (
        player.countCards("h", "shan") < 2 &&
        player.countCards("h", "tao") < 1
      ) {
        return true;
      }
      return false;
  ...
  twyuanhu: {
      ...
      result: {
        player(player, target) {
          ...
          if (target == player) {
            ...
            var hp = player.hp,
              hs = player.countCards(
                "h",
                (card) => card != ui.selected.cards[0],
              );
            var tp = target.hp,
    ...
    subSkill: {
        ...
        filter(event, player) {
          return (
            player.hasSkill("twyuanhu") &&
            player.hasCard({ type: "equip" }, "eh")
          );
        },
        async cost(event, trigger, player) {
          event.result = await player
  ...
  twjuezhu: {
    ...
    async cost(event, trigger, player) {
      let list = [3, 4]
        .filter((num) => player.hasEmptySlot(num))
        .map((num) => `equip${num}`);
      if (list.length > 1) {
        ...
        const { bool, targets, links } = await player
          .chooseButtonTarget({
            createDialog: [
              prompt,
              [list.map((i) => [i, get.translation(i)]), "tdnodes"],
            ],
            filterTarget(card, player, target) {
  ...
  twmouzhu: {
    ...
    async contentBefore(event, trigger, player) {
      ...
      const list = game.filterPlayer(function (current) {
        return (
          current != player && current != target && current.hp <= player.hp
        );
      });
      if (!list.length) {
        await player.loseHp();
    ...
    async contentAfter(event, trigger, player) {
      ...
      if (!num) {
        ...
        let list = ["sha", "juedou"].filter((name) =>
            player.canUse(get.autoViewAs({ name }, []), target, false),
          ),
          result;
        if (!list.length) {
          return;
          ...
          result = await target
            ...
            .set(
              "prompt",
              "谋诛：视为被" + get.translation(player) + "使用一张…",
            )
    ...
    content() {
      if (!lib.inpile.includes("meiyingqiang")) {
        ...
        const card = get.cardPile(function (card) {
          return (
            card.name == "meiyingqiang" && !player.getEquips(1).includes(card)
          );
        }, "field");
        if (card) {
          player.equip(card);
    ...
    checkx(event, player) {
      ...
      for (let tar of event.targets) {
        ...
        if (
          ...
          mayDamage +=
            hitOdds * get.damageEffect(tar, event.player, player, nature);
        }
      }
      if (damageBonus) {
        return damageBonus;
      ...
      return (
        mayDamage + (1 - odds) * get.damageEffect(event.player, player, player)
      );
    },
    async cost(event, trigger, player) {
      event.result = await player
        .chooseToDiscard(
          ...
          "he",
          get.prompt(event.skill, trigger.player),
          `弃置一张牌并令${get.translation(trigger.player)}使用的${get.translation(trigger.card)}伤害+1，但若其未造成伤害，则你对其造成1点伤害。`,
        )
        .set("ai", function (card) {
        ...
        .set(
          ...
          (() => {
            const num =
              (lib.skill.cuijin.checkx(trigger, player) *
                player.countCards("he")) /
              10;
            return num;
          })(),
  ...
  manji: {
    ...
    filter(event, player) {
      return (
        player != event.player &&
        (player.hp >= event.player.hp || player.isDamaged())
      );
    },
    logTarget: "player",
    content() {
  ...
  beini: {
    ...
    content() {
      ...
      if (
        list[1].isIn() &&
        list[0].isIn() &&
        list[1].canUse("sha", list[0], false)
      ) {
  ...
  twmiaolve: {
    ...
    async content(event, trigger, player) {
      if (trigger.name == "damage") {
        ...
        const list = _status.dz_mantianguohai_suits
          .randomRemove(2)
          .map((i) => game.createCard2("dz_mantianguohai", i, 5));
        if (list.length) {
  ...
  gx_taijifuchen: {
    ...
    async content(event, trigger, player) {
      ...
      if (lib.suit.includes(suit)) {
        next.set(
          "prompt2",
          `若弃置的是${get.translation(suit)}牌，则改为${get.translation(player)}获得之`,
        );
        next.set("suit", suit);
      }
      const result = await next.forResult();
      if (result?.bool && result?.cards?.length) {
        ...
        if (
          get.suit(cardx, target) == get.suit(card, false) &&
          get.position(cardx) == "d"
        ) {
      ...
      effect: {
        ...
        filter(event, player) {
          if (
            !event.card?.name ||
            !player
              .getStorage("gx_chongyingshenfu_effect")
              .includes(event.card.name)
          ) {
            return false;
          }
  ...
  twdanfa: {
    ...
    filter(event, player) {
      return (
        event.cards?.some(
          (card) => get.owner(card) == player || !get.owner(card),
        ) &&
        !player
          .getExpansions("twdanfa")
          .some((card) => get.suit(card) == get.suit(event.card))
      );
    },
    async content(event, trigger, player) {
      await player
    ...
    chooseButton: {
      ...
      check(button) {
        ...
        const suits = get
          ...
          .getHistory("lose", (evt) => {
            return (
              evt.getParent().name == "discard" &&
              evt.getParent(2).skill == "twlingbao_backup"
            );
          })
        ...
        return {
          ...
          async content(event, trigger, player) {
            ...
            if (colors.length == 1 && colors[0] == "red") {
              ...
              if (result?.targets?.length) {
                ...
                while (gain.length < 2) {
                  const card = get.cardPile(
                    (cardx) =>
                      get.type(cardx) == "basic" && !gain.includes(cardx),
                  );
                  if (card) {
                    gain.push(card);
                  } else {
            ...
            if (
              ...
              colors[0] == "black" &&
              game.hasPlayer((target) =>
                target.countDiscardableCards(player, "hej"),
              )
            ) {
              const result = await player
                .chooseTarget(
                  `灵宝：你弃置一名角色至多两个不同区域的共计至多两张牌`,
                  true,
                  (card, player, target) => {
                    return target.countDiscardableCards(player, "hej");
                  },
            ...
            if (colors.length > 1) {
              const canDiscard = game.hasPlayer((target) =>
                target.countDiscardableCards(target, "hej"),
              );
              const result = await player
                .chooseTarget(
                  `灵宝：你令一名角色摸两张牌` +
                    (canDiscard ? `，另一名角色弃置一张牌` : ``),
                  true,
                  (card, player, target) => {
                    if (!ui.selected.targets.length) {
                      return true;
        ...
        const result = await player
          .chooseButton(
            [
              "请选择你的初始法宝",
              [
                ["gx_lingbaoxianhu", "gx_taijifuchen", "gx_chongyingshenfu"],
                "vcard",
              ],
            ],
            true,
  ...
  twliancai: {
    ...
    content() {
      ...
      player.chooseTarget(
        "获得一名角色装备区内的一张牌",
        lib.skill.twliancai.filterTarget,
  ...
  chijie: {
    ...
    getGroups(player) {
      return lib.group.filter((group) => {
        return (
          group != player.group &&
          game.hasPlayer((current) => current.group == group)
        );
      });
    },
    trigger: {
    ...
    filter(event, player) {
      return (
        (event.name != "phase" || game.phaseNumber == 0) &&
        get.info("chijie").getGroups(player).length
      );
    },
    async cost(event, trigger, player) {
  ...
  waishi: {
    ...
    filterTarget(card, player, target) {
      return (
        target != player && target.countCards("h") >= ui.selected.cards.length
      );
    },
    filterCard: true,
    position: "he",
    ...
    selectCard() {
      ...
      return [
        1,
        Math.min(ui.selected.targets[0].countCards("h"), game.countGroup()),
      ];
    },
    discard: false,
    lose: false,
    ...
    async content(event, trigger, player) {
      const { cards, target } = event;
      const { links } = await player
        .choosePlayerCard(target, true, "h", cards.length)
        .forResult();
      if (!links || !links.length) {
        return;
      }
  ...
  cangjia: {
    ...
    filter(event, player, name) {
      ...
      return (
        cards?.length &&
        cards.some(
          (card) => !player.getStorage("cangjia").includes(get.suit(card)),
        )
      );
    },
    intro: { content: "已记录花色：$" },
    ...
    subSkill: {
      defend: {
        ...
        async content(event, trigger, player) {
          ...
          if (
            !source.hasCards("he", (card) =>
              lib.filter.cardDiscardable(card, source, event.skill),
            )
          ) {
            trigger.getParent()?.excluded.add(player);
          } else {
  ...
  duohui: {
    ...
    subSkill: {
      global: {
        ...
        async cost(event, trigger, player) {
          const targets = game.filterPlayer(
            (current) => current != player && current.hasSkill("duohui"),
          );
          event.result = await player
            .chooseCardTarget({
        ...
        async content(event, trigger, player) {
          ...
          const result = await target
            .chooseToGive(
              player,
              `交给${get.translation(player)}另一张${get.translation(suit)}牌，否则其摸一张牌`,
              "he",
            )
            .set("filterCard", (card) => {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player
        .chooseButton(
          [
            "跃渊：移去一个花色",
            [list.map((suit) => ["", "", `lukai_${suit}`]), "vcard"],
          ],
          true,
        )
        .set("ai", (button) => {
  ...
  zhengfu: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await target
        .chooseCard(
          "he",
          `交给${get.translation(player)}一张${get.translation(type)}牌，否则此【杀】不可被闪避`,
          (card) => {
            return get.type2(card) == get.event().type;
          },
        )
