...
const dynamicTranslates = {
	...
	dcliexiang(player, skill) {
		...
		if (player.hasSkill("dcliexiang_extra")) {
			return info.replace("一名", `至多${get.cnNumber(player.countMark("dcliexiang_extra") + 1)}名（按任意顺序）`);
		}
		return info;
	},
	dcsbyinmou(player) {
	...
	dcsbjuemou(player) {
		...
		const start = `转换技。①游戏开始时，你可以转换此技能状态；②当你使用锦囊牌时，${player.storage.dcsbjuemou_rewrite ? "或回合开始/结束时，" : ""}你可以：`,
