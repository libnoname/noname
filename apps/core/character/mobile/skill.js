			...
			xiuge: {
				...
				filter(event, player) {
					...
					return list.some(name => {
						...
						if (name == "sha") {
							return [null].concat(lib.inpile_nature).some(nature => {
								const vcard = { name: "sha", isCard: true, storage: { wzxiuge: true } };
								if (nature) {
									vcard.nature = nature;
								}
								return event.filterCard(new lib.element.VCard(vcard), player, event);
							});
						}
					...
					for (const name of list) {
						...
						if (name == "sha") {
							return [null].concat(lib.inpile_nature).some(nature => {
								...
								return filter(new lib.element.VCard(vcard), player, event);
							});
						}
						const vcard = new lib.element.VCard({ name: name, isCard: true, storage: { wzxiuge: true } });
				...
				check(card) {
					...
					if (_status.event.type == "phase") {
						...
						const name = ["sha", "shan", "tao", "jiu"][subtype?.slice(5) - 1];
						if (name) {
							let value = 0;
							if (name == "sha") {
								for (const nature of [null].concat(lib.inpile_nature)) {
									const vcard = { name: "sha", isCard: true, storage: { wzxiuge: true } };
									if (nature) {
										vcard.nature = nature;
									}
									value = Math.max(value, player.getUseValue(vcard));
								}
							} else {
								value = player.getUseValue({ name: name, isCard: true, storage: { wzxiuge: true } });
							}
							if (value > 0) {
							...
							const result = await player
								...
								.set("ai", () => {
									...
									return get
										...
										.controls.slice()
										.sort((a, b) => {
											const cardA = { name: "sha", nature: a == "普通杀" ? null : a };
											const cardB = { name: "sha", nature: b == "普通杀" ? null : b };
											return player.getUseValue(cardB) - player.getUseValue(cardA);
										})[0];
								})
								.forResult();
