...
const translates = {
	...
	jinghe: "经合",
	jinghe_info: `出牌阶段限一次，你可以展示至多四张牌名各不相同的牌并选择等量的角色。系统从“写满技能的天书”（${["releiji", "rebiyue", "new_retuxi", "remingce", "xinzhiyan", "nhyinbing", "nhhuoqi", "nhguizhu", "nhxianshou", "nhlundao", "nhguanyue", "nhyanzheng"].map(skill => get.poptip(skill)).join("、")} ）中随机选择等量的技能，然后这些角色依次选择获得其中的一个直到你的下回合开始。`,
	nhyinbing: "阴兵",
	...
	zhuili: "惴栗",
	zhuili_info: "锁定技。当你成为其他角色使用黑色牌的目标后，若你的〖漂萍〗：处于阳状态，则你将〖漂萍〗转换至阴状态；处于阴状态，则你令〖托献〗发动次数+1，然后若〖托献〗发动次数大于3，则〖惴栗〗于本回合内失效。",
	fengfang: "冯方",
	dcditing: "谛听",
	...
	shiyi: "是仪",
	dccuichuan: "榱椽",
	dccuichuan_info: "出牌阶段限一次。你可以弃置一张手牌并选择一名角色，其随机使用牌堆里一张其空置装备栏对应副类别且其能对其使用的装备牌，你摸X张牌（X为其装备区里的牌数）。然后若其装备区里的牌数增加至四张，你失去〖榱椽〗，获得〖佐谏〗，且令其在此回合结束后获得一个额外回合。",
	dczhengxu: "正序",
	...
	dcquxian: "驱险",
	dcquxian_info: "回合开始与结束时，你可以从牌堆获得一张【杀】并选择一名角色，攻击范围内包含其的角色可以依次对其使用一张【杀】，每有一名角色使用【杀】你摸一张牌。若其未以此法受到伤害，这些角色中未使用【杀】的其他角色失去X点体力（X为这些角色中使用【杀】的角色数）。",
	star_xunyu: "星荀彧",
	star_xunyu_prefix: "星",
	staranshu: "安庶",
	...
	cuilie: "崔烈",
	dczijue: "赀爵",
	dczijue_info: "出牌阶段限一次，你可令一名其他角色声明2-4中的一个数字，你可交给其X张牌并回复1点体力，然后其他角色计算与你的距离和你的拼点牌点数+X直到你的下回合开始且你拼点赢时摸一张牌；若你未交给其牌，则你摸X张牌（X为其声明的数字）。",
	dcchibi: "斥避",
	...
	starfanglang: "放浪",
	starfanglang_info: "摸牌阶段结束时，你可以展示一张此阶段获得的牌，然后直到你的下回合开始，除展示的牌之外，你每回合使用或打出第一张牌时，你摸X张牌（X为此牌与展示牌的类别、花色、点数相同的项数）。结束阶段，你可以弃置一张牌，然后你获得弃牌堆中与此牌类别、点数、花色相同的牌各一张牌。",
	star_xiahouba: "星夏侯霸",
	star_xiahouba_prefix: "星",
	star_weigu: "维谷",
	star_weigu_info: "你使用伤害牌指定唯一目标或成为伤害牌唯一目标时，你可以弃置一张可指定自己为目标（toself为true或牌面目标合法）的牌，然后选择一项:1、移动场上一张牌；2、令你攻击范围内的所有角色也成为此牌目标（不包括此牌使用者）。此牌结算后若牌未造成伤害，你失去1点体力并摸两张牌。",
	star_juefa: "绝伐",
	star_juefa_info: "限定技，出牌阶段，你可以将“维谷”中“移动场上一张牌”改为“对一名角色造成2点伤害”直到你的下个回合结束。若如此做，当你于此期间通过“维谷”杀死角色后，你将手牌数和体力值调整至体力上限；若于此期间你未杀死过角色，效果结束时你失去所有体力",
	caobao: "曹豹",
	yanjiu: "厌酒",
