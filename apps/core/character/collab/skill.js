...
const skills = {
	...
	oldianbu: {
		...
		forced: true,
		trigger: {
			player: ["phaseBegin", "respondAfter"],
		},
		filter(event, player) {
			if (event.name == "phase") {
			...
			if (!event.cards?.length || event.skill != "oljuhun_doudizhu") {
				return false;
			}
			return !player.getStorage("oldianbu_used").includes(event.cards.length);
		},
		async content(event, trigger, player) {
			if (trigger.name == "phase") {
				...
				player.markAuto(event.name + "_used", [trigger.cards.length]);
				const next = player.chooseUseTarget(get.autoViewAs({ name: "wuzhong", isCard: true }, "unsure"), true);
				player
					.when({ player: "gainAfter" })
					.filter(evt => evt.getParent(4) == next)
					.step(async (event, trigger, player) => {
						const cards = trigger.cards;
						if (cards?.length) {
							player.addGaintag(cards, "oldianbu");
						}
					});
				await next;
				if (trigger.cards.length == 4) {
					player.removeSkill(event.name + "_used");
				}
			}
		},
		mod: {
	...
	oljuhun: {
		...
		locked: false,
		changePoker(cards) {
			game.addGlobalSkill("oljuhun_clear");
		...
		filter(event, player) {
			return player.hasCards("h", card => card.name != "hschenzhi_poker");
		},
		async content(event, trigger, player) {
			const cards = player.getCards("h").filter(card => card.name != "hschenzhi_poker");
			get.info(event.name).changePoker(cards);
			player.addTempSkill(event.name + "_clear2");
		},
		group: "oljuhun_doudizhu",
		subSkill: {
			doudizhu: {
				enable: "phaseUse",
				audio: "oljuhun",
				filter(event, player) {
					return player.countCards("h", card => card.name == "hschenzhi_poker") >= 2;
				},
				filterCard(card, player) {
					if (card.name != "hschenzhi_poker") {
						return false;
					}
					const bool = (card, player) => {
					...
					if (nums.length == 1) {
						...
						return get.number(card) == nums[0] || (ui.selected.cards.length == 1 && find(nums[0]).includes(get.number(card)));
					} else {
						return (
							find(nums[0])
								.concat(find(nums[nums.length - 1]))
								.includes(get.number(card)) && !nums.includes(get.number(card))
						);
					}
					return true;
				},
				...
				filterOk() {
					...
					if ([2, 3, 4].includes(cards.length)) {
						...
						return !cards.some(card => get.number(card) != number);
					}
					if (cards.length == 5) {
						const nums = cards.map(card => get.number(card)).unique();
						if (nums.length != 5) {
							return false;
						}
						//特殊规则-斗地主
						if ([10, 11, 12, 13, 1].every(num => nums.includes(num))) {
							return true;
						} else if ([11, 12, 13, 1, 2].every(num => nums.includes(num))) {
							return false;
						} else if ([12, 13, 1, 2, 3].every(num => nums.includes(num))) {
							return false;
						} else if ([13, 1, 2, 3, 4].every(num => nums.includes(num))) {
							return false;
						} else if ([1, 2, 3, 4, 5].every(num => nums.includes(num))) {
							return false;
						} else if ([2, 3, 4, 5, 6].every(num => nums.includes(num))) {
							return false;
						}
						for (const num of nums) {
							if (!nums.some(n => [n + 1, n - 1].includes(num))) {
								return false;
							}
						}
						return true;
				...
				check: () => 1,
				prompt: "请选择你要打出的牌型（对子：视为使用一张基本牌（不计入次数）；三条：获得相邻角色各一张牌；炸弹：造成2点伤害；顺子：你将一名角色随机两张牌变为扑克牌）",
				discard: false,
				lose: false,
				delay: 0,
				hiddenCard(player, name) {
					if (get.type(name) == "basic" && player.countCards("h", card => card.name == "hschenzhi_poker") >= 2) {
						return true;
					}
				},
				async content(event, trigger, player) {
					...
					switch (cards.length) {
						case 2: {
							...
							const list = get.inpileVCardList(info => {
								...
								return evt.filterCard(get.autoViewAs({ name: info[2], nature: info[3], isCard: true }, "unsure"), player, event);
							});
							...
							const result = await player
								.chooseButton({
									createDialog: ["拘魂：视为使用一张基本牌（不计入次数）", [list, "vcard"]],
									forced: true,
									ai(button) {
										return player.getUseValue(get.autoViewAs({ name: button.link[2], nature: button.link[3], isCard: true }, "unsure"));
									},
								})
								.forResult();
							if (result?.bool && result.links?.length) {
								...
								game.broadcastAll(link => {
									lib.skill.oljuhun_backup.viewAs = {
										name: link[2],
										nature: link[3],
										isCard: true,
									};
									lib.skill.oljuhun_backup.prompt = "选择" + get.translation(link[2]) + "的目标";
								}, link);
								evt.set("_backupevent", "oljuhun_backup");
								evt.backup("oljuhun_backup");
								evt.set("openskilldialog", "选择" + get.translation(link[2]) + "的目标");
						...
						case 3: {
							const targets = [player.getNext(), player.getPrevious()]
								.unique()
								.filter(target => target.hasGainableCards(player, "he"))
								.sortBySeat();
							if (targets.length) {
						...
						case 5: {
							const targets = game.filterPlayer(current => current.hasCards("h", card => card.name != "hschenzhi_poker"));
							if (targets.length) {
								const result = await player
									.chooseTarget({
										prompt: "拘魂：选择一名角色令其随机两张手牌变为扑克牌",
								...
								if (result?.bool && result.targets?.length) {
									...
									const cards = target
										.getCards("h")
										.filter(card => card.name != "hschenzhi_poker")
										.randomGets(2);
									if (cards.length) {
										get.info("oljuhun").changePoker(cards);
									}
								}
							}
							break;
			...
			clear2: {
				...
				filter(event, player) {
					return player.hasCards("h", card => card.name == "hschenzhi_poker" && card.storage?.oljuhun_clear);
				},
				async content(event, trigger, player) {
					const cards = player.getCards("h", card => card.name == "hschenzhi_poker" && card.storage?.oljuhun_clear);
					get.info("oljuhun").clearPoker(cards);
				},
			},
			clear: {
				charlotte: true,
				...
				mod: {
					cardEnabled2(card, player) {
						const evt = get.event();
						if (evt.skill == "oljuhun_doudizhu") {
							return;
						}
						if ([card].concat(card.cards || []).some(card => card.name == "hschenzhi_poker" && card.storage?.oljuhun_clear)) {
							return false;
						}
					},
				},
				...
				trigger: {
					player: "loseEnd",
					global: ["gainEnd", "loseAsyncEnd", "addJudgeEnd", "addToExpansionEnd", "equipEnd"],
				},
				filter(event, player) {
					const evt = event.getl?.(player);
					return evt?.hs?.some(card => card.name == "hschenzhi_poker" && card.storage?.oljuhun_clear);
				},
				async content(event, trigger, player) {
					const cards = trigger.getl(player).hs.filter(card => card.name == "hschenzhi_poker" && card.storage?.oljuhun_clear);
					get.info("oljuhun").clearPoker(cards);
				},
			},
		},
