...
const skills = {
			...
			event.result = await player
				.chooseTarget(
					get.prompt2(event.skill) +
						`<br><span class=bluetext>上次弃牌数：${storage[0] || 0}<br>最后执行选项：${map[storage[1]]?.[0] || "无"}</span>`,
					(card, player, target) => {
	...
	sbqiaobian: {
		...
		async cost(event, trigger, player) {
			const list = trigger
				.getParent("phase", true)
				?.phaseList.filter(name => !["skip", "phaseZhunbei", "phaseJieshu"].some(i => name.startsWith(i)))
				.map(name => name.split("|")[0])
				.unique();
			if (!list.length) {
				return;
			}
			const createDialog = [get.prompt2(event.skill), [list.map(i => [i, get.translation(i)]), "tdnodes"]];
			let num = 0;
			[
				["h", "手牌区"],
				["e", "装备区"],
				...
				if (player.countCards(pos)) {
					createDialog.push(`<div class="text center" style="margin: 0px;">${str}</div>`);
					createDialog.push(player.getCards(pos));
					num++;
				}
			});
			const result = await player
				...
				.set("ai", button => {
					...
					if (typeof link != "string") {
						if (get.position(link) == "j") {
							return -get.info(link).ai.result.target(player, player) + 10;
						}
						return 7 - get.value(link);
					} else {
						switch (link) {
							case "phaseJudge": {
							...
							case "phaseUse":
								if (!player.canMoveCard(true)) {
									...
									if (
										...
										if (player.countCards("h", card => player.hasValueTarget(card)) > 2) {
											return 0;
										}
									}
									return 4;
				...
				.set("filterOk", () => {
					...
					const area = links.filter(i => typeof i == "string");
					const cards = links.slice().removeArray(area);
					return area.length == cards.length;
				})
				.forResult();
			if (result?.bool && result.links?.length) {
				const { links } = result;
		...
		async content(event, trigger, player) {
			const { cards, cost_data: list } = event;
		...
		subSkill: {
			phaseDraw: {
				charlotte: true,
				trigger: {
					player: ["Skipped", "Cancelled"].map(i => `phaseDraw${i}`),
				},
				filter(event, player) {
					return game.hasPlayer(target => target != player && target.countGainableCards(player, "h") > 0);
				},
				async cost(event, trigger, player) {
					event.result = await player
						.chooseTarget(get.prompt(event.skill), `获得至多两名其他角色各一张手牌`, [1, 2], (card, player, target) => {
							return target != player && target.countGainableCards(player, "h") > 0;
						})
						.set("ai", target => get.effect(target, { name: "shunshou_copy2", position: "h" }, get.player(), get.player()))
						.forResult();
				},
				async content(event, trigger, player) {
					player.removeSkill(event.name);
					await game.doAsyncInOrder(event.targets, async target => player.gainPlayerCard(target, "h", true).set("gaintag", ["sbqiaobian"]));
				},
			},
			phaseUse: {
				charlotte: true,
				trigger: {
					player: ["Skipped", "Cancelled"].map(i => `phaseUse${i}`),
				},
				filter(event, player) {
			...
			phaseDiscard: {
				charlotte: true,
				trigger: {
					player: ["Skipped", "Cancelled"].map(i => `phaseDiscard${i}`),
				},
				forced: true,
				locked: false,
				async content(event, trigger, player) {
	...
	sbliaoshi: {
		...
		usable: 1,
		filter(event, player) {
			return get.info("sbliaoshi").getList(event, player).length && player.hasCard(card => card.hasGaintag("sbqiaobian"), "h");
		},
		chooseButton: {
			dialog(event, player) {
				const list = get.info("sbliaoshi").getList(event, player);
				return ui.create.dialog("料势", [list, "vcard"], "hidden");
			},
			check(button) {
			...
			prompt(links, player) {
				return `将一张“巧变”当作${get.translation(links[0][3]) || ""}【${get.translation(links[0][2])}】使用或打出`;
			},
		},
		ai: {
			combo: "sbqiaobian",
			...
			skillTagFilter(player) {
				if (!player.hasCard(card => card.hasGaintag("sbqiaobian"), "h")) {
					return false;
				}
			},
			/*tag: {
				...
				if (types.length >= 3) {
					...
					const juedou = game.hasPlayer(
						current =>
							current != player &&
							current != target &&
							player.canUse(new lib.element.VCard({ name: "juedou", isCard: true }), current, false)
					);
					...
					if (juedou || wushuang) {
						...
						if (result.control == "选项一") {
							...
							if (result2?.bool && result2.targets?.length) {
								...
								await game.delayx();
								await player
									.useCard(new lib.element.VCard({ name: "juedou", isCard: true }), result2.targets[0], false, "noai")
									.set("animate", false);
							}
						} else {
			...
			const result = await player
				...
				.set("ai", button => {
					...
					const effectMap = new Map([
						[
							"phaseJudge",
							-player.getVCards("j").reduce((sum, card) => sum + get.effect(player, { name: card.name }, player, player), 0),
						],
						["phaseDraw", -get.effect(player, { name: "wuzhong" }, player, player)],
						[
							"phaseUse",
							-player.getCards("hs", card => player.hasValueTarget(card)).reduce((sum, card) => sum + player.getUseValue(card), 0),
						],
							...
							? (
									await player
										.chooseButton(
											[
												"神速：请选择你要视为使用的【杀】",
												'<div class="text center">无距离限制' + (goon ? "且不可被响应" : "") + "</div>",
												[vcard, "vcard"],
											],
											true
										)
										.set("ai", button => {
											...
											return Math.max(
												...game
													.filterPlayer(target => player.canUse(card, target, false))
						...
						while (true) {
							...
							if (arr[0] == "none") {
								...
								player.logSkill(
									...
									null,
									null,
									player.storage.sbjingceCount >= 6 ? [get.rand(7, 8)] : [get.rand(3, 4)]
								);
		...
		async cost(event, trigger, player) {
			...
			event.result = await player
				.chooseCard(
					...
					"将本次获得的任意张牌置于弃牌堆，然后获得至多等量名其他角色的各一张手牌",
		...
		async content(event, trigger, player) {
			...
			if (["equip", "all"].includes(control) && target.countCards("e")) {
				const { cards } = await player
					.choosePlayerCard(target, true, "e", `选择${get.translation(target)}的至多两张装备牌令其获得之`, [1, 2])
					.forResult();
				if (cards?.length) {
		...
		intro: {
			content(storage) {
				return (
					"出牌阶段开始时，" +
					(storage
						? "你进行判定并获得判定牌，然后若判定结果与你本局游戏因〖天妒〗弃置的牌花色相同，你受到1点无来源伤害。"
						: "你可以弃置两张手牌，然后视为使用一张普通锦囊牌")
				);
			},
		},
		audio: 2,
		...
		async content(event, trigger, player) {
			...
			if (storage) {
				...
				const { bool, links } = await player
					.chooseButton([`天妒：视为使用其中一张锦囊牌`, [list, "vcard"]])
					.set("ai", button => {
						const player = get.player(),
							{ link } = button;
	...
	sbhuanshi: {
		...
		async content(event, trigger, player) {
			...
			const result = await player
				.chooseCard(
					`缓释：${get.translation(trigger.player)}的${trigger.judgestr || ""}判定为${get.translation(trigger.player.judging[0])}，是否打出牌堆顶的牌替代之或打出一张手牌替换之？`,
					"hs",
					card => {
						const player = get.player();
			...
			const next = player.respond(cards, event.name, "highlight", "noOrdering");
			await next;
			cards = next.cards;
			if (!cards?.length) {
				return;
		...
		async cost(event, trigger, player) {
			...
			while (true) {
				result = await player
					...
					.set(
						"prompt2",
						"你可以消耗任意点蓄力值并选择一项：⒈你于本轮内至其他角色的距离-1，令系统选择牌堆中的X张【杀】；⒉其他角色于本轮内至你的距离+1，令系统选择牌堆中的X张【闪】（X为你消耗的蓄力值）。然后若你的“扈”数小于4，你将系统选择的牌置于武将牌上，称为“扈”。"
					)
					.set("ai", () => {
						return get.event().choice;
					})
				...
				mod: {
					cardEnabled(card, player) {
						if (
							...
							_status.currentPhase.hasSkill("sbjinjiu")
						) {
							return false;
						}
					},
	...
	sbquhu: {
		...
		async content(event, trigger, player) {
			...
			targets.sortBySeat();
			const result = await player
				.chooseCardOL(targets, "he", [1, Infinity], true, "驱虎：请将任意张牌扣置于武将牌上", "allowChooseAll")
				.set("ai", card => {
	...
	sbjieming: {
		...
		async cost(event, trigger, player) {
			...
			event.result = await player
				.chooseTarget(
					get.prompt(event.skill),
					`令一名角色摸四张牌，然后其可以弃置任意张牌。若其弃置的牌数小于${get.cnNumber(num)}张，你失去1点体力。`
				)
				.set("ai", target => {
					if (get.event().nope) {
						return 0;
		...
		async content(event, trigger, player) {
			...
			const { bool, cards } = await target
				.chooseToDiscard(
					"节命：是否弃置任意张牌？",
					`若你本次弃置的牌数小于${get.cnNumber(num)}张，${get.translation(player)}失去1点体力。`,
					[1, Infinity],
					"he",
					"allowChooseAll"
				)
	...
	sbfangzhu: {
		getList: [
			...
			},
			{
				...
				filter: player =>
					get.mode() != "doudizhu" && game.hasPlayer(target => target != player && !target.getStorage("sbfangzhu_ban").includes("equip")),
				filterTarget: {
	...
	sbqicai: {
		...
		chooseButton: {
			...
			check(button) {
				...
				if (
					!game.hasPlayer(target => target != player && target.hasEmptySlot(get.subtype(button.link)) && get.attitude(player, target) > 0)
				) {
		...
		subSkill: {
			...
			achieve: {
				...
				filter(event, player) {
					return (
						player
							.getAllHistory("sourceDamage", evt => evt.hasNature("fire") && evt.player != player)
			...
			mark: {
				...
				content() {
					...
					player.storage.sbhuoji_count = player
						.getAllHistory("sourceDamage", evt => evt.hasNature("fire") && evt.player != player)
						.reduce((num, evt) => num + evt.num, 0);
	...
	sbtianxiang: {
		...
		filter(event, player) {
			return (
				player.countCards("h", card => lib.skill.sbtianxiang.filterCard(card, player)) &&
				game.hasPlayer(target => lib.skill.sbtianxiang.filterTarget(null, player, target))
			);
		},
		filterCard(card, player) {
	...
	old_sbqiaobian: {
		...
		async cost(event, trigger, player) {
			...
			switch (trigger.name) {
				...
				case "phaseDraw":
					event.result = await player
						.chooseBool(get.prompt(skillName), "跳过摸牌阶段，于下个准备阶段摸五张牌并回复1点体力")
						.setHiddenSkill(skillName)
						.forResult();
					break;
				case "phaseUse": {
					...
					if (num <= 0) {
						next = player
							.chooseBool(get.prompt(skillName), "跳过出牌阶段和弃牌阶段，然后移动场上的一张牌")
							.set("choice", player.canMoveCard(true))
							.setHiddenSkill(skillName);
					} else {
						next = player
							.chooseToDiscard(
								...
								num,
								`弃置${get.cnNumber(num)}张手牌并跳过出牌阶段和弃牌阶段，然后移动场上的一张牌`,
								"allowChooseAll"
							)
							.set("ai", card => {
								const player = get.player();
	...
	sbzaiqi: {
		...
		content() {
			...
			player
				.chooseTarget(
					get.prompt("sbzaiqi"),
					"选择任意名角色并消耗等量蓄力值，令这些角色选择一项：1.令你摸一张牌；2.弃置一张牌，然后你回复1点体力",
					[1, player.countMark("charge")]
				)
				.set("ai", function (target) {
					var player = _status.event.player;
					var att = get.attitude(player, target);
			...
			if (!target.countCards("he")) {
				...
				target
					.chooseToDiscard(
						get.translation(player) + "对你发动了【再起】",
						"是否弃置一张牌令其回复1点体力？或者点击“取消”，令该角色摸一张牌。",
						"he"
					)
					.set("ai", card => {
	...
	sbjuxiang: {
		...
		content() {
			...
			player
				.chooseTarget(
					"请选择【巨象】的目标",
					"将游戏外的随机一张【南蛮入侵】交给一名角色（剩余" + get.cnNumber(_status.sbjuxiang_nanman.length) + "张）",
					true
				)
				.set("ai", target => {
					var player = _status.event.player;
				...
				ai: {
					...
					skillTagFilter(player, tag, arg) {
						if (tag === "nokeep") {
							...
							return (
								player.isPhaseUsing() &&
								(player.getStat("skill").sbjiang_qiben || 0) < limit &&
			...
			effect: {
				target(card, player, target) {
					...
					if (
						...
						((target.hasZhuSkill("sbzhiba") && game.countPlayer(current => current != target && current.group == "wu")) ||
		...
		subSkill: {
			...
			add: {
				...
				content() {
					...
					player
						.chooseTarget(
							get.prompt("sblianhuan_add"),
							"为" + get.translation(trigger.card) + "额外指定任意个目标",
							[1, Infinity],
							function (card, player, target) {
		...
		getEffect(event, player) {
			if (!event.tiaoxinEffect) {
				const targets1 = game.filterPlayer(current => {
						...
						return (
							get.effect(current, { name: "juedou" }, player, player) > 0 || current.countCards("h") <= player.countCards("hs", "sha")
						);
					}),
	...
	sbrende: {
		...
		chooseButton: {
			...
			prompt(links, player) {
				const isUse = links[0] !== "give";
				return isUse
					? "移去2枚“仁望”，视为使用或打出" + (get.translation(links[0][3]) || "") + get.translation(links[0][2])
					: "###仁德###出牌阶段每名角色限一次。你可以将任意张牌交给一名其他角色，然后你获得等量“仁望”标记（至多为" +
							lib.skill.sbrende.maxNum +
							"）";
			},
		},
		hiddenCard(player, name) {
		...
		subSkill: {
			...
			give: {
				...
				prompt(event) {
					return (
						"出牌阶段每名角色限一次。你可以将任意张牌交给一名其他角色，然后你获得等量“仁望”标记（至多为" + lib.skill.sbrende.maxNum + "）"
					);
				},
				ai1(card) {
		...
		async cost(event, trigger, player) {
			event.result = await player
				...
				.set("ai", target => {
					...
					return curs.reduce(
						(max, i) => Math.max(max, get.effect(i, new lib.element.VCard({ name: "sha", isCard: true }), target, player)),
						-1
					);
				})
				.forResult();
		...
		async content(event, trigger, player) {
			...
			if (!targets[0].canUse({ name: "sha", isCard: true }, targets[1], false)) {
				...
				result = await targets[0]
					...
					.set(
						"choice",
						get.effect(targets[1], { name: "sha" }, targets[0], targets[0]) >
							get.effect(targets[0], { name: "lebu" }, targets[0], targets[0])
							? 0
							: 1
					)
					.forResult();
	...
	sbjizhu: {
		...
		async cost(event, trigger, player) {
			event.result = await player
				...
				.set("ai", function (target) {
					return (
						get.threaten(target) * Math.sqrt(1 + target.countCards("h")) * (target.isTurnedOver() || target.hasJudge("lebu") ? 0.1 : 1)
					);
				})
				.forResult();
		},
		...
		subSkill: {
			effect: {
				...
				filter(event, player) {
					return (
						player.checkCooperationStatus(event.player, "sbjizhu") &&
						["sblongdan", "jdlongdan"].some(skill => player.hasSkill(skill, null, null, false))
					);
				},
				content() {
					...
					player
						.chooseTarget(
							...
							"你和" +
								...
								"协力成功，可以视为对至多三名其他角色使用一张【杀】，且此【杀】造成伤害时，你摸等同于伤害值的牌",
							[1, 3],
							true,
							function (card, player, target) {
	...
	sbshipo: {
		...
		content() {
			...
			if (result.control == "选项一") {
				player
					.chooseTarget("选择一名体力少于你的角色", (card, player, target) => target.hp < player.hp, true)
					.set("ai", target => -get.attitude(player, target) / Math.sqrt(Math.max(0.1, 2 * target.hp + target.countCards("h"))));
			} else {
				event._result = {
					bool: true,
		...
		content() {
			...
			player
				...
				.set("translationList", [
					...
					`以防止${get.translation(player)}获得你一张牌`,
					`若成功，你获得${get.translation(target)}一张牌`,
					`若成功，你摸两张牌`,
				])
				.set("ai", button => {
			...
			result: {
				target(player, target) {
					if (
						game.hasPlayer(current => {
							return (
								(!get.rawAttitude || get.rawAttitude(player, current) > 0) && current != player && get.attitude(player, current) <= 0
							);
						}) &&
						game.countPlayer(current => {
		...
		subSkill: {
			effect: {
				...
				content() {
					"step 0";
					var choiceList = [
						"令" + get.translation(trigger.target) + "获得其“威”",
						"移去" + get.translation(trigger.target) + "的“威”，取消" + get.translation(trigger.card) + "对其的目标",
					];
					player
						.chooseControl()
						.set("choiceList", choiceList)
	...
	sbhujia: {
		...
		content() {
			...
			player
				.chooseTarget(
					get.prompt("sbhujia"),
					"将" + get.translation(trigger.source) + "即将对你造成的" + trigger.num + "点伤害转移给一名其他魏势力角色",
					(card, player, target) => {
		...
		subSkill: {
			draw: {
				...
				content() {
					...
					player
						.chooseTarget(get.prompt("sbyanyu"), "令一名其他角色摸" + get.cnNumber(event.num) + "张牌", lib.filter.notMe)
						.set("ai", target => {
							var player = _status.event.player;
							return get.effect(target, { name: "draw" }, player, player);
						});
			...
			skillTagFilter(player, tag, arg) {
				if (tag === "nokeep") {
					return (
						...
						player.isPhaseUsing() &&
						!player.getStat().skill.sbzhiheng &&
						player.hasCard(card => get.name(card) !== "tao", "h")
					);
				}
			},
	...
	sbjiuyuan: {
		...
		filter(event, player) {
			return (
				...
				event.player.isIn() &&
				player.hasZhuSkill("sbjiuyuan", event.player)
			);
		},
		content() {
			player.draw();
	...
	sbxiayuan: {
		...
		filter(event, player) {
			return (
				...
				!player.hasSkill("sbxiayuan_round", null, false, false)
			);
		},
		content() {
			"step 0";
	...
	sbjieyue: {
		...
		content() {
			"step 0";
			player
				.chooseTarget(lib.filter.notMe, get.prompt("sbjieyue"), "令一名其他角色获得1点护甲，然后该角色可以交给你一张牌。")
				.set("ai", function (target) {
	...
	sbliegong: {
		...
		filter(event, player) {
			return (
				!event.getParent()._sbliegong_player &&
				event.targets.length == 1 &&
				event.card.name == "sha" &&
				player.getStorage("sbliegong").length > 0
			);
		},
		prompt2(event, player) {
			let str = "",
		...
		subSkill: {
			...
			count: {
				...
				content() {
					...
					player.addTip(
						"sbliegong",
						get.translation("sbliegong") + player.getStorage("sbliegong").reduce((str, suit) => str + get.translation(suit), "")
					);
				},
			},
		},
	...
	spyingwu: {
		...
		filter(event, player) {
			return (
				player.hasSkill("splveying", null, null, false) &&
				get.type(event.card) == "trick" &&
				!get.tag(event.card, "damage") &&
				player.countMark("splveying") > 1
			);
		},
		async content(event, trigger, player) {
			player.removeMark("splveying", 2);
		...
		subSkill: {
			add: {
				...
				filter(event, player) {
					return (
						player.hasSkill("splveying") &&
						get.type(event.card) == "trick" &&
						!get.tag(event.card, "damage") &&
						player.isPhaseUsing() &&
						player.countMark("spyingwu_used") < 2
					);
				},
				async content(event, trigger, player) {
					player.addMark("splveying", 1);
	...
	spxianchou: {
		...
		async content(event, trigger, player) {
			...
			const next = target.useCard(card, trigger.source, false);
			await next;
			if (
	...
	new_spxianchou: {
		...
		async content(event, trigger, player) {
			...
			const result = await target
				.chooseBool(`是否视为对${get.translation(trigger.source)}使用一张杀？`)
