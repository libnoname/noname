...
const translates = {
	...
	sb_zhanghe: "谋张郃",
	sbqiaobian: "巧变",
	sbqiaobian_info: "准备阶段，你可以弃置你区域里的至多三张牌张牌，跳过等量个阶段（判定阶段和结束阶段除外），并于跳过对应阶段后执行对应效果。若以此法跳过：摸牌阶段，你可以获得至多两名其他角色各一张手牌；出牌阶段，你可以移动场上的一张牌，然后若你的牌数未增加，你可以令牌数因此增加的角色摸两张牌；弃牌阶段，你摸两张牌，然后结束阶段，若你的手牌数不小于场上角色数，你执行一个额外的出牌阶段（你因此获得的牌或移动至你装备区的牌称为“巧变”牌）。",
	sbliaoshi: "料势",
